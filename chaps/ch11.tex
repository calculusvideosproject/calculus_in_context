...
\footnote{Undo a differentiation}
...
\footnote{A function has  many antiderivatives}	
...
\footnote{What the `$+\ C$' term really means} 
...
\footnote{Antiderivatives  of basic functions}	
...
a)$.
\includegraphics{../ps/calc2/ch11-1.pdf}
...
\mbox{}
\includegraphics{../ps/calc2/ch11-2a.pdf}
\includegraphics{../ps/calc2/ch11-2b.pdf}
...
\includegraphics{../ps/calc2/ch11-3a.pdf}
\includegraphics{../ps/calc2/ch11-3b.pdf}
...
represents an antiderivative of $f$.  
	\footnote{Write an antiderivative as an integral}
...
With this new notation\index{notation!antiderivative} the antiderivatives we have listed so far can be written \index{antiderivative} 
	\footnote{The basic antiderivatives again}
...
\footnote{The \emph{definite} integral\\ is a number\ldots}
...
\footnote{\ldots while the \emph{indefinite} integral is a function}
...
\footnote{The calculation  doesn't depend on $C$,  so take $C = 0$}
...
\footnote{We may not recognize the \mbox{antiderivative\ldots}}	
...
\footnote{\ldots and there may even be no formula for it}	
...
\footnote{We differentiate  to verify a statement  about antiderivatives} 
...
\footnote{The constant multiple and addition rules}
...
\footnote{Trial and error}\index{trial and error}
...
\footnote{Tables of Integrals}\index{integrals!table of}
...
\footnote{Integration can  now be done  quickly and efficiently  by computer software}
...
\footnote{The integral form  of the chain rule}
...
\footnote{Reduction methods transform problems into equivalent, simpler, problems} 
...
\footnote{A compact notation for expressing the integral form of the chain rule} 
...
\footnote{An integral form  of the product rule}
...
\footnote{Integrate only part  of the integrand}
...
set routine.
\footnote{The ingredients  of a successful  integration by parts}	
...
integral is {\em not\/} simpler than the original.  \footnote{What went
...
As you know, 
\footnote{A new method for solving $y' = y$}	
...
\footnote{Find the inverse function instead}
...
\footnote{The new differential equation\ldots}
	$$
...
\end{align*}
Thus $y(t) = Ce^t$, 
\footnote{\ldots solves the  original problem}
...
\footnote{The differential equation expressed using differentials}
...
\footnote{The variables  are now \emph{separated}}
...
\footnote{The solution  once again}
we solve for~$y$:
	$$
...
\footnote{To integrate,  separate the variables}
...
\footnote{Separate the variables}
...
\footnote{Bring in the  initial condition}
...
\footnote{Interpreting  the formula}
...
\footnote{Step 1: separate  the variables}	
...
Integrating the new equation we get
	...
	$$
We are stuck now, 
\footnote{The denominator has an unfamiliar form}
...
\footnote{Step 2: write the integrand as a sum  of simple fractions}
...
\footnote{Determining   $\alpha$ and $\beta$}
...
equal, it follows that
	...
	$$
Thus $\alpha = \beta = 1$, 
\footnote{The partial fractions decomposition}
and we have 
	$$
...
\footnote{Step 3: evaluate  the integrals}
...
\footnote{The formula for $P(t)$}
by the coefficient $1 + Be^{kt}$:
	$$
	P(t) = \dfrac{CBe^{kt}}{1 + Be^{kt}}.
	$$
Lastly, 
\footnote{Step 5: incorporate  the initial condition}
...
\footnote{The complete solution}
...
\footnote{The final form  of the antiderivative  of the cosecant}
...
\footnote{The antiderivative  of the secant}
...
\footnote{Success sometimes comes by making things more complicated} 
...
\footnote{Some useful substitutions}
...
\footnote{The antiderivative  of the cotangent}\index{cotangent!antiderivative of}
...
\footnote{The antiderivative  of the tangent}\index{tangent!antiderivative of}
...
\footnote{In its current form,  the reduction formula works best for $n>0$}
...
\footnote{The reduction formula for negative exponents}
...
\mbox{}
\end{enumerate}
\item Use the analysis of Example 17 (page~\pageref{ex17}) to find an
...
\footnote{A return to  numerical methods}	
...
\footnote{Efficient numerical integration}	
...
\footnote{Replace rectangles  by trapezoids}
...
graph.
\includegraphics{../ps/calc2/trap1.pdf}
Let's figure out the areas of these trapezoids.  
\footnote{Each trapezoid is sandwiched between two rectangles\ldots}	
...
trapezoid.  
\includegraphics{../ps/calc2/trap2.pdf} 
It is sandwiched between  two rectangles, one taller and one shorter.  
\footnote{\ldots whose average area equals the area of the trapezoid}
...
\footnote{The trapezoidal approximation is the average of left and right Riemann sums} 
...
\footnote{Comparing approximations} However, we will use equal subintervals
...
\footnote{A surprise:  the midpoint approximation is even better than the trapezoidal}	
...
\mbox{}
\includegraphics{../ps/calc2/trap3.pdf}
The second step is to compare 
\footnote{Shading depicts  the errors}	
...
\footnote{Combine good approximations\ldots}
...
\footnote{\ldots so that most of the error cancels}
...
\footnote{Using Riemann sums to carry out  Simpson's rule}	
...
\footnote{The accuracy of Simpson's rule}	
...
\footnote{An error bound for Simpson's rule}\index{error bound}
...
\footnote{The crucial factor $n^{-4}$}
...
\footnote{The lifetime of a light bulb is unpredictable}	
...
\footnote{Lifetime is the integral of burnout rate\ldots}
...
\footnote{\ldots but there is no upper limit to the lifetime}
...
\footnote{An integral is {\em improper\/} if the domain of integration is
  infinite}
...
\footnote{The normal probability distribution involves  an improper integral}
...
\footnote{An integral is also improper if its integrand becomes infinite}	
...
\footnote{Antiderivatives help find improper integrals}	
