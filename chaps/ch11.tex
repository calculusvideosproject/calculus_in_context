...
[[NOTE:A function has  many antiderivatives]]	
...
[[NOTE:What the `$+\ C$' term really means]] 
...
[[NOTE:Antiderivatives  of basic functions]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/ch11-1.pdf}
...
\label{subsubsec:inverse-trigonometric-functions}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/ch11-2a.pdf}

\noindent \includegraphics[width=\linewidth]{../ps/calc2/ch11-2b.pdf}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/ch11-3a.pdf}

\noindent \includegraphics[width=\linewidth]{../ps/calc2/ch11-3b.pdf}
...
represents an antiderivative of $f$.  
	[[NOTE:Write an antiderivative as an integral]]
...
With this new notation\index{notation!antiderivative} the antiderivatives we have listed so far can be written \index{antiderivative} 
	[[NOTE:The basic antiderivatives again]]
...
[[NOTE:The \emph{definite} integral  is a number\ldots]]
...
[[NOTE:\ldots while the \emph{indefinite} integral is a function]]
...
\label{subsec:using-antiderivatives}
...
{\bf [[EX:1]]}\quad  Find $\dint_1^4 x^2 \, dx$.  We have that
	\[
...
{\bf [[EX:2]]}\quad Find $\dint_0^{\pi/2} \cos{t} \, dt$.  This time
...
[[NOTE:The calculation  doesn't depend on $C$,  so take $C = 0$]]
...
[[NOTE:We may not recognize the \mbox{antiderivative\ldots}]]	
...
[[NOTE:\ldots and there may even be no formula for it]]	
...
\label{subsec:finding-antiderivatives}
...
$\displaystyle \int f(x)\,dx = F(x)+C \mbox{\qquad and \qquad} F' = f.$
}}}
\end{picture}

\end{center}
In other words, a statement 
[[NOTE:We differentiate  to verify a statement  about antiderivatives]] 
...
[[NOTE:The constant multiple and addition rules]]
...
{\bf [[EX:3]]}\quad This example illustrates the use of both the
...
[[NOTE:Trial and error]]\index{trial and error}
...
[[NOTE:Tables of Integrals]]\index{integrals!table of}
...
[[NOTE:Integration can  now be done  quickly and efficiently  by computer software]]
...
\begin{enumerate}
\item What is the inverse of the function $y = 1/x$?  Sketch the graph
of the function and its inverse.


\item What is the inverse of the function $y = 1/x^3$?  Sketch the
graph of the the function and its inverse.  Do the same for $y = 3x -
2$.


\item Let $\theta=\arctan{x}$.  Then $\tan{\theta} = x$.  Refer to the
...
What is the value of the constant?
  ...
  \begin{enumerate}
  \item Use part (a), and the derivative of $\arctan{x}$, to find the
derivative of $\arctan(1/x)$.

  \item Use the chain rule to verify your answer to part (b).


\end{enumerate}
\item The logarithm for the base $b$ is defined as the inverse to the
...
\item Define $\arccos{x}$,\index{function!arccosine} the inverse of
the cosine function.  Be sure to limit the domain of the cosine
function to an interval on which it is one-to-one.

  \begin{enumerate}
  \item Sketch the graph $y = \arccos{x}$.  How did you limit the range
of $y$?

  \item Determine $dy/dx$.


\end{enumerate}
\item If $\theta = \arcsin{x}$, refer to the picture on
...
constant?
  ...
  \begin{enumerate}
  \item Use part (a), and the derivative of $\arcsin{x}$, to determine
the derivative of $\arccos{x}$.  Does this result agree with what you
got in the last exercise?


\end{enumerate}
\item Find a formula for $\dint \dfrac{dx}{\sqrt{1 - x^2}}$.



\item Verify that the antiderivatives given in the table on
page~\pageref{table} are correct.


\item Find an antiderivative of each of the following functions.  Don't
hesitate to use the ``trial and error'' method of [[EX:4]] above.
\begin{center}
...
\item Find a formula for each of the following indefinite integrals.
  ...
  \item $\dint 7y + \dfrac{1}{y} \, dy$


  \item $\dint 7y - \dfrac{4}{y^2} \, dy$
		

  \item $\dint 5\sin{w} - 2\cos{w} \, dw$
  ...
  \item $\dint \dfrac{5}{1+r^2}\,dr$


  \item $\dint \dfrac{1}{\sqrt{1-4s^2}}\, ds$
\mbox{}



\end{enumerate}
\item Find an antiderivative $F(x)$ of $f(x) = 7$ for which $F(0) =
12$.

  \begin{enumerate}
  \item Find an antiderivative $G(x)$ of $f(x) = 7$ for which $G(3) = 1$.

  \item Do $F(x)$ and $G(x)$ differ by a constant?  If so, what is the
value of that constant?



\end{enumerate}
\item Find an antiderivative $F(t)$ of $f(t) = t + \cos{t}$ for which
$F(0) = 3$.

  \begin{enumerate}
  \item Find an antiderivative $G(t)$ of $f(t) = t + \cos{t}$ for which
$G(\pi/2) = -5$.

  \item Do $F(t)$ and $G(t)$ differ by a constant?  If so, what is the
value of that constant?


\end{enumerate}
\item Find an antiderivative of the function $a + by$ when $a$ and $b$
are fixed constants.


\item Verify that $(1 + x^3)^{10}$ is an antiderivative of $30x^2(1 +
x^3)^9$.

  \begin{enumerate}
  \item Find an antiderivative of $x^2(1 + x^3)^9$.

  \item Find an antiderivative of $x^2 + x^2(1 + x^3)^9$.


\end{enumerate}
\item Verify that $x \ln{x}$ is an antiderivative of $1 + \ln{x}$.

  \begin{enumerate}
  \item Find an antiderivative of $\ln{x}$.  [Do you see how you can use
  part (a) to find this antiderivative?]



\end{enumerate}
\item Recall that $F(y) = \ln(y)$ is an antiderivative of $1/y$ for
$y>0$.  According to the text, {\em every\/} antiderivative of $1/y$
over this domain must be of the form $\ln(y) + C$ for an appropriate
value of $C$.

  \begin{enumerate}
  \item Verify that $G(y) = \ln(2y)$ is also an antiderivative of $1/y$.

  \item Find $C$ so that $\ln(2y) = \ln(y) + C$.


\end{enumerate}
\item Verify that $-\cos^2{t}$ is an antiderivative of $2\sin{t} \,
...
\item Since\label{log} the function $\ln{x}$ is defined only when $x >
0$, the equation
	\[
\int \dfrac{1}{x} \, dx = \ln{x} + C
\]
...
integral (i.e., antiderivative) of $1/x$ is when $x < 0$.
  ...
  \begin{enumerate}
  \item When $x < 0$ then $ -x > 0$ so $\ln(-x)$ is defined.  In these
circumstances, show that $\ln(-x)$ is an antiderivative of $1/x$.

  \item Now put these two ``pieces'' of antiderivative together by
...
way that it is clear that $F(x)$ is an antiderivative of $1/x$.
  ...
  \item Explain why $F(x) = \ln{|x|}$.  For this reason a table of
...
\]
  ...
  \item Every function $\ln{|x|} + C$ is an antiderivative of $1/x$, but
...
way that it is clear that $G(x)$ is an antiderivative of $1/x$.
  ...
  \item Explain why there is no value of $C$ for which
	\[
\ln{|x|} + C = G(x).
\]
...
antiderivatives of $1/x$.
  ...
  \item Construct two more antiderivatives of $1/x$ and sketch their
...
\end{enumerate}
\item On page~\pageref{tangent} of the text there is an antiderivative
...
negative.
  ...
  \begin{enumerate}
  \item How many separate intervals does the domain of $\tan{x}$ break
down into?

  \item For what values of $x$ is $\cos{x}$ equal to zero, and for what
values is it negative?

  \item Modify the antiderivative $-\ln(\cos{x})$ so that it {\em is\/}
defined when $\cos{x}$ is negative.  (How is this problem with the
logarithm function treated in the previous question?)


  \item In a typical table of integrals you will find the statement
	\[
\int \tan{x} \, dx =  - \ln{|\cos{x}|} + C.
\]
Explain why this does not cover all the possibilities.

  \item Give a more precise expression for $\dint \tan{x}\,dx$, modelled
on the way you answered part (f) of problem 18.  How many different
constants will you need?

  \item Find a function $G$ that is an antiderivative for $\tan{x}$ and
...
\end{enumerate}
\item In the table on page~\pageref{table} the antiderivative of $x^p$
is given as 
\[
\dfrac{1}{p+1} x^{p+1} +C.
\]
...
\item Find $F'(x)$ for the following functions.  In parts (a), (b), and (d) do the problems two ways: by finding an antiderivative, and by using the fundamental theorem to get the answer without evaluating an antiderivative.  Check that the answers agree.

  \begin{enumerate}
  \item $F(x) = \dint_0^x (t^2 + t^3) \, dt$.

  \item $F(x) = \dint_1^x \dfrac{1}{u} \, du$.

  \item $F(x) =
	\dint_1^x \dfrac{v}{1+v^3} \, dv$.

  \item $F(x) = \dint_0^{x^2} \cos{t} \, dt$.
...
\end{enumerate}
\item Consider the two functions
	\[
...
\]
  ...
  \begin{enumerate}
  \item Show that $F$ and $G$ both satisfy the initial value problem
	\[
y' = \dfrac{x}{\sqrt{1+x^2}},  \qquad y(0) = 0.
\]


  \item Since an initial value problem typically has a {\em unique\/}
...
\end{enumerate}
\item The connection between integration and differentiation that is
...
equations.
  ...
  \begin{enumerate}
  \item Use either a program (e.g., PLOT) or a differential equation
solver on a computer to get a graphical solution $A(x)$ to the initial
value problem above.

  \item Sketch the graph of $y = A(x)$ over the domain $0 \leq x \leq 4$.

  \item Your graph should increase from left to right.  How can you tell
this even before you see the computer output?

  \item Your graph should level off as $x$ increases.  Determine $A(5)$,
$A(10)$, $A(30)$.  (Approximations provided by the computer are
adequate here.)


  \item Estimate ${\displaystyle \lim_{x \to \infty} A(x)}$.  \hfill [The
  {\em exact\/} value is $\sqrt{\pi}/2$.]


  \item Determine $\dint_0^1 e^{-t^2} dt$ and $\dint_0^2 e^{-t^2} dt$.

  \item Determine $\dint_1^2 e^{-t^2} dt$.


\end{enumerate}
\item Find the area under the curve $y = x^3 + x$ for $x$ between 1
and~4.  (See chapter 6.3.)


\item Find the area under the curve $y = e^{3x}$ for $x$ between 0 and
$\ln{3}$.


\item The {\bf average value} of the function $f(x)$ on the interval $a
...
\pageref{average-begin}--\pageref{average-end}.)
  ...
  \begin{enumerate}
  \item Find the average value of each of the functions $y = x$, $x^2$,
$x^3$, and $x^4$ on the interval $0 \leq x \leq 1$.

  \item Explain, using the graphs of $y = x$ and $y = x^2$, why the
average value of $x^2$ is less than the average value of $x$ on the
interval $[0, 1]$.


\end{enumerate}
\item What are the maximum, minimum, and average values of the function
	\[
f(x) = x + 2 e^{-x}
\]
on the interval $[0, 3]$?
  ...
  \begin{enumerate}
  \item Sketch the graph of $y = f(x)$ on the interval $[0, 3]$.  Draw
the line $y = \mu$, where $\mu$ is the average value you found in part
(a).

  \item For which $x$ does the graph of $y = f(x)$ lie above the line $y
...
[[NOTE:The integral form  of the chain rule]]
...
[[NOTE:Reduction methods transform problems into equivalent, simpler, problems]] 
...
{\bf [[EX:1]]}\quad  Suppose we try to find a formula for the integral
	\[
...
[[NOTE:A compact notation for expressing the integral form of the chain rule]] 
...
{\bf [[EX:2]]}\quad  Can we use the method of substitution to find
	\[
...
{\bf [[EX:3]]}\quad The method of substitution is useful in simple
...
\label{subsec:substitution-in-definite-integrals}
...
{\bf [[EX:4]]}\quad Consider the definite integral
\[
...
\begin{enumerate}
\item Evaluate the following using substitution.  Do parts (a) through
...
remaining parts in the way you feel most confident.
  ...
  \begin{enumerate}
  \item ${\dint 2y(y^2+1)^{50}\,dy}$


  \item ${\dint \sin(5z)\,dz}$


  \item ${\dint \dfrac{e^{\sqrt x}}{\sqrt x}\,dx}$


  \item ${\dint (5t+7)^{50}\,dt}$


  \item ${\dint 3u^2 \, \root 3 \of {u^3+8}\,du}$


  \item ${\dint \dfrac{1}{2v+1}\,dv}$


  \item ${\dint \tan{x}\,dx}$


  \item ${\dint \tan^2(x)\sec^2(x)\,dx}$


  \item ${\dint \sec(x/2)\tan(x/2)\,dx}$

  \item ${\dint \sin(w)\,\sqrt{\cos(w)}\,dw}$


  \item ${\dint \dfrac{\sin(\sqrt{s})}{\sqrt{s}}\,ds}$


  \item ${\dint \sqrt{3-x}\,dx}$


  \item ${\dint \dfrac{dr}{r\ln{r}}}$


  \item ${\dint e^x\sin(1+e^x)\,dx}$


  \item ${\dint \dfrac{y}{1+y^2}\,dy}$


  \item ${\dint \dfrac{w}{\sqrt{1-w^2}}\, dw}$


  \item ${\dint \dfrac{1}{1+4y^2}\,dy}$


  \item ${\dint \dfrac{1}{\sqrt{1-9w^2}}\, dw}$
\mbox{}


\end{enumerate}
\item Use integration by substitution to find the numerical value of
...
RIEMANN\index{program!RIEMANN}.
  ...
  \begin{enumerate}
  \item ${\dint_0^1 \dfrac{e^s}{e^s+1}\,ds}$


  \item ${\displaystyle \int_0^{\ln{e}}
\dfrac{e^s}{e^s+1}\,ds}$


  \item ${\displaystyle \int_1^3 \dfrac{1}{2x+1}\,dx}$


  \item ${\displaystyle \int_{-3}^{-1} \dfrac{1}{2x+1}\,dx}$

  \item ${\displaystyle \int_0^1
  ...
  \item ${\displaystyle \int_0^2
	\dfrac{1}{1+ (x^2/4)}\,dx}$


  \item ${\displaystyle \int_0^{\tfrac{1}{3}}
	\dfrac{1}{\sqrt{1-9y^2}}\,dy}$
\mbox{}


\end{enumerate}
\item This question concerns the integral
$I = \dint \sin{x} \, \cos{x} \, dx$.

  \begin{enumerate}
  \item Find $I$ by using the substitution $u = \sin{x}$.

  \item Find $I$ by using the substitution $u = \cos{x}$.

  \item Compare your answers to (a) and (b).  Are they the same?  If not,
...
true here? If so, what is the constant?
  ...
  \item Now calculate the value of the {\em definite\/} integral
	\[
...
\end{enumerate}
\item Find all functions $y = F(x)$ that satisfy the differential equation
	\[
...
\]
  ...
  \begin{enumerate}
  \item From among the functions $F(x)$ you found in part (a), select the
...
\end{enumerate}
\item Find a function $y = G(t)$ that solves the initial value problem
	\[
\dfrac{dy}{dt} = t e^{-t^2}
		\qquad y(0) = 3.
\]
...
\item What is the average value of the function $f(x) =
x/\sqrt{1+x^2}$ on the interval $[0, 2]$?

  \begin{enumerate}
  \item Show that the average value of the $f(x)$ on the interval $[-2,
  2]$ is 0.  Sketch a graph of $y = f(x)$ on this interval, and
explain how the graph also shows that the average is 0.


\end{enumerate}
\item Sketch the graph of the function $y = x e^{-x^2}$
on the interval $[0, 5]$.

  \begin{enumerate}
  \item Find the area between the graph of $y = x e^{-x^2}$
...
$A(1000)$?
  ...
  \item It is possible to argue that the area between the graph of $y = x
...
\end{enumerate}
\item Use a computer graphing utility to establish that
	\[
...
Sketch these graphs.
  ...
  \begin{enumerate}
  \item Find a formula for $\dint \sin^2{x} \, dx$.  (Suggestion: replace
$\sin^2{x}$ by the expression involving $\cos(2x)$, above, and
integrate by substitution.)


  \item What is the average value of $\sin^2{x}$ on the interval $[0,
  \pi]$?  What is its average value on any interval of the form $[0, k
  \pi]$, where $k$ is a whole number?

  \item Explain your results in part (c) in terms of the graph of
$\sin^2{x}$ you drew in part (a).

  \item Here's a differential equations proof of the identity in part
...
[[NOTE:An integral form  of the product rule]]
...
{\bf [[EX:1]]}\quad  We will use the formula in the box to find
	\[
\int x \cdot \cos{x} \, dx.
\]
...
[[NOTE:Integrate only part  of the integrand]]
...
{\bf [[EX:2]]}\quad  Use integration by parts to find\label{lnder}
	\[
\int \ln(x) \, dx.
\]
...
set routine.
[[NOTE:The ingredients  of a successful  integration by parts]]	
...
[[EX:2]] this wasn't obvious.)  Next, you need to identify the parts
...
rather than towards one.  (See [[EX:3]] below for a cautionary
...
{\bf [[EX:3]]}\quad  Use integration by parts to find
	\[
\int t \cdot e^t \, dt.
\]
Set
...
integral is {\em not\/} simpler than the original.  [[NOTE:What went
...
\begin{enumerate}
\item Use integration by parts to find a formula for each of the
following integrals.
  ...
  \item ${\displaystyle \int te^{t}\,dt}$


  \item ${\displaystyle \int we^{-w}\,dw}$
  ...
  \item $\dint u^2 \cos{u} \, du$


  \item ${\displaystyle \int x\sec^2{x}\,dx}$


  \item ${\displaystyle \int e^{2x}(x + e^x)\,dx}$
\mbox{}
...
\end{enumerate}
\item Use integration by parts to obtain a formula for
	\[
\int (\ln{x})^2\,dx.
\]
...
\item Find $\dint x^2 e^x \, dx$.

  \begin{enumerate}
  \item Find $\dint x^3 e^x \, dx$.  (Reduce this to part (a)).

  \item Find $\dint x^4 e^x \, dx$.

  \item What is the general pattern here?  Find a formula for $\dint x^n e^x \, dx$, where $n$ is any positive integer.

  \item Find $\dint e^x \left(5x^2 - 3x + 7\right) \, dx$.


\end{enumerate}
\item Draw the graph of $y=\arctan{x}$ over the interval $0\leq x\leq
...
and show on your graph the area this corresponds to.
  ...
  \item If we add the results of part (b) and part (c), what do you get?
From the geometry of the picture, what should you have gotten?


\end{enumerate}
\item Repeat the analysis of the preceding problem by calculating the
value of
\[
...
\item Generalize the preceding two problems to the case where $f$ and
$g$ are any two functions that are inverses of each other whose graphs
pass through the origin.



\item What is the average value of the function $\ln{x}$ on the
interval $[1, e]$?

  \begin{enumerate}
  \item What is the average value of $\ln{x}$ on $[1, b]$?  Express this
in terms of $b$.  Discuss the following claim: The average value of
$\ln{x}$ on $[1, b]$ is approximately $\ln(b) - 1$ when $b$ is large.


\end{enumerate}
\item Sketch the graph of $f(x) = x e^{-x}$ on the interval $[0, 4]$.

  \begin{enumerate}
  \item What is the area between the graph of $y = f(x)$ and the $x$-axis
for $0 \leq x \leq 4$?
...
\end{enumerate}
\item Find three solutions $y = f(t)$ to the differential equation
	\[
\dfrac{dy}{dt} = 5 - 2 \ln{t}.
\]


\item Find the solution $y = \varphi(t)$ to the initial value problem
	\[
...
\]
  ...
  \begin{enumerate}
  \item The function $\varphi(t)$ increases as $t$ increases.  Show this
...
(What is true about the derivative of an increasing function?)
  ...
  \item Does the value of $\varphi(t)$ increase without bound as $t \to
\infty$?  If not, what value does $\varphi(t)$ approach?


\end{enumerate}
\item {\bf The differential form of Integration by Parts}\quad If $u$
...
\]
  ...
  \begin{enumerate}
  \item Solve a couple of the preceding problems using this notation.


\end{enumerate}
\end{enumerate}
\subsubsection{Sine and cosine integrals}\index{integral!sine and cosine}
\label{subsubsec:sine-and-cosine-integrals}
...
\begin{enumerate}
\item In the later exercises we shall make frequent use of the
...
the other two identities.
  ...
  \begin{enumerate}
  \item We can give a different argument for the identities above using
...
Hence conclude the two functions must be the same.
  ...
  \item Find an initial value problem that is satisfied by both
$f(x)=\cos^2{\alpha x} - \sin^2{\alpha x}$ and by $g(x) = \cos{2\alpha
  x}$.


\end{enumerate}
\item {\bf Evaluating } $\displaystyle \int \sin^2{x} \, dx$
...
\]
  ...
  \item Using the identity $\sin^2{\alpha x} + \cos^2{\alpha x} = 1$,
...
\]
  ...
  \item Combining (a) and (b) algebraically, show that
	\[
2 \int \sin^2{x} \, dx = - \sin{x} \cos{x} + x + C.
\]


  \item Using algebra and a trigonometric identity, conclude that
	\[
\int \sin^2{x} \, dx = 
...
\end{enumerate}
\item Modify the argument of the preceding exercise to show		%qqq
	\[
...
\item {\bf Evaluating }$\displaystyle \int \sin^2{\alpha x} \, dx$
...
carrying out the following steps.
  ...
  \begin{enumerate}
  \item From the trigonometric identities\index{trigonometric identities}
...
\]
  ...
  \item Using the formula in (a), conclude that
	\[
\int \sin^2{\alpha x} \, dx = 
	\dfrac{x}{2} - \dfrac{1}{4\alpha}\sin{2\alpha x} + C.
\]
...
\end{enumerate}
\item {\bf Evaluating }$\displaystyle \int \cos^2{\alpha x} \, dx$
...
\item {\bf Evaluating }$\displaystyle
		\int \sin{\alpha x} \cos{\alpha x} \, dx$  


  \begin{enumerate}
  \item Using the identity $\sin{2\alpha x} = 2 \sin{\alpha x}
...
\]
  ...
  \item Using integration by substitution, obtain the alternative formula
	\[
\int \sin{\alpha x} \cos{\alpha x} \, dx = 
...
\]
  ...
  \item Show that your results in (a) and (b) are compatible.  (For
...
\]
  ...
  \item Using integration by parts again, show that the new integral in
...
\]
  ...
  \item Let $J = \dint \sin{\alpha x} \sin{\beta x}\, dx$; show that
...
\end{enumerate}
\item Imitate the methods of the preceding exercise to deduce		%qqq
	\[
\int \cos{\alpha x} \cos{\beta x}\, dx =
	\dfrac{1}{\beta^2 - \alpha^2} 
...
\item Determine the following.
  ...
  \begin{enumerate}
  \item ${\displaystyle \int_0^{2\pi} \sin^2{x}\, dx}$.

  \item ${\displaystyle \int_0^{n\pi} \cos^2{x}\, dx}$, $n$ a positive integer.

  \item ${\displaystyle \int_0^{2\pi} \sin{x}
\sin{\beta x}\, dx}$, $\beta \neq 1$.

  \item ${\displaystyle \int_0^{\pi} \sin{x}
\sin{\beta x}\, dx}$, $\beta \neq 1$.
\mbox{}
...
\label{sec:separation-of-variables-and-partial-fractions}
...
\label{subsec:the-differential-equation-y-y}

As you know, 
[[NOTE:A new method for solving $y' = y$]]	
...
[[NOTE:Find the inverse function instead]]
...
[[NOTE:The new differential equation\ldots]]
	\[
...
\end{align*}
Thus $y(t) = Ce^t$, 
[[NOTE:\ldots solves the  original problem]]
where we have replaced the constant $e^{-k}$ by $C$.


\subsubsection{Indefinite integrals}
\label{subsubsec:indefinite-integrals}
\index{indefinite integral}

The language of indefinite integrals 
[[NOTE:The differential equation expressed using differentials]]
...
[[NOTE:The variables  are now \emph{separated}]]
...
[[NOTE:To integrate,  separate the variables]]
...
\label{subsec:separation-of-variables}
...
[[NOTE:Bring in the  initial condition]]
...
\label{subsec:partial-fractions}
...
\label{subsubsec:logistic-growth}
...
[[NOTE:Step 1: separate  the variables]]	
...
\]
We are stuck now, 
[[NOTE:The denominator has an unfamiliar form]]
...
[[NOTE:Step 2: write the integrand as a sum  of simple fractions]]
...
[[NOTE:Determining   $\alpha$ and $\beta$]]
...
\]
Thus $\alpha = \beta = 1$, 
[[NOTE:The partial fractions decomposition]]
and we have 
	\[
...
[[NOTE:Step 3: evaluate  the integrals]]
...
Lastly, 
[[NOTE:Step 5: incorporate  the initial condition]]
...
The method of partial fractions\index{partial
...
[[NOTE:The final form  of the antiderivative  of the cosecant]]
...
[[NOTE:The antiderivative  of the secant]]
...
\label{subsubsec:separation-of-variables}
\index{separation of variables}
...
\begin{enumerate}
\item Use the method of separation of variables to find a formula for
...
\item Use the method of separation of variables to find formulas for
...
\end{enumerate}
\item {\bf A cooling liquid}.  According to Newton's law of
...
The constant $k$ gives the rate at which the object cools.
  ...
  \begin{enumerate}
  \item Find a formula for the solution to this equation using the method
of separation of variables.  Your formula should contain an arbitrary
constant.

  \item Suppose $A$ is 20$^\circ$C and $k$ is .1$^\circ$ per minute per
$^\circ$C.  If time $t$ is measured in minutes, and $Q(0) =
90^\circ$C, what will $Q$ be after 20 minutes?

  \item How long does it take for the temperature to drop to 30$^\circ$C?


\end{enumerate}
\item Suppose a cold drink at 36$^\circ$F is sitting in the open air
...
will happen to the temperature of the drink over time.
  ...
  \begin{enumerate}
  \item Obtain a formula for the temperature of the drink as a function
of the number of minutes $t$ that have passed since its temperature
was 36$^\circ$F.

  \item What will the temperature of the drink be after 5 minutes; after
10 minutes?

  \item How long will it take for the drink to reach 55$^\circ$F?


\end{enumerate}
\item {\bf A leaking tank}.\index{leaking tank} In chapter~4.2 we used
...
(see page~\pageref{leaking tank-begin}).
  ...
  \begin{enumerate}
  \item Use the method of separation of variables to show that
	\[
...
\end{enumerate}
\item {\bf A falling body with air resistance}.\index{falling body} We
...
\] to model the motion of a body falling under the influence
  ...
  \begin{enumerate}
  \item Solve the differential equation by separating variables, and obtain
	\[
v(t) = \dfrac{1}{b} \, \left( Ce^{-bt} - g \right),
\]
where $C$ is an arbitrary constant.

  \item Now impose the initial condition $v(0) = 0$ (so the body starts
it fall from rest) to determine the value of $C$.  What is the formula
for $v(t)$ now?

  \item Exercise 21 on page~\pageref{ex:falling body1} gives the solution
...
this exercise.
  ...
  \item The distance $x(t)$ that the body has fallen by time $t$ is given
by the integral
	\[
...
\end{enumerate}
\item {\bf Supergrowth}.\index{supergrowth} We have analyzed the
...
arbitrary constant $C$.
  ...
  \begin{enumerate}
  \item Add the initial condition $Q(0) = A$.  This will fix the value of
the constant~$C$.  What is the formula for $Q(t)$ when the initial
condition is incorporated?

  \item Your formula in part (b) should demonstrate that $Q$ becomes
...
initial population size~$A$.
  ...
  \item Suppose the values of $k$ and $A$ are known only imprecisely, and
...
\end{enumerate}
\item {\bf General supergrowth}.  Find the solution to the initial
value problem
	\[
...
\label{subsubsec:partial-fractions}
\index{partial fractions}


\begin{enumerate}
\item Use the method of partial fractions to determine the values of
$\alpha$, $\beta$, and $\gamma$ in the following equations.



  \begin{enumerate}
  \item $\dfrac{1}{(x - 1)(x + 2)} = \dfrac{\alpha}{x - 1} +
	\dfrac{\beta}{x + 2}$
	

  \item $\dfrac{x}{(x - 1)(x + 2)} = \dfrac{\alpha}{x - 1} +
	\dfrac{\beta}{x + 2}$


  \item $\dfrac{1}{x(x^2-1)} = \dfrac{\alpha}{x} +
	\dfrac{\beta}{x-1} + \dfrac{\gamma}{x+1}$

  \item $\dfrac{x}{2x^2 +3x + 1} = \dfrac{\alpha}{2x + 1} +
	\dfrac{\beta}{x + 1}$
...
\end{enumerate}
\item Find a formula for each of these indefinite integrals.




  \begin{enumerate}
  \item $\dint \dfrac{3 \, dx}{(x - 1)(x + 2)}$


  \item $\dint \dfrac{5x + 3}{(x - 1)(x + 2)} \, dx$


  \item $\dint \dfrac{dt}{t(t^2-1)}$

  \item $\dint \dfrac{x \, dx}{1 - x^2}$


  \item $\dint \dfrac{1 - u}{u^2 - 4} \, du$


  \item $\dint \dfrac{x^2 + 2x + 1}{x(x^2 + 1)} \, dx$
\mbox{}
...
\item Determine
  ...
  \begin{enumerate}
  \item $\dint_2^3 \dfrac{3 \, dx}{(x - 1)(x + 2)}$


  \item $\dint_2^4 \dfrac{dt}{t(t^2-1)}$

  \item $\dint_0^{\pi/4} \dfrac{x \, dx}{1 - x^2}$


  \item $\dint_1^{\sqrt{3}} \dfrac{x^2 + 2x + 1}{x(x^2 + 1)} \, dx$
\mbox{}


\end{enumerate}
\item Mirror the derivation of $\dint \csc{x}\,dx$ to find $\dint
\sec{x}\,dx.$


\item Consider the particular logistic growth model defined by
	\[
...
\] (Compare this with the fermentation problems, pages
  ...
  \begin{enumerate}
  \item Obtain the formula for the solution to this initial value problem.

  \item How large will $P$ be after 3 hours; after 10 hours?

  \item When will $P$ reach one-half the carrying capacity--that is, for
which $t$ is $P = 5$ lbs?


\end{enumerate}
\item Derive the formula for $\dint \sec{x}\,dx$ given on
...
\label{subsubsec:review-of-trigonometric-identities}
...
\label{subsec:inverse-substitution}
...
[[NOTE:Success sometimes comes by making things more complicated]] 
...
{\bf [[EX:1]]}\quad  Suppose we want to evaluate
\[
\int \sqrt{4-x^2}\,dx.
\]
...
[[NOTE:Some useful substitutions]]
...
\label{subsec:inverse-substitution-and-definite-integrals}
...
$\displaystyle \dint_a^b f(x)\,dx = F(b)-F(a)=\dint_A^B f(G(u))\,g(u)\,du,$
...
{\bf [[EX:4]]}\quad Evaluate
\[
\int_{-2}^2 \sqrt{4-x^2}\,dx.
\]
In [[EX:1]] we found an antiderivative for this function by making
...
{\bf [[EX:5]]}\quad Suppose we wanted the integral
\[
\int_0^3 \dfrac{dx}{\sqrt{x^2+9}}.
\]
In [[EX:2]] we found an antiderivative by letting $x=3\tan{u}$.  Here
...
{\bf [[EX:6]]}\quad Consider the integral
\[
...
{\bf [[EX:7]]}\quad The technique of completing the square even works
...
\label{subsec:trigonometric-polynomials}
...
\label{subsubsec:category-i-either-boldmath-mgeq-0-or-ngeq-0-or-both}
...
\subsubsection{Remark:}
The instance $m=-1$ in this 
[[NOTE:The antiderivative  of the cotangent]]\index{cotangent!antiderivative of}
...
\subsubsection{Remark:}
Notice that  $n=-1$ gives us 
[[NOTE:The antiderivative  of the tangent]]\index{tangent!antiderivative of}
...
{\bf [[EX:13]]}\quad If we let $n=2$, we quickly get the
...
[[NOTE:In its current form,  the reduction formula works best for $n>0$]]
...
[[NOTE:The reduction formula for negative exponents]]
...
{\bf [[EX:14]]} \quad We can check this formula by trying $k=2$, which will give us the antiderivative of $\csc^2{x}$:
\begin{align*}
\dint \csc^2{x}
  &= \dint \sin^{-2}{x}\,dx \\
...
\label{subsubsec:category-ii-both-boldmath-m0-and-n0}
...
{\bf [[EX:16]]} \quad($r$ and $s$ both even already)
\begin{align*}
\sin^{-4}{x}\,\cos^{-6}{x}
...
{\bf [[EX:17]]} \quad($r$ even and $s$ odd)\label{ex17}
\begin{align*}
\sin^{-4}{x}\,\cos^{-5}{x}
...
{\bf [[EX:18]]} \quad(both $r$ and $s$ odd)
\begin{align*}
\sin^{-3}{x}\,\cos^{-5}{x}
...
\begin{enumerate}
\item Find the following antiderivatives ($a$ is a positive constant):



  \begin{enumerate}
  \item $\dint \dfrac{dx}{\sqrt{1-4x^2}}$


  \item $\dint \dfrac{dx}{\sqrt{1-4x^2}}$


  \item $\dint \dfrac{dx}{\sqrt{4+x^2}}$


  \item $\dint \dfrac{x\,dx}{\sqrt{4+x^2}}$

  \item $\dint \dfrac{dx}{(a^2-x^2)^{3/2}}$


  \item $\dint \dfrac{dx}{4+x^2}$


  \item $\dint \dfrac{x\,dx}{4+x^2}$

  \item $\dint \dfrac{dx}{x\sqrt{4+x^2}}$


  \item $\dint \dfrac{x\,dx}{\sqrt{x^2-a^2}}$


  \item $\dint \dfrac{dx}{(a^2+x^2)^2}$
\mbox{}
...
\item Evaluate the following integrals:
  ...
  \begin{enumerate}
  \item $\dint_1^{-1} \dfrac{dx}{4-x^2}$


  \item $\dint_1^2 \sqrt{x^2-1}\,dx$


  \item $\dint_0^{\pi/3} x\,\sec^2{x}\,dx$

  \item $\dint_0^1 \dfrac{dx}{(2-x^2)^{3/2}}$


  \item $\dint_0^\infty \dfrac{dx}{9+x^2}$


  \item $\dint_a^{2a} x^3\sqrt{x^2-a^2}\,dx$
\mbox{}


\end{enumerate}
\item Sketch the ellipse $\dfrac{x^2}{a^2}+ \dfrac{y^2}{b^2} = 1$,
...
\item Find the following antiderivatives:
  ...
  \begin{enumerate}
  \item $\dint \dfrac{dx}{\sqrt{x^2-2x-8}}$


  \item $\dint \dfrac{dx}{x^2+6x+10}$


  \item $\dint \dfrac{dx}{\sqrt{x^2+6x+8}}$


  \item $\dint \dfrac{x\,dx}{x^2+4x+5}$

  \item $\dint \dfrac{x\,dx}{\sqrt{5+4x-x^2}}$


  \item $\dint \dfrac{(2x+7)\,dx}{4x^2+4x+5}$


  \item $\dint \dfrac{(4x-3)\,dx}{\sqrt{-x^2-2x}}$


  \item $\dint \dfrac{dx}{(a^2-x^2-2x)^2}$
\mbox{}


\end{enumerate}
\item If $x= a\,\sec{u}$, where $a$ is a constant, draw a right
triangle containing an angle $u$ with lengths of sides specified to
reflect this relation between $x$, $a$, and $u$.

  \begin{enumerate}
  \item In terms of $x$ and~$a$, what is $\sin{u}$?  What is $\cos{u}$?
What is $\tan{u}$?
...
\item Evaluate the following:
  ...
  \begin{enumerate}
  \item $\dint \dfrac{dx}{\sin{x}\,\cos{x}}$


  \item $\dint \cos^3{x}\,\sin^{-4}{x}\,dx$


  \item $\dint \csc^4{x}\,\cot^2{x}\,dx$


  \item $\dint \sin{3x}\,\cot{3x}\,dx$


  \item $\dint_0^{\pi/2} \sin^n{x}\,\cos^3{x}\,dx$

  \item $\dint \tan^5{x}\,dx$


  \item $\dint \dfrac{\sin^3{5x}\,dx}{\sqrt[3]{\cos{5x}}}$


  \item $\dint \dfrac{\cos^3(\ln{x})\,dx}{x}$


  \item $\dint \sec^4{x}\,\ln(\tan{x})\,dx$


  \item $\dint_0^{a/2} \dfrac{dx}{(a^2-x^2)^{3/2}}$
\mbox{}


\end{enumerate}
\item Use the analysis of [[EX:17]] (page~\pageref{ex17}) to find an
antiderivative for $\sin^{-4}{x}\,\cos^{-5}{x}$.

\end{enumerate}
\subsubsection{Reduction formulas}
\label{subsubsec:reduction-formulas}



\begin{enumerate}
\item Derive the reduction formulas for the cosine function given on
page~\pageref{cosred}.


\item By writing $\tan^n{x}=\tan^{n-2}{x}(\sec^2{x}-1)$, get a
reduction formula which expresses $\dint \tan^n{x}\,dx$ in terms of
$\dint \tan^{n-2}{x}\,dx$.

  \begin{enumerate}
  \item Use this evaluation formula to find $\dint \tan^6{x}\,dx.$
...
\]
  ...
  \item Give a clear argument why \quad $\displaystyle
...
\end{enumerate}
\item By writing $\sec^n{x}$ as $\sec^{n-2}{x}\,\sec^2{x}$ and using
integration by parts, get a reduction formula which expresses $\dint
\sec^n{x}\,dx$ in terms of $\dint \sec^{n-2}{x}\,dx$.

  \begin{enumerate}
  \item Since $\sec{x}=\cos^{-1}{x}$, the formula you got in part (a)
...
\end{enumerate}
\item Find a reduction formula that expresses
\[
...
\]
  ...
  \begin{enumerate}
  \item Using the results of part (a), show that
\[
...
\]
  ...
  \item Explain why, for a fixed value of $t$,
\[
...
\end{enumerate}
\item Find a reduction formula expressing
\[
...
trigonometric substitution.\label{1plusx2}
  ...
  \begin{enumerate}
  \item What is the exact value of $\dint_0^1 \dfrac{dx}{(1+x^2)^5}$?


\end{enumerate}
\item Our approach to integrating trigonometric polynomials was to
...
[[NOTE:A return to  numerical methods]]	
...
[[NOTE:Efficient numerical integration]]	
...
\label{subsec:the-trapezoid-rule}
...
[[NOTE:Replace rectangles  by trapezoids]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/trap1.pdf}

Let's figure out the areas of these trapezoids.  
[[NOTE:Each trapezoid is sandwiched between two rectangles\ldots]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/trap2.pdf} 




It is sandwiched between  two rectangles, one taller and one shorter.  
[[NOTE:\ldots whose average area equals the area of the trapezoid]]
...
[[NOTE:The trapezoidal approximation is the average of left and right Riemann sums]] 
...
[[NOTE:Comparing approximations]] However, we will use equal subintervals
...
[[NOTE:A surprise:  the midpoint approximation is even better than the trapezoidal]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/trap3.pdf}



The second step is to compare 
[[NOTE:Shading depicts  the errors]]	
...
[[NOTE:Combine good approximations\ldots]]
...
[[NOTE:\ldots so that most of the error cancels]]
...
[[NOTE:Using Riemann sums to carry out  Simpson's rule]]	
...
[[NOTE:The accuracy of Simpson's rule]]	
...
[[NOTE:An error bound for Simpson's rule]]\index{error bound}
...
\begin{enumerate}
\item Evaluate the expression above (including the factor of 4) use
...
\item Over the interval $0 \leq x \leq 1$ it is true that
\[
...
question.
  ...
  \begin{enumerate}
  \item Show that if $n = 128 = 2^7$ then the error bound for Simpson's
rule does {\em not\/} guarantee that you can find the first 10
decimals of $\pi$ by the same method.

  \item Run Simpson's rule with $n = 2^7$ to estimate $\pi$.  How many
...
\end{enumerate}
\item In chapter 6.3 (page~\pageref{int error}) a left Riemann sum for
\[
\int_0^1 e^{-x^2} \, dx
\] 
...
until three digits stabilize.
  ...
  \begin{enumerate}
  \item If you use Simpson's rule with $n = 1000$ to estimate this
integral, how many digits stabilize?


\end{enumerate}
\item On page~\pageref{midpoint for x-cube} a midpoint Riemann sum with
$n = 10000$ shows that
\[
...
\label{subsec:the-lifetime-of-light-bulbs}

Ordinary light bulbs are supposed to burn about 700 hours, 
[[NOTE:The lifetime of a light bulb is unpredictable]]	
...
[[NOTE:Lifetime is the integral of burnout rate\ldots]]
...
[[NOTE:\ldots but there is no upper limit to the lifetime]]
...
[[NOTE:An integral is {\em improper\/} if the domain of integration is
  infinite]]
...
[[NOTE:The normal probability distribution involves  an improper integral]]
...
\label{subsec:evaluating-improper-integrals}

An integral with an infinite domain of integration is 
[[NOTE:An integral is also improper if its integrand becomes infinite]]	
...
[[NOTE:Antiderivatives help find improper integrals]]	
...
\subsubsection{[[EX:2]].}
To evaluate $\dint_0^1 \ln{x} \, dx$, we use the
indefinite integral
	\[
...
{\bf [[EX:3]]}. $\displaystyle \left. \dint_0^1 \dfrac{dx}{x} 
= \lim_{a \to 0} \int_a^1 \dfrac{dx}{x}
...
\begin{enumerate}
\item Find the value of each of the following improper
integrals.  (The value may be $\infty$.)



  \begin{enumerate}
  \item $\dint_{-\infty}^0 e^x \, dx$


  \item $\dint_1^{\infty} \dfrac{du}{u}$



  \item $\dint_0^1 \dfrac{dy}{y^2}$


  \item $\dint_0^{\pi/2} \tan{x} \, dx$
	

  \item $\dint_0^{\infty} x e^{-x} \, dx$
	

  \item $\dint_1^{\infty} \dfrac{du}{u^2}$
	

  \item $\dint_0^{\infty} \dfrac{x}{1 + x^2} \, dx$
	 

  \item $\dint_1^3 \dfrac{x}{x^2 - 1}\, dx$
\mbox{}


\end{enumerate}
\item Use the reduction formula for $\dint \dfrac{dx}{(1+x^2)^n}$ you
...
\label{subsubsec:the-normal-density-function}
...
\begin{enumerate}
\item First, use RIEMANN\index{program!RIEMANN} to estimate
the value of
	\[
...
\item To construct a second estimate, begin by sketching the graph of
...
\]
  ...
  \begin{enumerate}
  \item Now consider the accumulation function
\[
...
\]
  ...
  \item Does your results in part (b) and question 2 agree?  Do they
...
\label{subsubsec:the-gamma-function}
...
\item Using the property $\Gamma(x + 1) = x \cdot \Gamma(x)$, calculate
...
\item Estimate the value of $\Gamma(1/2)$.  (Exercises 2 and 3 above
offer two ways to estimate the value of an improper integral.)


\item In fact, $\Gamma(1/2) = \sqrt{\pi}$ exactly.  You can show this
...
\]
  ...
  \begin{enumerate}
  \item Make the substitution $u = (2t)^{\tfrac{1}{2}}$ and show that the
integral becomes
	\[
\Gamma(1/2) = 
...
(Check this.)  Now, using some algebra, show $\Gamma(1/2) = \sqrt{\pi}$.
  ...
  \item Compare your estimate for $\Gamma(1/2)$ from exercise 7 with the
exact value $\sqrt{\pi}$.


\end{enumerate}
\item Determine the exact values of $\Gamma(3/2)$ and $\Gamma(5/2)$.

  \begin{enumerate}
  \item In exercise 6 you gave a meaning to the expression
