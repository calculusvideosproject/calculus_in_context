...
[[NOTE:Factorials  in probability]]	
...
[[NOTE:$n!$ is difficult  to calculate]]
...
\label{subsec:stage-one-deriving-the-general-form}
...
[[NOTE:These estimates  are often adequate]]	
...
\label{subsec:stage-two-evaluating-c}
...
\label{subsubsec:deriving-walliss-formula}
...
\label{subsubsec:further-refinements}

Using even more careful methods of analysis, 
[[NOTE:Some refinements]]
...
\label{subsec:the-binomial-distribution}
...
{\bf [[EX:1]]}\quad How likely is it to get four 2's if we roll
...
{\bf [[EX:2]]}\quad What is the probability of getting exactly 47
...
\begin{enumerate}
\item Go through the derivation in this section and find several
...
\item Confirm the values given in the table on page~\pageref{table2}
for the approximations of $100!$ and $1000!$ that Stirling's formula
produces.


\item {\bf Rate of growth of $n!$}\quad Factorials get very large very
...
\]
  ...
  \begin{enumerate}
  \item Show that $a_k = \dfrac{N}{k}a_{k-1}$, and conclude that
\begin{center}
...
\]
  ...
  \item If $k>2N$ show that $a_k<.5a_{k-1}$.  Hence conclude that
${\displaystyle \lim_{n\rightarrow\infty}a_n=0}$.

  \item Use Stirling's approximation to show that
\[
...
more rapidly than the series $\{n!\}$.
  ...
  \item Show that $a_n<1$ if $n>eN$.  This gives an upper bound on how
long it takes the factorials to catch up with the exponentials.


\end{enumerate}
\item If $n\geq 5$, then $n!$ terminates in a certain number of zeroes.
...
\label{subsubsec:the-binomial-distribution}




\begin{enumerate}
\item The formula for the binomial distribution gives us that the
...
\item More generally, if we try a certain experiment $n$ times with a
...
\item {\bf One-dimensional random walk}\quad An important class of
...
question.
  ...
  \begin{enumerate}
  \item Explain why the particle will be $r$ units to the right of the
...
could never be 3 units or 7 units to the right after 100 steps.
  ...
  \item Using the same symbols as in part (a), show that the probability
...
\]
  ...
  \item Use Stirling's formula to show that this probability of being $r$
...
\]
  ...
  \item To simplify the denominator of this fraction, recall the Taylor
...
\]
  ...
  \item Explain how we can get the answer to exercise~5 as a special case
of the result just obtained in part (d).

  \item Using the approximation from part (d), calculate the probability
...
\]
and 
[[NOTE:The relation between the half-life and  the decay constant]]
therefore
\[
T=\dfrac{\ln{2}}{k}.
\]
...
\label{subsubsec:the-definition-of-probability}
\index{probability}
...
\label{subsubsec:some-properties-of-probabilities}
...
[[NOTE:The addition rule  for probabilities]]
\begin{quote}  
...
[[NOTE:The product rule  for probabilities]]
\begin{quote}
...
[[NOTE:The probability  that something  doesn't happen]]
\begin{quote}
...
\label{subsubsec:the-notion-of-a-probability-model}
...
\label{subsec:the-poisson-probability-distribution}


\subsubsection{The Poisson model of $\alpha$-ray emission}
\label{subsubsec:the-poisson-model-of-alpha-ray-emission}
\index{Poisson model}
...
\[p_n = \lim_{N\to\infty} \dfrac{N_n}{N}\ ,\]
it follows 
[[NOTE:The general formula for the expected value in a probability model]]
...
\label{subsubsec:derivation-of-the-poisson-model}
...
$k<100$---then $(M-k)/M$ 
[[NOTE:$(M-k)/M$ is essentially equal to 1]]
...
\label{subsubsec:other-applications-of-the-poisson-distribution}
...
\label{subsubsec:probability-models}
...
\begin{enumerate}
\item A fair coin is tossed.  If it comes up $H$ (heads), a fair die is
...
\item Two identical fair coins are put in cup, shaken, and spilled out
onto a table.  Construct a probability model for this experiment.



\item In the disintegration of large numbers of particles of radium
...
What is a model for the disintegration of a single particle of $Ra$?
  ...
  \begin{enumerate}
  \item Construct a probability model for the disintegration of two
particles of~$Ra$.
...
\label{subsubsec:the-poisson-distribution}




\begin{enumerate}
\item The purpose of this exercise is to present another way to show
...
\end{align*}
  ...
  \begin{enumerate}
  \item This pattern generalizes: show that
\[np_n = \lambda p_{n-1}\quad\hbox{for all $n > 0$}\ .\]


  \item Use part (a) to compute the expectation $E$ (you will need to
...
\end{enumerate}
\item A model is to be constructed for the number of rain drops that
...
\item In analyzing flying-bomb hits in the south of London during World
...
\item A meteorite shower sprinkles a large area of the earth's surface
...
\item The central processing unit (CPU) of a laptop computer will
...
\label{sec:the-power-spectrum}


This section is an 
[[NOTE:The problem of  signal + noise]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/lynx2.pdf}
...
it did.  [[NOTE:The power spectrum filters noise to detect periodic
...
[[NOTE:The noise level becomes stronger]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/noise3.pdf}




If we increase the original noise level 
[[NOTE:The noise level becomes overwhelming]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/noise4.pdf}
...
\label{subsec:detecting-the-frequency-of-a-signal}

Assume we have a signal that may be distorted by a lot of noise.  
[[NOTE:Compare the signal  to a probe whose frequency can be varied]] 	
...
[[NOTE:When the signal matches the  test frequency]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sintest1.pdf}  	  



On the right we see what happens 
[[NOTE:When the signal {\em doesn't\/} match the  test frequency]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sintest2.pdf}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sintest3.pdf}
...
[[NOTE:The average value  of a function]]
$a \leq x \leq b$ is
\[
...
[[NOTE:Integrals  with parameters  define functions]]
...
[[NOTE:$D(\omega)$ peaks when  $\omega$ is the frequency  of the signal]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect1.pdf}
...
[[NOTE:The peak in $D(\omega)$  is sharper if the signal  is
  tested over a  longer time interval]] 
$0 \leq t \leq 100$ seconds.

\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect2.pdf}
...
Of course, it is rare to find a formula for
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect3.pdf}
...
[[NOTE:A periodic   signal \ldots   
  \ldots and its  frequency detector]]
Can you guess a formula for $g(t)$?

\noindent \includegraphics[width=\linewidth]{../ps/calc2/period3.pdf}

\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect4.pdf}
...
\label{subsec:the-problem-of-phase}
...
[[NOTE:Arbitrary phase shifts]]\index{phase!shift}	
...
shift $\varphi$.  In particular, if [[NOTE:The average value varies with
...
[[NOTE:The problem  of phase \ldots]]	
...
[[NOTE:The graphs of  $D_s$ and $D_c$]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect5.pdf}
...
[[NOTE:The physical analogue of a detector  is a resonator]]
...
[[NOTE:Resonance occurs all around us.  Sometimes it is a
...
\label{subsubsec:detector-as-transform}
...
[[NOTE:Integrals transform  $S$ into $D_s$ and $D_c$]]
...
[[NOTE:The French mathematician Jean Baptiste Fourier (1768--1830)
...
\label{subsec:the-power-spectrum}

The sine and cosine detectors 
[[NOTE:A detector that ignores phase differences]]\index{phase}	
...
[[NOTE:\mbox{Power spectrum of} $3\sin(7t - \pi/3)$]]

\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect6.pdf}
...
[[NOTE:Two signals whose components differ  only in phase]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/period4.pdf}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect7.pdf}
...
[[NOTE:Detecting a periodic wave in a noisy signal]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect8.pdf}
...
[[NOTE:In the power spectrum, noise and signal are separated]]	
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect9.pdf}

\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect10.pdf}



\mbox{}

\noindent \includegraphics[width=\linewidth]{../ps/calc2/detect11.pdf}
...
\label{subsubsec:the-problem-of-phase}




\begin{enumerate}
\item Use the ``sum of two angles formula,''
	\[
...
\item Express $\sin(5t - \pi/3)$ as a sum of a pure sine function and
a pure cosine function.

  \begin{enumerate}
  \item Express $\frac{\sqrt{3}}{2}\sin(7t) + \frac{1}{2}\cos(7t)$ in the
form $A\sin(bt - \varphi)$.  To check your result, graph it together
with the given function using a computer graphing utility.

  \item Express $f(t) = \sin(t) + 2\cos(t)$ in the form $A\sin(bt -
...
\item The functions $\sin(t) + 2 \cos(t)$ and $2\sin(t) + \cos(t)$ have
...
\item Choose values for $A$, $b$, and $\varphi$ so that the function
	\[
...
\item Choose values of $A$ and $\varphi$ so that the function
	\[
...
\label{subsubsec:the-programs-detector-and-power}
...
graph on page~\pageref{testfunction}.
  ...
  \begin{enumerate}
  \item Graph the power spectrum of $h(t)$ over the frequency domain $0
...
\end{enumerate}
\item (Continuation of the previous exercise.)  Use SDETECTOR to
...
\item (Continuation.)  Use CDETECTOR to analyze $h(t)$.  Follow the
guidelines of the previous question.


\end{enumerate}
\subsubsection{A Grain of Salt}
\label{subsubsec:a-grain-of-salt}
...
\begin{enumerate}
\item Use the program POWER to graph the power spectrum of the function
...
\item Now increase the domain of integration to $0 \leq x \leq 30$, and
set 
\begin{verbatim}
     numberofsteps = 300
\end{verbatim}
...
\item Leave $0 \leq x \leq 30$, but restore {\tt numberofsteps = 100}.
...
\item Let {\tt numberofsteps = 50}, and calculate the power spectrum
...
\item Use SDETECTOR and CDETECTOR to analyze $\sin(2\pi x)$.  Take $0
...
\item According to the previous exercise, the peak in the power
...
\item Make a sketch of the graph of the function
	\[
...
\item Determine the value of $h(x)$ at $x = 0$, .3, .6, .9, 1.2, and
...
\item Compare the values of the detector
	\[
\dfrac{2}{30}\int_0^{30} \sin(2\pi x) 
	\sin\left(2\pi \tfrac{7}{3} x\right)\, dx,
\]
...
[[NOTE:The {\em true\/} spectrum  is the limit of the computed graphs  of the spectrum]]	
...
[[NOTE:Difficulties with polynomial approximations]]
...
[[NOTE:Approximating  periodic functions]]
...
form 
[[NOTE:Trigonometric polynomial of degree $n$ and period $T$]]
\begin{align*}
...
[[NOTE:Coefficients are independent of $n$]] 
...
\framebox[5in]
{\parbox{4.5in}{
 For a function $f$ with period $2 \pi$, its least
...
{\bf [[EX:1]]}\quad\label{triangle function} Let's find the
...
\]
Then the graph of $f$
[[NOTE:The graph of $f$  is ``triangular"]]
simply consists of two line segments:

\noindent \includegraphics[width=\linewidth]{../ps/calc2/sawtoo1.pdf}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sawtoo2.pdf}
...
[[NOTE:Approximating a triangular wave-form]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sawtoo3.pdf}
...
[[NOTE:The general rule  for calculating  Fourier series]] 
...
\framebox[5in]
{\parbox{4.6in}{
 For a function $f(t)$ with period $T$, its least squares
...
{\bf [[EX:2]]}\quad\label{May Fourier} Consider the 
[[NOTE:Fourier series for the periodic solutions of May's predator-prey model]] 
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/BMay.pdf}




\subsubsection{Derivation of the formula for the Fourier coefficients}
\label{subsubsec:derivation-of-the-formula-for-the-fourier-coefficients}
...
[[NOTE:Integrating products  of sines and cosines]]
...
\begin{enumerate}
\item Use the formulas on page~\pageref{`trig'} in chapter~11.3 to
derive the following equalities; $k$ and $m$ are integers.
  ...
  \begin{enumerate}
  \item $\dint_0^T \sin\dfrac{2k\pi x}{T} \cos\dfrac{2m\pi x}{T} \, dx
  =  0$ for all $k$ and $m$.
  ...
  \item $\dint_0^T \sin\dfrac{2k\pi x}{T} \sin\dfrac{2m\pi x}{T} \, dx
  = \begin{cases}
	T/2 & \text{if $k=m$}, \\
	0   & \text{otherwise}.
     \end{cases}$
  ...
  \item $\dint_0^T \cos\dfrac{2k\pi x}{T} \cos\dfrac{2m\pi x}{T} \, dx
  = \begin{cases}
	T/2  & \text{if $k=m \ne 0$}, \\
	T & \text{if $k=m=0$}, \\
	0    & \text{otherwise}.
...
\end{enumerate}
\item Show that in the Fourier series for the triangular function
discussed in the text (example ~1, page~\pageref{triangle function}),
all the coefficients of the sine terms really are~0.



\item Find the Fourier series for the following functions over the
interval $[-\pi,\pi]$:


  \begin{enumerate}
  \item $f(x)=x$. \hfill [Ans. ${\displaystyle
...
\end{enumerate}
\item In May's predator--prey model, find the first seven terms of the
