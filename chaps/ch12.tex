...
\footnote{Factorials  in probability}	
...
\footnote{$n!$ is difficult  to calculate}
...
\footnote{These estimates  are often adequate}	
...
$$
and 
\footnote{The relation between the half-life and  the decay constant}
therefore
$$
T=\dfrac{\ln{2}}{k}.
$$
...
\footnote{Randomness has structure} 
...
\footnote{ The addition rule\\ for probabilities}
\begin{quote}  
...
\footnote{ The product rule\\ for probabilities}
\begin{quote}
...
\footnote{ The probability\\ that something\\ doesn't happen}
\begin{quote}
...
\[p_n = \lim_{N\to\infty} \dfrac{N_n}{N}\ ,\]
it follows 
\footnote{The general formula for the expected value in a probability model}
...
$k<100$---then $(M-k)/M$ 
\footnote{$(M-k)/M$ is essentially equal to 1}
...
This section is an 
\footnote{The problem of  signal + noise}	
...
\includegraphics{../ps/calc2/lynx2.pdf}
...
it did.  \footnote{The power spectrum filters noise to detect periodic
...
\footnote{The noise level becomes stronger}	
...
\includegraphics{../ps/calc2/noise3.pdf}
If we increase the original noise level 
\footnote{The noise level becomes overwhelming}	
...
\includegraphics{../ps/calc2/noise4.pdf}
...
\footnote{Compare the signal  to a probe whose frequency can be varied} 	
...
\footnote{When the signal matches the  test frequency}	
...
\includegraphics{../ps/calc2/sintest1.pdf}  	  
On the right we see what happens 
\footnote{When the signal {\em doesn't\/} match the\\ test frequency}	
...
was 4:1.
\includegraphics{../ps/calc2/sintest2.pdf}
In the second the noise was stronger; the signal-to-noise ratio was 1:1.
\includegraphics{../ps/calc2/sintest3.pdf}
...
\footnote{The average value  of a function}
$a \leq x \leq b$ is
\[
...
\footnote{Integrals  with parameters  define functions}
...
\footnote{$D(\omega)$ peaks when  $\omega$ is the frequency  of the signal}
...
of the signal.
\mbox{}
\includegraphics{../ps/calc2/detect1.pdf}
...
\footnote{The peak in $D(\omega)$  is sharper if the signal  is
  tested over a  longer time interval} 
$0 \leq t \leq 100$ seconds.
\includegraphics{../ps/calc2/detect2.pdf}
...
\includegraphics{../ps/calc2/detect3.pdf}
...
\footnote{A periodic   signal \ldots   
  \ldots and its  frequency detector}
Can you guess a formula for $g(t)$?
\includegraphics{../ps/calc2/period3.pdf}
\includegraphics{../ps/calc2/detect4.pdf}
...
\footnote{Arbitrary phase shifts}\index{phase!shift}	
...
shift $\varphi$.  In particular, if \footnote{The average value varies with
...
\footnote{The problem  of phase \ldots}	
...
\footnote{The graphs of  $D_s$ and $D_c$}
...
\linebreak
\mbox{}
\includegraphics{../ps/calc2/detect5.pdf}
...
\footnote{The physical analogue of a detector  is a resonator}
...
\footnote{Resonance occurs all around us.  Sometimes it is a
...
\footnote{Integrals transform  $S$ into $D_s$ and $D_c$}
...
in a stereo set.
\footnote{The French mathematician Jean Baptiste Fourier (1768--1830)
...
\footnote{A detector that ignores phase differences}\index{phase}	
...
\footnote{\mbox{Power spectrum of} $3\sin(7t - \pi/3)$}
\includegraphics{../ps/calc2/detect6.pdf}
\begin{center}
...
\footnote{Two signals whose components differ  only in phase}
...
\mbox{}
\includegraphics{../ps/calc2/period4.pdf}
...
other.
\includegraphics{../ps/calc2/detect7.pdf}
...
\footnote{Detecting a periodic wave in a noisy signal}	
...
\includegraphics{../ps/calc2/detect8.pdf}
...
\footnote{In the power spectrum, noise and signal are separated}	
...
\includegraphics{../ps/calc2/detect9.pdf}
\includegraphics{../ps/calc2/detect10.pdf}
\mbox{}
\includegraphics{../ps/calc2/detect11.pdf}
\subsection{Exercises}
\label{subsec:exercises}
...
\footnote{The {\em true\/} spectrum\\ is the limit of the computed graphs\\ of the spectrum}	
...
\footnote{Difficulties with polynomial approximations}
...
\footnote{Approximating  periodic functions}
...
form 
\footnote{Trigonometric polynomial of degree $n$ and period $T$}
\begin{align*}
...
\footnote{Coefficients are independent of $n$} 
...
\footnote{Joseph Fourier (1768--1830)\index{Fourier, J.}
...
\]
Then the graph of $f$
\footnote{The graph of $f$  is ``triangular"}
simply consists of two line segments:
\includegraphics{../ps/calc2/sawtoo1.pdf}
...
\footnote{The Fourier series for $f$}
for~$f$: 
\[
...
\includegraphics{../ps/calc2/sawtoo2.pdf}
...
\footnote{Approximating a triangular wave-form}
...
example, is the graph of $\phi_{20}(x)$ over the interval $[-\pi,
  5\pi]$:
\includegraphics{../ps/calc2/sawtoo3.pdf}
...
\footnote{The general rule  for calculating  Fourier series} 
...
\footnote{Fourier series for the periodic solutions of May's predator-prey model} 
...
\includegraphics{../ps/calc2/BMay.pdf}
...
\footnote{Integrating products  of sines and cosines}
