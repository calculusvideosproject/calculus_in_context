...
\mbox{ and } R(t)$), \footnote{Rate equations tell us where to go next}
...
\footnote{Smaller steps generate a smaller discrepancy}
...
\item Change the next three lines to use the values we just calculated
  ...
  \footnote{The {\em sign} of {\tt deltat} determines whether we move
...
\footnote{With a computer we can generate lots of data and look for patterns}
...
as 
\footnote{Smaller steps generate a discrepancy which can be made as
  small as we like} 
...
\footnote{Approximations lead to exact values} 
...
$R(1)$ columns as well.
\footnote{The limit concept lies at the heart of calculus.  Later on
...
points).
\mbox{}
\includegraphics{../ps/calc1/picture3.pdf}
...
\mbox{}
\includegraphics{../ps/calc1/picture2.pdf}
The graphs become indistinguishable from each other and increasingly
 look like smooth
\footnote{Graphs made up of  line segments look like smooth curves if  the
...
\footnote{A single function  may not be specified  by a single equation}
...
\footnote{What does it mean  to "know" a number like~$\pi$?}
...
All this discussion of approximations may strike you as an unfortunate
 ...
 expect
\footnote{Being able to approximate a number to 12 decimal places is
...
have been added):
\includegraphics{../ps/calc1/picture4.pdf}\label{`logistic'}
...
of $y_{11}(t)$:
\includegraphics{../ps/calc1/picture5.pdf}\label{`sequence'}
...
stabilized.  If we
\footnote{Functions and graphs can be limits, too}
...
not have been distinguishable \footnote{Euler's method is the process of
...
\end{itemize}
\footnote{Even the process of building a mathematical model for a
...
\footnote{Calculators and computers really work by making approximations}
...
In many of the questions below you are asked to do things like divide
 ...
 places.  This assumes you have fast computers \footnote{Use common sense
