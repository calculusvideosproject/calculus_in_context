...
\label{chap:successive-approximations}
...
\label{sec:making-approximations}
...
\mbox{ and } R(t)$), [[NOTE:Rate equations tell us where to go next]]
...
\label{subsec:the-longest-march-begins-with-a-single-step}
...
\label{subsubsec:there-and-back-again-again}
...
[[NOTE:Smaller steps generate a smaller discrepancy]]
...
\item Change the next three lines to use the values we just calculated
  ...
  [[NOTE:The {\em sign} of {\tt deltat} determines whether we move
...
[[NOTE:With a computer we can generate lots of data and look for patterns]]
...
as 
[[NOTE:Smaller steps generate a discrepancy which can be made as
  small as we like]] 
...
[[NOTE:Approximations lead to exact values]] 
...
[[NOTE:The limit concept lies at the heart of calculus.  Later on
...
\label{subsec:one-picture-is-worth-a-hundred-tables}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/picture3.pdf}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/picture2.pdf}

The graphs become indistinguishable from each other and increasingly
 look like smooth
[[NOTE:Graphs made up of  line segments look like smooth curves if  the
...
\label{subsec:piecewise-linear-functions}
...
[[NOTE:A single function  may not be specified  by a single equation]]
...
\label{subsec:approximate-versus-exact}
...
[[NOTE:What does it mean  to "know" a number like~$\pi$?]]
...
All this discussion of approximations may strike you as an unfortunate
 ...
 expect
[[NOTE:Being able to approximate a number to 12 decimal places is
...
The fact that computers and calculators are really
 only dealing with approximations
[[NOTE:However \ldots   \mbox{}]]
...
\begin{enumerate}
\item Using this three-step approximation, what is $S_{3}(1.7)$?  What
 is $S_{3}(2.5)$?  \hfill


\item How would you modify SIRVALUE to get $S_{3000}(3)$ ?  Do it; what
 do you get?


\item What additional changes would you make to get the values of $t$,
...
\item Suppose we wanted to determine the value of $S_{300}(2.84135)$.

  \begin{enumerate}
  \item In which of the 300 segments of the graph of $S_{300}(t)$ would
 we look to find this information?

  \item What are the values of $t$, $S$, and $S'$ at the beginning of
 this segment?

  \item What is the equation of this segment?

  \item What is $S_{300}(2.84135)$ ?


\end{enumerate}
\item How would you modify SIRVALUE to calculate estimates for $S$,
 $I$, and $R$ when $t = -6$, using $\Delta t=.05$ ?  Do it; what do
 you get?



\item We want to use SIRPLOT to look at the graph of $S(t)$ over the
 first 20 days, using $\Delta t=.01$.

  \begin{enumerate}
  \item What changes would we have to make in the program?

  \item Sketch the graph you get when you make these changes.

  \item If you wanted to plot the graph of $I(t)$ over this same time
 ...
 hit its peak?
  ...
  \item Modify SIRPLOT to sketch on the same graph all three functions
...
\begin{enumerate}
\item Make these modifications; what value for $t$ do you get?


\item You could modify the {\tt PRINT t} command to also print out
other quantities.

  \begin{enumerate}
  \item What is the value of $I$ at its peak?

  \item What is the value of $S$ when $I$ is at its peak?  Does this
agree with the threshold value we predicted in chapter 1?


\end{enumerate}
\item Suppose you change the initial value of $S$ to be 5400 and run
the previous program.  Now what happens?  Why?


\item Suppose we wanted to know how long the epidemic lasts.  We could
...
stop and see what the value of $t$ was.
  ...
  \begin{enumerate}
  \item What modifications would you make in SIRVALUE to get this
information?

  \item Run your modified program.  What value do you get for $t$?

  \item Run the program using $\Delta t = .01$.  Now what is your
estimate for the duration of the epidemic?  What can you say about the
actual time required for $I$ to drop below 1?


\end{enumerate}
\item If we think the epidemic started with a single individual, we
...
infection arrived?
  ...
  \begin{enumerate}
  \item How many Recovereds were there at the start of the epidemic?
...
\label{sec:the-mathematical-implications---eulers-method}
\index{Euler's method}

\hyphenation{Eu-ler}
\subsection{Approximate Solutions}
\label{subsec:approximate-solutions}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/picture4.pdf}\label{`logistic'}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/picture5.pdf}\label{`sequence'}
...
stabilized.  If we
[[NOTE:Functions and graphs can be limits, too]]
...
not have been distinguishable [[NOTE:Euler's method is the process of
...
\label{subsubsec:bf-approximate-solutions-versus-exact}
...
\label{subsubsec:approximate-solutions}




\begin{enumerate}
\item Modify SIRVALUE and SIRPLOT to analyze the population of
...
exceed 100,000,000.
  ...
  \begin{enumerate}
  \item Estimate the population in 2085.

  \item Sketch the graph that describes this population growth.


\end{enumerate}
\end{enumerate}
\subsubsection{The Logistic Equation}
\label{subsubsec:the-logistic-equation}
...
\begin{enumerate}
\item By modifying SIRVALUE in the way we modified SIRPLOT to get
...
\item Referring to the graph of $y(t)$ obtained in the text on
 page~\pageref{`sequence'}, what can you say about the behavior of $y$
 as $t$ gets large?

  \begin{enumerate}
  \item Suppose we had started with $y(0)=1000$.  How would the
 population have changed over time? Why?

  \item Suppose we had started with $y(0)=1500$.  How would the
 population have changed over time? Why?

  \item Suppose we had started with $y(0)=0$.  How would the population
 have changed over time? Why?

  \item The number 1000 in the denominator of the rate equation is called
 the {\bf carrying capacity}\index{carrying capacity} of the system.
  Can you give a physical interpretation for this number?


\end{enumerate}
\item Obtain graphical solutions for the rate equation for different
...
\item Keep everything in the original problem unchanged except for the
...
\item Returning to the original logistic equation, modify SIRVALUE or
 DO--WHILE to find the value for $t$ such that $y(t)=900$.


\item Suppose we wanted to fit a logistic rate equation to a
...
\label{subsubsec:using-sequence}
...
\begin{enumerate}
\item Each Euler approximation is made up of a certain number of
...
\item What is the slope of the first graph?  What are the slopes of the
 two parts of the second graph?  [You should be able to answer these
 questions without resorting to a computer.]


\item Modify the line in the program SEQUENCE which determines the
...
\item Modify SEQUENCE to produce a sequence of Euler approximations to
...
\end{enumerate}
\item Modify SEQUENCE to construct a sequence of Euler approximations
 for population of Poland (from exercise 1, above).  Sketch the limit
 graph $P(t)$, and mark the values of $P(0)$ and $P(100)$ at the two
 ends.


\item Construct a sequence of Euler approximations to the function
...
\label{sec:approximate-solutions}
\index{solution!approximate}
...
[[NOTE:Even the process of building a mathematical model for a
...
\label{subsec:calculating-pi---the-length-of-a-curve}
...
\label{subsec:finding-roots-with-a-computer}
\index{root}

When we casually turn to our calculator 
[[NOTE:Calculators and computers really work by making approximations]]
...
\label{subsubsec:a-geometric-approach}
...
\put(0,0){\framebox(80,80){\parbox{80pt}{%
...
\end{thicklines}
\put(64,36){\framebox(8,8){\parbox{8pt}{%
...
\put(100,0){\framebox(80,80){\parbox{80pt}{%
...
\end{thicklines}
\put(140,36){\framebox(8,8){\parbox{8pt}{%
...
\put(220,0){\framebox(80,80){\parbox{80pt}{%
...
\end{thicklines}
\put(272,36){\framebox(8,8){\parbox{8pt}{%
...
\label{subsubsec:an-algebraic-approach----the-babylonian-algorithm}
\index{root}
...
\begin{observation}
\bf If \boldmath $x$ is an estimate
			for \boldmath $\sqrt{a}$, \\
...
In many of the questions below you are asked to do things like divide
 ...
 places.  This assumes you have fast computers [[NOTE:Use common sense
...
\begin{enumerate}
\item By using a computer to graph $y = x^2 -
 2^x$, find the solutions of the equation $x^2 = 2^x$ to four decimal
 place accuracy.

\end{enumerate}
\subsubsection{The program LENGTH}
\label{subsubsec:the-program-length}




\begin{enumerate}
\item Run the program LENGTH to verify that it gives the lengths of the
 individual segments and their total length.


\item What line in the program gives the instruction to work with the
 function $f(x) = x^2$?  What line indicates the number of segments to
 be measured?


\item Each segment has a left and a right endpoint.  What lines in the
 program designate the $x$- and $y$-coordinates of the left endpoint;
 the right endpoint?


\item Where in the program is the length of the $k$-th segment
...
\item Modify the program so that it uses 20 segments to estimate the
 length of the parabola.  What is the estimated value?


\item Modify the program so that it estimates the length of the
...
\item What is the length of the parabola $y = x^2$ over the interval $0
 \leq x \leq 1$, correct to 8 decimal places?  What is the length,
 correct to 12 decimal places?


\item Starting at the origin, and moving along the parabola $y=x^2$,
 where are you when you've gone a total distance of 10?


\item Modify the program to find the length of the curve $y = x^3$ over
 the interval $0 \leq x \leq 1$.  Find a value that is correct to 8
 decimal places.


\item {\bf Back to the circle.}  Consider the unit circle centered at
 ...
 x \leq 1$ should then be exactly $\pi$.
  ...
  \begin{enumerate}
  \item Divide the interval into 100 pieces---what is the corresponding
 length?

  \item How many pieces do you have to divide the interval into to get an
 accuracy equal to that of Archimedes?

  \item Find the length of the curve $y = g(x)$ over the interval $-1
 \leq x \leq 1$, correct to eight decimal places accuracy.


\end{enumerate}
\item This question concerns the function $h(x) = \dfrac{4}{1 + x^2}$.

  \begin{enumerate}
  \item Sketch the graph of $y = h(x)$ over the interval $-2 \leq x \leq
 2$.

  \item Find the length of the curve $y = h(x)$ over the interval $-2
 \leq x \leq 2$.


\end{enumerate}
\item Find the length of the curve $y=\sin x$ over the interval $0 \leq
 x \leq \pi$.


\end{enumerate}
\subsubsection{The program BABYLON}
\label{subsubsec:the-program-babylon}




\begin{enumerate}
\item Run the program BABYLON on a computer to verify the tabulated
 estimates for $\sqrt{5}$.


\item In whatever computer language you are using, it should be
...
\item Use the Babylonian algorithm to find $\sqrt{80}$.

  \begin{enumerate}
  \item First use 2 as your initial estimate.  How many steps are needed
 for the calculations to stabilize---that is, to reach a value that
 doesn't change from one step to the next?

  \item Since $9^2 = 81$, a good first estimate for $\sqrt{80}$ is 9.
  How many steps are needed this time for the calculations to
 stabilize?  Are the final values in (a) and (b) the same?


\end{enumerate}
\item Use the Babylonian algorithm to find $\sqrt{250}$ and
...
\item The Babylonian algorithm is considered to be {\bf very fast}, in
...
\begin{enumerate}
\item We have considered the logistic equation
\[
y' = {\displaystyle .1y \left( 1-\frac{y}{1000} \right)}
\]
On page~\pageref{`logistic'} we looked at the resulting graph of $y$
 ...
 is the value given by the logistic equation for the given $y$ value.
  ...
  \begin{enumerate}
  \item For what value of $y$ does this graph take on its largest value?
  Where does this $y$ value appear in the graph of $y(t)$ versus $t$?

  \item For what values of $y$ does the graph of $y'$ versus $y$ cross
...
\label{subsubsec:grids-on-graphs}
...
\begin{enumerate}
\item If the screen window runs from $-20$ to 120 horizontally, and 250
...
\item Go back to our basic $S$-$I$-$R$ model.  Modify SIRPLOT to
