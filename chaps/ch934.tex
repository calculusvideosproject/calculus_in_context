...
Here is why.  The gradient\index{gradient} vector $\mbox{grad} f (a, b)$ 
        \mar{A proof}
tells us how the graph of $z = f(x, y)$ is tilted at the point $(a, b)$.  (We discuss the geometric meaning of the gradient on [[REF:page542]].)
        % qqq
...
{\bf [[EX:1]]}.  We'll start with the function
        \[
z = f(x, y) =  x^3 - 4 x - y^2
\]
...
{\bf [[EX:2]]}.  Here is a somewhat more complicated function:
        \[
...
{\bf [[EX:3]]}.  Here is a function whose critical points we {\em can't\/} find using just algebraic computations:
        \[
...
{\bf [[EX:1]]}.  Let's see how the method of steepest ascent will find the local maxima of the function 
        \[
z = h(x, y) = x^2 y^2 - x^4 - y^4 - 2x^2 + 5 xy + y
\]
...
{\bf [[EX:2]]}.  Manufacturing companies ship their products to regional warehouses from large distribution centers.   Suppose a company has regional warehouses at $A$, $B$, and $C$, as shown on the map at the left.  Where should it put its distribution center $X$ so as to minimize the total cost of supplying the three regional warehouses?
\label{distribstart} 
