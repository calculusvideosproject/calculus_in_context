...
\item[The Poisson Distribution] [[SEC:12.2]] continues the probability theme by developing the Poisson distribution and using it to study the frequency of radioactive decay events.  
\item[The Power Spectrum] [[SEC:12.3]] builds on the study of periodicity begun in chapter 7. We develop the Fourier transform, a basic tool in the sciences for detecting the relative strength of periodic components in a noisy data set.
\item[Fourier Series]  [[SEC:12.4]] expands on some of the ideas in chapter 11.  Here we develop tools for approximating functions over intervals using sums of sine and cosine terms.  This is an extensively used method in a wide range of disciplines, from thermodynamics to music synthesis.
\end{description} 
...
\]
Using the reduction formula derived in [[SEC:11.5]] for antiderivatives of $\sin^n{x}$, we have a similar reduction formula for the $I_k$:
\begin{eqnarray*}
I_k&=&\int_0^{\pi/2}\sin^kx\,dx  \\
...
{\bf [[EX:1]]}\quad How likely is it to get four 2's if we roll twelve dice?  The probability of getting a 2 by throwing one die is ${1\over 6}$.  Therefore the answer to the question is
\[
...
{\bf [[EX:2]]}\quad What is the probability of getting exactly 47 heads if we flip 100 pennies? Since the probability of getting heads on a single toss of a penny is ${1\over 2}$, 
\[
