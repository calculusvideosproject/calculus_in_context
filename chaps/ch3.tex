...
\footnote{The time of sunrise  is a function}
...
\footnote{Calculate the rate using earlier and  later dates}
...
\]
\footnote{Notice that, in each case, we have calculated the rate on a
...
the $S$-$I$-$R$ model.  
\footnote{Predictions over  long time spans  are less reliable} 
...
weight).  \footnote{Examples of rates} Other common rates are the annual
...
\footnote{The rate of change  of a rate}
...
\includegraphics{../ps/calc1/profile.pdf}
...
\index{graph!data}
\includegraphics{../ps/calc1/sunrise1.pdf}
\begin{minipage}{2in}
...
\footnote{Zoom in on the graph with a microscope}
...
\includegraphics{../ps/calc1/sunrise2a.pdf}
...
for each day of the year.  \footnote{The graph looks straight under a
...
$-1.5$ min/day, 
\footnote{Slope and  rate calculations  are the same}
...
the space between the data points.  \footnote{High-power magnification is
...
\includegraphics{../ps/calc1/mag1.pdf}
\includegraphics{../ps/calc1/mag2.pdf}
\includegraphics{../ps/calc1/mag3.pdf}
\includegraphics{../ps/calc1/mag4.pdf}
\includegraphics{../ps/calc1/mag5.pdf}
\includegraphics{../ps/calc1/mag6.pdf}
...
\footnote{The field of view  of a window}
...
\footnote{The microscope used to study functions is real, but it is
...
We should treat the values of $\Delta y/\Delta x$ as \footnote{The slope is
...
\footnote{A microscope gives  a local view}
...
\footnote{All the standard functions are  locally linear at  almost all points}
...
\includegraphics{../ps/calc1/cusp1.pdf}
\enlargethispage*{20pt}
...
\includegraphics{../ps/calc1/cusp2.pdf}
...
fail to be locally linear anywhere!  Such functions are called {\bf
  fractals}.\index{fractal} \footnote{Fractals are locally {\em
...
\includegraphics{../ps/calc1/prob114.pdf}
\end{enumerate}
...
\includegraphics{../ps/calc1/prob115.pdf}
  \begin{enumerate}
...
$\Delta x$ smaller than the spacing between the data points.  \footnote{A
...
\footnote{There are rules for finding derivatives}
...
is a matter of convention.  \footnote{Convention: if the graph of $f$ is
...
\footnote{The German philosopher Gottfried Wilhelm Leibniz (1646-1716)
...
\footnote{Travel time  depends on speed}
...
\footnote{How does travel time respond to  changes in speed?}
how much does any \\
...
mph.
\footnote{The slope of the graph in the microscope window}
...
\footnote{How travel time changes with speed}
...
\footnote{$\Delta s$ and $\Delta T$ now have a special meaning}
...
\footnote{$\Delta s$ and $\Delta T$ are coordinates in the
...
\footnote{Global vs. local descriptions}\index{global}
...
\footnote{The microscope equation is the analytic form of local linearity}
...
\item $f'(a)$ is small and negative $\Rightarrow$ small decrease in
  $y\/$.
\end{itemize}
\includegraphics{../ps/calc1/micro1.pdf}
...
\footnote{The microscope equation is the recipe for building solutions to
  rate equations}
...
\footnote{How much does  a house expand  in the heat?}
...
multiplication.
\footnote{Exact values can be harder to calculate than estimates.}
...
\footnote{How uncertain is  the calculated value  of the volume?}
...
\]
Thus, for example, 
\footnote{The microscope equation describes how errors propagate}
...
\footnote{Bigger numbers  have bigger errors} 
...
\footnote{A watchmaker who measures the tiny objects that go into a watch
...
\footnote{Absolute and  relative error}
...
\footnote{Percentage error  is relative error}
...
\footnote{The derivative  is a function  in its own right}\index{function!derivative}
...
\includegraphics{../ps/calc1/funny.pdf}
Then we know that its derivative $L'$ must be 0 at points $a$, $b$,
$c$, and $d$; 
\footnote{Finding a derivative   ``by eye''} 
...
\footnote{Formulas are combinations of  basic functions}
...
\end{tabular}
\end{center}
\footnote{Remember that the input to the trigonometric functions is
...
$f(x) = g(x) + h(x)$.
\footnote{The addition rule}\index{addition rule}
...
\footnote{The constant multiple rule}
...
\includegraphics{../ps/calc1/sinprob.pdf}
...
\includegraphics{../ps/calc1/shaded.pdf}
  \begin{enumerate}
...
derivative.
\includegraphics{../ps/calc1/qualsketch.pdf}
\pagebreak
\end{enumerate}
...
\footnote{How volume responds to changes in length}
\[
\Delta V \approx 120000 \ 
...
microscope equation with it: \footnote{How length responds to
...
\]
We can condense 
\footnote{How volume responds to changes in temperature}
this to     
\[
\Delta V \approx 48 \ 
...
\footnote{How the rates combine}
	$$
...
In these two expressions we have the same microscope equation, so the multipliers must be equal.  Thus, we recover the chain rule:
	...
	$$
\footnote{Recall that Leibniz\index{Leibniz, G.}\index{differential} worked
...
two
	\footnote{An input--output chain}
diagrams:
	$$
...
\footnote{A chain and its links}
...
The third function, $y = g(f(x))$, needs a name of its own; let's call it $h$.  Thus 
	\footnote{Composition  of functions}
	$$
	 y = h(x) = g(f(x)).
	$$
...
\footnote{The time of sunrise depends on latitude as well as on the date}
...
\footnote{A function of several variables has several rates of change}
...
\index{multiplier}
	...
	\footnote{The microscope equation for dates}
	$$
...
T(76,42^\circ\mbox{N}) \approx \mbox{6:09.7} .
	...
	\footnote{The microscope equation for latitudes}
	$$
...
\footnote{To find a partial derivative, treat the other variable as a constant}
...
Changes $\Delta l$ and $\Delta w$ in the dimensions produce a change
	\footnote{The full microscope equation for rectangular area}
	$$
