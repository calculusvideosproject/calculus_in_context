...
\label{chap:series-and-approximations}
...
[[NOTE:Ordinary arithmetic  lies at the heart  of all calculations]]
...
\label{sec:approximation-near-a-point-or-over-an-interval}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sin1.pdf}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sin2.pdf}
...
[[NOTE:There's more than one way to make the "best fit" to a given curve]]
...
In chapter 3 we discovered that functions
...
[[NOTE:The behavior of a function can often be inferred from the
...
[[NOTE:We want the  best fit at $x=0$]]
it
...
[[NOTE:The best fit should pass through the point $(0, f(0))$]]
...
But we're 
[[NOTE:The best fit should have the right slope at $(0, f(0))$]]
...
[[NOTE:The final criterion  for best fit at $x=0$]]
...
We have used the
[[NOTE:Notation for  higher derivatives]]
...
With this notation 
[[NOTE:The desired rule for finding the coefficients]]
...
above to get a
[[NOTE:General rule for  the Taylor polynomial  at $x=0$]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sin3.pdf}\index{Taylor polynomial!graphs for $\sin{x}$}



While each polynomial eventually wanders off
[[NOTE:The higher the degree of the polynomial,  the better the fit]]
...
[[NOTE:Approximating polynomials for  other basic functions]]
...
[[NOTE:General rule for  the Taylor polynomial  at $x=a$]]
...
\subsubsection{A computer program for graphing Taylor polynomials}
Shown above
...
\label{subsec:new-taylor-polynomials-from-old}
...
\subsubsection{Substitution in Taylor Polynomials.}
...
\subsubsection{Multiplying Taylor Polynomials.}
Suppose we wanted the 5-th
...
\subsubsection{Differentiating Taylor Polynomials.}
Suppose we know a Taylor
...
\subsubsection{Integrating Taylor Polynomials.}
Again suppose we have functions
...
[[NOTE:Graph the difference between a function and its Taylor polynomial]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/j101.pdf}



This graph looks very much like a cubic polynomial.  
[[NOTE:The difference looks like a power of $x$]] 
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/j102.pdf}

\noindent \includegraphics[width=\linewidth]{../ps/calc2/j103.pdf}



If $\sin(x) - P(X)$ were {\em exactly\/} a multiple of $x^5$, 
[[NOTE:Finding the multiplier]]
...
[[NOTE:What the fit means computationally]]        
...
[[NOTE:A formula for  the remainder]]       
...
[[NOTE:Another formula for the remainder]]   
...
[[NOTE:Determining $C$  from $f$ at $x = a$]]       
...
Consider $\sqrt{x}$ near $x = 100$.  The second degree Taylor polynomial for $\sqrt{x}$, centered at $x = 100$, is
        \[
Q(x) = 10 + \dfrac{(x - 100)}{20} - \dfrac{(x - 100)^2}{8000}.
\]

\noindent \includegraphics[width=\linewidth]{../ps/calc2/j104.pdf}

\noindent \includegraphics[width=\linewidth]{../ps/calc2/j105.pdf}


Plot $y = Q(x)$ and $y = \sqrt{x}$ together; 
[[NOTE:Version 1:  the remainder is $O((x-100)^3)$]]
...
[[NOTE:Version 2:  determining $C$ in terms of $\sqrt{x}$ at $x = 100$]]    
...
[[NOTE:Accuracy of  the polynomial approximation]]
...
[[NOTE:Version 3:   an explicit   error bound]]  
...
An obvious use 
[[NOTE:Now you can do anything your calculator can!]]
...
{\bf Evaluating Integrals.} The fundamental theorem of
...
[[NOTE:The error function]] defined by
\[
...
\]
giving us, finally, 
[[NOTE:A formula for approximating the  error function]]
...
Our final application of Taylor polynomials
...
[[NOTE:Extending the  `big oh' notation]]    
...
\begin{enumerate}
\item Find a seventh degree Taylor polynomial centered at $x=0$ for the
...
\dfrac{x^5}{5 \cdot 5!} - \dfrac{x^7}{7 \cdot 7!}$.}
  ...
  \item $\dint \sin(x^2) \, dx$.


\end{enumerate}
\item Plot the 7-th degree polynomial you found in part (a) above over
...
\item Using the seventh degree Taylor approximation
        \[
...
\item Calculate the values of $\sin(.4)$ and $\sin(\pi/12)$ using the
...
\item Find the third degree Taylor polynomial for $g(x) = x^3 - 3x$ at
...
\item Find the seventh degree Taylor polynomial centered at $x=\pi$
for\\ (a) $\sin(x)$; \quad (b) $\cos(x)$; \quad (c) $\sin(3x)$.


\item In this problem you will compare computations using Taylor
polynomials centered at $x=\pi$ with computations using Taylor
polynomials centered at $x=0$.

  \begin{enumerate}
  \item Calculate the value of $\sin(3)$ using a seventh degree Taylor
polynomial centered at $x=0$.  How many decimal places of your estimate
appear to be fixed?

  \item Now calculate the value of $\sin(3)$ using a seventh degree
...
\end{enumerate}
\item Write a program which evaluates a Taylor polynomial to print out
...
\item {\bf Why \boldmath $0!=1$}. \quad When you were first introduced to
...
\end{alignat*}
  ...
  \begin{enumerate}
  \item Show that to preserve property A we have to define $2^0=1$.

  \item Show that we then have to define $2^{-3}=1/2^3$ if we are to
continue to preserve property A.

  \item Show why $2^{1/4}$ must be $\sqrt[4]{2}$.

  \item In the same way, you should convince yourself that a basic
...
we have to define $0!=1$.
  ...
  \item Show that there is no way to define $(-1)!$ which preserves this
property.


\end{enumerate}
\item Use the general rule to derive the 5-th degree Taylor polynomial
...
\item Use the general rule to derive the formula for the $n$-th degree
...
\item Use the result of the preceding problem to get the 6-th degree
Taylor polynomial centered at $x=0$ for $1/\sqrt[3]{1+x^2}$.

\ans{$1 - \dfrac{1}{3}x^2 + \dfrac{2}{9}x^4 - \dfrac{14}{81}x^6$.}


\item Use the result of the preceding problem to approximate
\[
\int_0^1 \dfrac{1}{\sqrt[3]{1+x^2}}\, dx.
\]


\item Calculate the first 7 decimals of erf$(.3)$. Be sure to show why
...
\item Apply the general formula for calculating Taylor polynomials
...
\ans{$\tan(x)\approx x+x^3/3+2x^5/15$.}
  ...
  \begin{enumerate}
  \item Recall that $\tan(x) =\sin(x)/\cos(x)$.  Multiply the 5-th degree
...
\end{enumerate}
\item Show that the $n$-th degree Taylor polynomial centered at $x=0$
...
\item Find a formula for the $n$-th degree Taylor polynomial centered
at $x=1$ for $\ln(x)$.

  \begin{enumerate}
  \item Compare your answer to part (a) with the Taylor polynomial
centered at $x=0$ for $\ln(1-x)$ you found in the previous problem.
Are your results consistent?


\end{enumerate}
\item The first degree Taylor polynomial for $e^x$ at $x=0$ is $1+x$.
Plot the remainder $R_1(x) = e^x - (1+x)$ over the interval $-.1 \leq
x \leq .1$.  How does this graph demonstrate that $R_1(x) = O(x^2)$ as
$x \rightarrow 0$?

  \begin{enumerate}
  \item There is a constant $C_2$ for which $R_1(x) \approx C_2 x^2$ when
$x \approx 0$.  Why?  Estimate the value of $C_2$.


\end{enumerate}
\item This concerns the second degree Taylor polynomial for $e^x$ at
...
$R_2(x) = O(x^3)$ as $x \rightarrow 0$?
  ...
  \begin{enumerate}
  \item There is a constant $C_3$ for which $R_2(x) \approx C_3 x^3$ when
$x \approx 0$.  Why?  Estimate the value of $C_3$.


\end{enumerate}
\item Let $R_3(x) = e^x - P_3(x)$, where $P_3(x)$ is the third degree
Taylor polynomial for $e^x$ at $x=0$.  Show $R_3(x) = O(x^4)$ as $x
\rightarrow 0$.


\item At first glance, Taylor's theorem says that
\[
...
\item Using a suitable formula (that is, a Taylor polynomial with
remainder) for each of the functions involved, find the indicated limit.

  \begin{enumerate}
  \item ${\displaystyle \lim_{x \to 0} \dfrac{\sin(x)}{x}}$ \hfill [Answer: 1]


  \item ${\displaystyle \lim_{x \to 0} \dfrac{e^x - (1 + x)}{x^2}}$ \hfill
[Answer: 1/2]
  ...
  \item ${\displaystyle \lim_{x \to 0} \dfrac{x - \sin(x)}{x^3}}$ \hfill
[Answer: 1/6]


  \item ${\displaystyle \lim_{x \to 0} \dfrac{\sin(x^2)}{1 - \cos(x)}}$


\end{enumerate}
\item Suppose $f(x) = 1 + x^2 + O(x^4)$ as $x \rightarrow 0$.  Show
that
\[
...
\item Using $\sin x = x - \tfrac{1}{6} x^3 + O(x^5)$ as $x \rightarrow
0$, show
\[
...
\]
  ...
  \begin{enumerate}
  \item Using $\cos x = 1 - \tfrac{1}{2} x^2 + \tfrac{1}{24} x^4 +
...
\]
  ...
  \item Using the previous parts, show $(\sin x)^2 + (\cos x)^2 = 1 +
O(x^5)$ as $x \rightarrow 0$.  (Of course, you already know $(\sin
x)^2 + (\cos x)^2 = 1$ {\em exactly}.)


\end{enumerate}
\item Apply the general formula for calculating Taylor polynomials to
the tangent function to get the 5-th degree approximation.



  \begin{enumerate}
  \item Recall that $\tan(x) = \sin(x)/\cos(x)$, so $\tan(x) \cdot
...
show that $e^{\sin{x}} = 1 + x + \tfrac{1}{2} x^2 + O(x^4)$ as              $x \to 0$.
  ...
  \begin{enumerate}
  \item Apply the general formula to obtain the third degree Taylor
polynomial for $e^{\sin{x}}$ at $x = 0$, and compare your result with
the formula in part (a).


\end{enumerate}
\item Using $\dfrac{e^x -1}{x} = 1 + \tfrac{1}{2} x + \tfrac{1}{6} x^2 +
...
\item Show that the following are true as $x \rightarrow \infty$.

  \begin{enumerate}
  \item $x + 1/x = O(x)$.

  \item $5x^7 - 12x^4 + 9 = O(x^7)$.

  \item $\sqrt{1 + x^2} = O(x)$.

  \item $\sqrt{1 + x^p} = O(x^{p/2})$.


\end{enumerate}
\item Let $f(x) = \ln(x)$.  Find the smallest bound $M$ for which
        \[
...
\]
  ...
  \begin{enumerate}
  \item Let $P_3(x)$ be the degree 3 Taylor polynomial for $\ln(x)$ at $x
...
for all $x$ satisfying $|x - 1| \leq .5$.
  ...
  \item If you use $P_3(x)$ to approximate the value of $\ln(x)$ in the
interval $.5 \leq x \leq 1.5$, how many digits of the approximation
are correct?

  \item Suppose we restrict the interval to $|x - 1| \leq .1$.  Repeat
...
\end{enumerate}
\item Suppose $\phi(x) = O(x^6)$ as $x \rightarrow 0$.  Show the following.

  \begin{enumerate}
  \item $\phi(x) = O(x^5)$ as $x \rightarrow 0$.

  \item $\phi(x) = o(x^5)$ as $x \rightarrow 0$.

  \item It is false that $\phi(x) = O(x^7)$ as $x \rightarrow 0$.  (One
...
$\phi(x) = O(x^7)$ is false.)
  ...
  \item It is false that $\phi(x) = o(x^6)$ as $x \rightarrow 0$.


\end{enumerate}
\item Sketch the graph $y = x \ln(x)$ over the interval $0 < x \leq 1$.
...
[[NOTE:You have seen  infinite sums before]]
...
[[NOTE:Infinite degree polynomials are to be viewed like infinite decimals]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/Darct.pdf}\index{Taylor polynomial!graphs for $1/(1+x^2)$}

It appears that the 
[[NOTE:A Taylor series  may not converge  for all values of $x$]]
...
[[NOTE:Brook Taylor (1685--1731) was
...
On the one hand it is perhaps not too surprising that a
...
\begin{enumerate}
\item Suppose you wanted to use the Taylor series centered at $x=0$ to
calculate $\sin(100)$.  How large does $n$ have to be before the term
$(100)^n/{n!}$ is less than 1?

  \begin{enumerate}
  \item If we wanted to calculate $\sin(100)$ directly using this Taylor
...
(but still using the Taylor series centered at $x=0$)?  Do it.
  ...
  \item Show that we can calculate the sine of any number by using a
Taylor series centered at $x=0$ {\em either} for $\sin(x)$ {\em or}
for $\cos(x)$ to a suitable value 0f $x$ between 0 and $\pi/4$.


\end{enumerate}
\item Suppose we wanted to calculate $\ln{5}$ to 7 decimal places. An
...
\ans{The Taylor series converges for $-1\leq x < 1$.}
  ...
  \begin{enumerate}
  \item Explain how you could use the fact that $\ln(1/A)=-\ln{A}$ for
any real number $A>0$ to evaluate $\ln{x}$ for $x>2$.  Use this to
compute $\ln{5}$ to 7 decimals.  How far out in the Taylor series did
you have to go?

  \item If you wanted to calculate $\ln 1.5$, you could use the Taylor
...
\end{enumerate}
\item We can improve the speed of our calculations of the logarithm
function slightly by the following series of observations: 
  \begin{enumerate}
  \item Find
...
\ans{$u-u^2/2+u^3/3-u^4/4+u^5/5+\cdots$} 
  ...
  \item Find the Taylor series centered at $u=0$ for
\[
...
(Remember that $\ln(A/B) = \ln{A} - \ln{B}$.)
  ...
  \item Show that {\em any} $x>0$ can be written in the form
$(1-u)/(1+u)$ for some suitable $-1<u<1$.

  \item Use the preceding to evaluate $\ln{5}$ to 7 decimal places. How
far out in the Taylor series did you have to go?


\end{enumerate}
\item Evaluate $\arctan(.5)$ to 7 decimal places.
...
\end{enumerate}
\item {\bf Calculating $\pi$}\index{$\pi$, calculation of} \quad The
...
to 3 decimal places?
  ...
  \begin{enumerate}
  \item The reason the preceding approximations converged so slowly was
...
\end{enumerate}
\item {\bf Raising $e$ to imaginary powers}\index{$e$, raised to
...
so makes perfectly good sense even if $x$ is a complex number
  ...
  \begin{enumerate}
  \item Show that if $s$ is any real number, we must define $e^{i\,s}$ to
be $\cos(s)+i\,\sin(s)$ if we want to preserve this property.

  \item Show that $e^{\pi\,i}=-1$.

  \item Show that if $r+s\,i$ is any complex number, we must have
\[
e^{r+s\,i}=e^r(\cos{s}+i\,\sin{s})
\]
if we want complex exponentials to preserve all the right properties.

  \item Find a complex number $r+si$ such that $e^{r+s\,i}= -5$.


\end{enumerate}
\item {\bf Hyperbolic trigonometric functions}\index{function!hyperbolic}\index{$\cosh$}\index{$\sinh$} The hyperbolic trigonometric
...
the adjectives {\em hyperbolic} and {\em trigonometric}.
  ...
  \begin{enumerate}
  \item Modify the Taylor series centered at $x=0$ for $e^x$ to find a
Taylor series for $\cosh(x)$.  Compare your results to the Taylor
series centered at $x=0$ for $\cos(x)$.

  \item Now find the Taylor series centered at $x=0$ for $\sinh(x)$.
Compare your results to the Taylor series centered at $x=0$ for
$\sin(x)$.

  \item Parts (a) and (b) of this problem should begin to explain the
...
\end{enumerate}
\item Consider the Taylor series centered at $x=0$ for $(1+x)^c$.

  \begin{enumerate}
  \item What does the series give if you let $c=2$?  Is this reasonable?

  \item What do you get if you set $c=3$?

  \item Show that if you set $c=n$, where $n$ is a positive integer, the
...
and the last three terms of this formula.
  ...
  \item Use an appropriate substitution for $x$ and a suitable value for
$c$ to derive the Taylor series for $1/(1-u)$. Does this agree with
what we previously obtained?

  \item Suppose we want to calculate $\sqrt{17} \,$. We might try letting
$x=16$ and $c=1/2$ and using the Taylor series for $(1+x)^c$.  What
happens when you try this?

  \item We can still use the series to help us, though, if we are a
...
decimal place accuracy.  How many terms does it take?
...
\end{enumerate}
\item Show that, in general, the number of multiplications and
...
\item How many multiplications and divisions are required to evaluate
this expression?  \ans{$3+4+4+4+4+1=20$.}


\item Thus this way of evaluating $P_{11}(x)$ is roughly three times as
...
\item Find a general formula for the number of multiplications and
...
\item Write two computer programs to evaluate the 300th degree Taylor
...
length of time required.
  ...
  \begin{enumerate}
  \item Use these two programs to graph the
...
\label{sec:power-series-and-differential-equations}
...
\subsubsection{Note}
In general, once we have derived a power series expression
...
[[NOTE:Friedrich Wilhelm Bessel (1784--1846) was a German astronomer who studied the
...
\addtolength{\arraycolsep}{3pt}%
In general, 
[[NOTE:Finding the  coefficient of $x^n$]]
...
[[NOTE:The graph of the \mbox{Bessel function $J_0$}]]
polynomial approximations of degree 2, 4, 6, \ldots , 30 over the
interval $[0,14]$:

\noindent \includegraphics[width=\linewidth]{../ps/calc2/Bessel.pdf}\index{Bessel function!$J_0$, graph of}
...
\label{subsec:the-s-i-r-model-one-more-time}
...
As before, 
[[NOTE:Finding the coefficients of the power series  for $S(t)$]]
...
[[NOTE:Finding the  power series  for $I(t)$]]
...
The opening command in this program introduces a new
...
[[NOTE:Extending SIRSERIES to graph $S$ and $I$]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sircalc.pdf}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sircalc2.pdf}
...
\begin{enumerate}
\item Find power series solutions of the form
\[
...
\end{enumerate}
\item Find power series solutions to the differential equation
$y''=-y$.  Start with
\[
y = a_0 + a_1x + a_2 x^2 + a_3 x^3 + \cdots + a_n x^n + \cdots .
\]
...
only odd powers, with $a_1$ as arbitrary constant.
...
powers, with $a_1$ as an arbitrary constant.
  ...
  \begin{enumerate}
  \item The two power series you obtained in part a) are the Taylor
...
\end{enumerate}
\item {\bf The Bessel Equation.}\index{Bessel equation}\index{Bessel function}


  \begin{enumerate}
  \item Take $p=1\,$.  The solution satisfying the initial condition $y'
...
$x^{2n+1}$?
  ...
  \item Show by direct calculation from the series for $J_0$ and $J_1$
that
\[
J_0' = - J_1.
\]
  ...
  \item To see, from another point of view, that $J_0' = -
...
\end{enumerate}
\item When we found the power series expansion for solutions to the
...
satisfies $y=1$ and $y'=1$ when $x=0$?

  \begin{enumerate}
  \item You might get more
...
\end{enumerate}
\item {\bf Legendre's differential equation}\index{Legendre's equation}
\[
...
``quantum"\index{quantum}---in this case, the number 1/2.

  \begin{enumerate}
  \item Find
power series solutions of Legendre's equation.

  \item Quantization of
...
\end{enumerate}
\item It turns out that the power series solutions to the $S$-$I$-$R$
...
\item {\bf Logistic Growth}\index{logistic growth} Find the first five
...
the carrying capacity is 1 and the intrinsic growth rate is 1.
  ...
  \begin{enumerate}
  \item Using the initial condition $y=.1$ when $x=0$, plot this power
series solution on the same graph as the solution obtained by Euler's
method.  How do they compare?

  \item Do the same thing with initial conditions $y=2$ when $x=0$.
...
It is,
[[NOTE:Convergence means  essentially that  ``decimals stabilize"]]
...
In other words, 
[[NOTE:What it means for  an infinite sum  to converge]]
...
[[NOTE:A necessary condition for convergence]]
...
\subsubsection{Remark:}
It is important to recognize what this criterion does
and does [[NOTE:{\em Necessary} and {\em sufficient} mean different
...
[[NOTE:Detecting  divergent series]]
...
\label{subsubsec:the-harmonic-series}
...
\]
will converge.  
[[NOTE:An important counterexample]]
...
\label{subsec:the-geometric-series}
...
[[NOTE:To avoid divergence, $|x|$ must be less than 1]]
...
[[NOTE:A simple expression for the partial sum $S_n$]]
...
[[NOTE:Finding the limit of $S_n$ as $n \to \infty$]]
...
Similarly, 
[[NOTE:Summing another geometric series]]
...
To summarize, we have thus proved that
        [[NOTE:Convergence and divergence of the geometric series]]


\begin{center}
\framebox[5.0in] {\parbox{4.45in}{ 
The geometric
...
\label{subsec:alternating-series}

A large class of common 
[[NOTE:Many common series are alternating, at least for some values of $x$]]
...
\subsubsection{Convergence criterion for alternating series.}
Let us write a generic alternating series as
\[
...
It is this property 
[[NOTE:Alternating series  are easy to test  for convergence]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/altseries.pdf}
...
[[NOTE:The partial sums oscillate, with the exact sum trapped between
...
[[NOTE:A simple estimate  for the accuracy  of the partial sums]]
...
\subsubsection{Proof:}
Suppose $n$ is odd.  Then we have, as above, that
...
\subsubsection{Example.}
Let's apply the error estimate 
[[NOTE:Estimating the error in approximating $\cos(.7)$]]
...
[[NOTE:How many terms  are needed  to obtain accuracy  to 12 decimal places?]]
...
[[NOTE:It may be possible  to convert  a given problem  to one involving 
  alternating series]]
...
\label{subsec:the-radius-of-convergence}
...
automatically
[[NOTE:The answer to the convergence question]]
...
[[NOTE:Radius of convergence of the geometric series]]
...
[[NOTE:Convergence is determined by the ``tail" of the series]]
...
% --- end observation ---
Let's see 
[[NOTE:The ratio test for the geometric series $\ldots$]]
...
[[NOTE:Finding the limit of $|b_{m+1}|/|b_m|$ may require some algebra]]
...
\subsubsection{Example.}
Let's apply the ratio test to the Taylor series
...
\begin{enumerate}
\item Find a formula for the sum of each of the following power series
...
\item Determine the value of each of the following infinite sums.
...
\item {\bf The Multiplier Effect}.  Economists know that the effect on
...
amount spend in the town is thus given by a {\em series.}
  ...
  \begin{enumerate}
  \item Write the series giving the total amount of money spent in the
town and calculate its sum.

  \item Your answer in a) is a certain multiple of $A$---what is the
multiplier?

  \item Suppose the recirculation rate is $r$ instead of 3/5.  Write the
series giving the total amount spent and calculate its sum.  What is
the multiplier now?

  \item Suppose the recirculation rate is 1/5; what is the multiplier in
this case?

  \item Does a lower recirculation rate produce a smaller multiplier
effect?
...
\end{enumerate}
\item Which of the following alternating series converge, which diverge? Why? \\


  \begin{enumerate}
  \item ${\displaystyle \sum_{n=1}^{\infty} (-1)^n\dfrac{n}{n^2+1}}$

  \item ${\displaystyle \sum_{n=1}^{\infty} (-1)^n\dfrac{1}{\sqrt{3n+2}}}$

  \item ${\displaystyle \sum_{n=2}^{\infty} (-1)^n\dfrac{n}{\ln{n}}}$

  \item ${\displaystyle \sum_{n=1}^{\infty} (-1)^n\dfrac{n}{5n-4}}$

  \item ${\displaystyle \sum_{n=1}^{\infty} (-1)^n\dfrac{\arctan{n}}{n}}$

  \item ${\displaystyle \sum_{n=1}^{\infty} (-1)^n\dfrac{(1.0001)^n}{n^{10}+1}}$

  \item ${\displaystyle \sum_{n=1}^{\infty} (-1)^n\dfrac{1}{n^{1/n}}}$

  \item ${\displaystyle \sum_{n=2}^{\infty} (-1)^n\dfrac{1}{\ln{n}}}$

  \item ${\displaystyle \sum_{n=1}^{\infty} (-1)^n\dfrac{n!}{n^n}}$

  \item ${\displaystyle \sum_{n=1}^{\infty} (-1)^n\dfrac{n!}{1\cdot 3\cdot 5\cdots (2n-1)}}$


\end{enumerate}
\item For each of the sums in the preceding problem that converges, use
...
\item Find a value for $n$ so that the $n$th degree Taylor series for
$e^x$ gives at least 10 place accuracy for all $x$ in the interval
$[-3, 0]$.


\item We defined the harmonic series as the infinite sum
\[
1 + \dfrac{1}{2} + \dfrac{1}{3} + \dfrac{1}{4} + \cdots 
   = \sum_{i=1}^\infty \dfrac{1}{i} .
\]
  ...
  \begin{enumerate}
  \item Use a calculator to find the partial sums
\[
S_n = 1 + \dfrac{1}{2} + \dfrac{1}{3} + \dfrac{1}{4} + \cdots \dfrac{1}{n}
\]
for  $n = 1$, 2, 3, \ldots , 12.

  \item Use the following program to find the value of $S_n$ for $n =
...
\end{verbatim}
  ...
  \item Group the terms in the harmonic series as indicated by the parentheses:
...
Explain why each parenthetical grouping totals at least $1/2$.
  ...
  \item Following the pattern in part (c), if you add up the terms of the
...
explain why $S_n$ exceeds $1+k\cdot \tfrac{1}{2}$.
  ...
  \item Use part (d) to explain why the harmonic series {\em diverges}.

  \item You might try this problem if you've studied physics---enough to
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/cards.pdf} 
...
\end{enumerate}
\item {\bf An estimate for the partial sums of the harmonic
...
\]
  ...
  \begin{enumerate}
  \item Let's look at the case $n=6$ to see what's going on.  Consider
the following picture:

\noindent \includegraphics[width=\linewidth]{../ps/calc2/harmonic.pdf}
...
\]
  ...
  \item Show more generally that \quad $\dfrac{1}{n}<S_n-\ln(n)<1$.

  \item Use part (b) to get upper and lower bounds for the value of $S_{10000}$.

\ans{$9.21044 < S_{10000} < 10.21035$.}


  \item Use the result of part (b) to get an estimate for how many cards
...
[[NOTE:Remarkably, partial sums of the harmonic series exceed $\ln(n)$
...
where $\gamma = .5772\ldots$ is called {\bf Euler's
  constant.} (You have seen another constant
...
\end{enumerate}
\item Show that the power series for $\arctan{x}$ and $(1+x)^c$ diverge
...
\item Find the radius of convergence of each of the following power series.
  ...
  \begin{enumerate}
  \item $1 + 2 x + 3 x^2 + 4 x^3 + \cdots$.

  \item $x + 2 x^2 + 3 x^3 + 4 x^4 + \cdots$.

  \item $1 + \dfrac{1}{1^2} x + \dfrac{1}{2^2} x^2 + \dfrac{1}{3^2} x^3 +
     \dfrac{1}{4^2} x^4 + \cdots$. \hfill [Answer: $R = 1$]


  \item $x^3 + x^6 + x^9 + x^{12} + \cdots$.

  \item $1 + (x + 1) + (x+1)^2 + (x+1)^3 + \cdots$.

  \item $17 + \dfrac{1}{3} x + \dfrac{1}{3^2} x^2 + \dfrac{1}{3^3} x^3 +
\dfrac{1}{3^4} x^4 + \cdots$.



\end{enumerate}
\item Write out the first five terms of each of the following power
...
\hfill [Answer: $R = 1$]
  ...
  \item ${\displaystyle \sum_{n = 0}^{\infty} \dfrac{n^2}{2^n} x^n}$. \hfill
     [Answer: $R = 2$]


  \item ${\displaystyle \sum_{n = 0}^{\infty} (n+5)^2\, x^n}$. \hfill
     [Answer: $R = 1$]
...
\end{enumerate}
\item Find the radius of convergence of the Taylor series for $\sin{x}$
and for $\cos{x}$.  For which values of $x$ can these series therefore
represent the sine and cosine functions?


\item Find the radius of convergence of the Taylor series for $f(x) =
...
\item In the text we used the alternating series for $e^x$, $\; x < 0$,
...
this is true.
  ...
  \begin{enumerate}
  \item Consider first the more general situation where two functions are
reciprocals, $g(x) = 1/f(x)$.  Express $g'(x)$ in terms of $f(x)$ and
$f'(x)$.

  \item Use your answer in part a) to find an expression for the {\em
  relative error} in $g$, $\Delta g/g(x) \approx g'(x) \Delta x/g(x)$,
in terms of $f(x)$ and $f'(x)$.  How does this compare to the relative
error in $f$?

  \item Apply your results in part b) to the functions $e^x$ and $e^{-x}$
...
\label{sec:approximation-over-intervals}
...
\label{subsec:approximation-by-polynomials}

\subsubsection{Example.}
Let's return to the problem introduced at the
beginning of this chapter: find the second degree polynomial which
best fits the function $\sin(x)$ over the interval $[0, \pi]$.
[[NOTE:Two possible criteria for best fit]]
...
criteria very attractive, they turn out to be largely [[NOTE:Why we
...
[[NOTE:The criterion  we shall use]]
...
[[NOTE:A mathematical formulation of the problem]]
...
We now want 
        [[NOTE:Setting the partials equal to zero gives equations for $a_0, a_1, a_2$]]
...
All of these integrals 
        [[NOTE:Evaluating the integrals gives three linear equations]]
...
The analysis 
[[NOTE:How to find least squares polynomial approximations in general]]
...
[[NOTE:Solving the equations is a job for the computer]]
...
\subsubsection{Example.}
Let's find polynomial approximation for
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/leastsq1.pdf}
...
\subsubsection{Example.}
We can even use this new technique to find 
[[NOTE:The technique works  even when differentiability fails]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/leastsq2.pdf}
...
[[NOTE:The technique is useful for data functions]]
...
[[NOTE:The importance  of using the  right-sized units]]
...
[[NOTE:Using Simpson's rule helps reduce errors]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc2/sireuler.pdf}
...
Numerical least-squares fitting of the sort
...
\begin{enumerate}
\item To find polynomial approximations for $\sin(x)$ over the interval
...
\enlargethispage*{20pt}
  ...
  \begin{enumerate}
  \item Evaluate $I_0$ and $I_1$.  Suggestion: use integration by parts
(Chapter~11.3) to evaluate~$I_1$.

\ans{$I_0=2$, and $I_1=\pi$.}


  \item Use integration by parts twice to prove the general {\bf
...
\]
  ...
  \item Evaluate $I_2$, $I_3$, and $I_4$.

\ans{$I_2=\pi^2-4$, $I_3=\pi^3-6\pi$, and $I_4=\pi^4-12\pi^2+48$.}



  \item If you have access to a computer package that will solve a system
...
\end{enumerate}
\item To find polynomial approximations for $|x|$ over the interval
...
\]
  ...
  \item Derive the quadratic least squares approximation to $|x|$ over
$[-1,1]$.
...
\end{enumerate}
\item To find polynomial approximations for $1/(1+x^2)$ over the
...
Call this integral $I_n$.
  ...
  \begin{enumerate}
  \item Evaluate $I_0$ and $I_1$.

\ans{$I_0 = \arctan(1)=\pi/4$, and $I_1=(\ln{2})/2=.3465736$.}


  \item Prove the general reduction formula:
\[
...
\end{enumerate}
\item Set up the equations (including evaluating all the integrals) for
finding the best fitting 6-th degree polynomial approximation to
$\sin(x)$ over the interval $[-\pi, \pi]$.


\item In the $S$-$I$-$R$ model, find the best fitting 8-th degree
