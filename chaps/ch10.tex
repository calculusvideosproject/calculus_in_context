...
\footnote{Ordinary arithmetic  lies at the heart  of all calculations}
...
\includegraphics{../ps/calc2/sin1.pdf}
...
\includegraphics{../ps/calc2/sin2.pdf}
...
\footnote{There's more than one way to make the "best fit" to a given curve}
...
\footnote{The behavior of a function can often be inferred from the
...
\footnote{We want the  best fit at $x=0$}
it
...
\footnote{The best fit should pass through the point $(0, f(0))$}
...
\end{center}
But we're 
\footnote{The best fit should have the right slope at $(0, f(0))$}
...
\footnote{The final criterion  for best fit at $x=0$}
...
\footnote{Notation for  higher derivatives}
...
well!
With this notation 
\footnote{The desired rule for finding the coefficients}
...
above to get a
\footnote{General rule for  the Taylor polynomial  at $x=0$}
...
\includegraphics{../ps/calc2/sin3.pdf}\index{Taylor polynomial!graphs for $\sin{x}$}
While each polynomial eventually wanders off
\footnote{The higher the degree of the polynomial,  the better the fit}
...
\footnote{Approximating polynomials for  other basic functions}
...
\footnote{General rule for  the Taylor polynomial  at $x=a$}
...
\footnote{Graph the difference between a function and its Taylor polynomial}
...
\includegraphics{../ps/calc2/j101.pdf}
This graph looks very much like a cubic polynomial.  
\footnote{The difference looks like a power of $x$} 
...
0$.
\includegraphics{../ps/calc2/j102.pdf}
\includegraphics{../ps/calc2/j103.pdf}
If $\sin(x) - P(X)$ were {\em exactly\/} a multiple of $x^5$, 
\footnote{Finding the multiplier}
...
\footnote{Comparing  two numbers}     
...
\footnote{What the fit means computationally}        
...
\footnote{A formula for  the remainder}       
...
\footnote{Another formula for the remainder}   
...
\footnote{Determining $C$  from $f$ at $x = a$}       
...
{\bf Example}.  Consider $\sqrt{x}$ near $x = 100$.  The second degree Taylor polynomial for $\sqrt{x}$, centered at $x = 100$, is
        ...
        $$
\includegraphics{../ps/calc2/j104.pdf}
\includegraphics{../ps/calc2/j105.pdf}
Plot $y = Q(x)$ and $y = \sqrt{x}$ together; 
\footnote{Version 1:  the remainder is $O((x-100)^3)$}
...
\footnote{Version 2:  determining $C$ in terms of $\sqrt{x}$ at $x = 100$}    
...
\footnote{Accuracy of  the polynomial approximation}
...
\footnote{Version 3:   an explicit   error bound}  
...
\footnote{Now you can do anything your calculator can!}
...
\footnote{The error function} defined by
\[
...
Thus,
        ...
        $$
giving us, finally, 
\footnote{A formula for approximating the  error function}
...
\footnote{Extending the  `big oh' notation}    
...
\footnote{Infinite degree polynomials are to be viewed like infinite decimals}
...
positive~$x$:
\includegraphics{../ps/calc2/Darct.pdf}\index{Taylor polynomial!graphs for $1/(1+x^2)$}
It appears that the 
\footnote{A Taylor series  may not converge  for all values of $x$}
...
convergence.
\footnote{Brook Taylor (1685--1731)\index{Taylor, Brook} was
...
series solution.
\footnote{Friedrich Wilhelm Bessel (1784--1846)\index{Bessel, Wilhelm
...
\addtolength{\arraycolsep}{3pt}%
In general, 
\footnote{Finding the  coefficient of $x^n$}
...
\footnote{The graph of the \mbox{Bessel function $J_0$}}
polynomial approximations of degree 2, 4, 6, \ldots , 30 over the
interval $[0,14]$:
\includegraphics{../ps/calc2/Bessel.pdf}\index{Bessel function!$J_0$, graph of}
...
\end{align*}
As before, 
\footnote{Finding the coefficients of the power series  for $S(t)$}
...
\footnote{Finding the  power series  for $I(t)$}
...
\footnote{Extending SIRSERIES to graph $S$ and $I$}
...
\includegraphics{../ps/calc2/sircalc.pdf}
...
\includegraphics{../ps/calc2/sircalc2.pdf}
...
It is,
\footnote{Convergence means  essentially that  ``decimals stabilize"}
...
\end{center}
In other words, 
\footnote{What it means for  an infinite sum  to converge}
...
\footnote{A necessary condition for convergence}
...
and does \footnote{{\em Necessary} and {\em sufficient} mean different
...
\footnote{Detecting  divergent series}
...
\]
will converge.  
\footnote{An important counterexample}
...
\footnote{To avoid divergence, $|x|$ must be less than 1}
...
\footnote{A simple expression for the partial sum $S_n$}
...
\footnote{Finding the limit of $S_n$ as $n \to \infty$}
...
is 2.
Similarly, 
\footnote{Summing another geometric series}
...
To summarize, we have thus proved that
        \footnote{Convergence and divergence of the geometric series}
\begin{center}
...
\footnote{Many common series are alternating, at least for some values of $x$}
...
\end{center}
It is this property 
\footnote{Alternating series  are easy to test  for convergence}
...
\includegraphics{../ps/calc2/altseries.pdf}
...
\footnote{The partial sums oscillate, with the exact sum trapped between
...
\footnote{A simple estimate  for the accuracy  of the partial sums}
...
\footnote{Estimating the error in approximating $\cos(.7)$}
...
\footnote{How many terms  are needed  to obtain accuracy  to 12 decimal places?}
...
\footnote{It may be possible  to convert  a given problem  to one involving 
  alternating series}
...
automatically
\footnote{The answer to the convergence question}
...
\footnote{Radius of convergence of the geometric series}
...
\footnote{Convergence is determined by the ``tail" of the series}
...
\end{center}
Let's see 
\footnote{The ratio test for the geometric series $\ldots$}
...
\footnote{Finding the limit of $|b_{m+1}|/|b_m|$ may require some algebra}
...
\includegraphics{../ps/calc2/cards.pdf} 
\begin{minipage}{3in}
...
\includegraphics{../ps/calc2/harmonic.pdf}
...
\ans{It would take approximately $10^{87}$ cards---a number which is
  ...
  universe!}
\footnote{Remarkably, partial sums of the harmonic series exceed $\ln(n)$
...
\footnote{Two possible criteria for best fit}
...
criteria very attractive, they turn out to be largely \footnote{Why we
...
\footnote{The criterion  we shall use}
...
\footnote{A mathematical formulation of the problem}
\begin{center}
\framebox[4.8in]
{\parbox{4.45in}{
...
\end{align*}
We now want 
        \footnote{Setting the partials equal to zero gives equations for $a_0, a_1, a_2$}
...
All of these integrals 
        \footnote{Evaluating the integrals gives three linear equations}
...
The analysis 
\footnote{How to find least squares polynomial approximations in general}
...
\footnote{Solving the equations is a job for the computer}
...
details):
\includegraphics{../ps/calc2/leastsq1.pdf}
...
\footnote{The technique works  even when differentiability fails}
...
\includegraphics{../ps/calc2/leastsq2.pdf}
...
\footnote{The technique is useful for data functions}
...
\footnote{The importance  of using the  right-sized units}
...
\footnote{Using Simpson's rule helps reduce errors}
...
(decadays), we get
\includegraphics{../ps/calc2/sireuler.pdf}
