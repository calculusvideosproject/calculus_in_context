...
\label{sec:modelling-with-differential-equations}
\index{model}
...
[[NOTE:Models can  provide successive approximations  to reality]]\index{successive approximation}
...
\label{subsec:single-species-models-rabbits}
\index{population model!single-species}


\subsubsection{The problem.}
If we turn 2000 rabbits\index{rabbits} loose on a
...
{\bf The first model}.  Let's assume that, at any time $t$,
[[NOTE:Constant  per capita growth]]
...
[[NOTE:Use Euler's method  to find $R(t)$]]\index{Euler's method}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/pic178.pdf}
...
[[NOTE:It is a good idea to think qualitatively about the functions determined by a differential equation and make some rough estimates before doing extensive calculations.  Your sketches may help you see ways in which the model doesn't correspond to reality.  Or, you may be able to catch errors in your computations if they differ noticeably from what your estimates led you to expect.]]


\noindent\label{logistic-start}
\subsubsection{The second model.}
One way out of the problem of unlimited growth is to modify equation (1) to take into account the fact that any given ecological system 
	[[NOTE:The carrying capacity of the environment]]
...
Check to see that this equation really does have the behavior claimed for it---namely, that a population of 25,000 rabbits neither grows or declines.  Notice also that $R'$ is positive as long as $R$ is less than 25000, so the population increases.  However, as $R$ approaches 25000, 
	[[NOTE:The graph of $R(t)$ levels off near $R = 25000$]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/pic180.pdf}
...
\label{subsec:two-species-models-rabbits-and-foxes}
\index{population model!two-species}

	No species lives alone in an environment, 
	[[NOTE:Introduce predators]]\index{predator}
...
\end{align*}
These are 
[[NOTE:Lotka--Volterra equations with bounded growth]] 
...
{\bf An example}.  To see what kind of predictions the Lotka--Volterra equations make, we'll work through an example with specific values for the parameters.  Let 
	\[
\begin{array}{rcrl}
...
(Check that these five parameters have the right units.)  These choices give us the specific differential equations
	...
	[[NOTE:The graphs   of $R$ and $F$]]
Then the two populations will vary in the following way over the next 250 months.

\noindent \includegraphics[width=\linewidth]{../ps/calc1/rf1.pdf}
	...
	[[NOTE:How rabbits respond to changes in the initial fox population]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/rf2.pdf}
...
\label{subsubsec:single-species-models}



\begin{enumerate}
\item {\bf Constant per capita Growth}.  This question considers the initial value problem given in the text:
	\[
...
\]
  ...
  \begin{enumerate}
  \item Use Euler's method to determine how many rabbits there are after 6 months.  Present a table of successive approximations from which you can read the exact value to whole-number accuracy.

  \item Determine, to whole-number accuracy, how many rabbits there are after 24 months.

  \item How many months does it take for the rabbit population to reach 25,000?


\end{enumerate}
\item {\bf Logistic Growth}.\index{logistic growth}  The following questions concern  a rabbit population described by the logistic model
	\[
...
\]
  ...
  \item Sketch the functions determined by the logistic equation if you start with either 2000 or 40000 rabbits.  (Suggestion: you can modify the program SIRPLOT to answer this question.)  Compare the two functions.  How do they differ?  In what ways are they similar?


\end{enumerate}
\item {\bf Seasonal Factors} Living conditions for most wild
...
explores ways of modifying the logistic model to reflect these facts.
  ...
  \begin{enumerate}
  \item For the eastern cottontail rabbit, most young are born during the
...
convenient way to incorporate the varying reproductive rate.
...
\end{enumerate}
\item {\bf World population}.\index{population!world} The world's
...
with a per capita growth rate of 20/1000 = 0.02.
  ...
  \begin{enumerate}
  \item Write a differential equation for $P$ that expresses this assumption.  Use $P$ to denote the world population, measured in billions.

  \item According to the differential equation in (a), at what rate (in billions of persons per year) was the world population growing in 1990?

  \item By applying Euler's method to this model, using the initial value of 5 billion in 1990, estimate the world population in the years 1980, 2000, 2040, and 2230.  Present a table of successive approximations that stabilizes with one decimal place of accuracy (in billions).  What step size did you have to use to obtain this accuracy?


\end{enumerate}
\item {\bf Supergrowth}.\index{supergrowth}  Another model for the world population, one that actually seems to fit recent population data fairly well, assumes ``supergrowth''---the rate $P'$ is proportional to a {\em higher power} of $P$, rather than to $P$ itself.  The model is
	\[
P'=.015 \, P^{1.2}.
\]
As in the previous exercise, assume that $P$ is measured in billions, and the population in 1990 was about 5 billion.
  ...
  \begin{enumerate}
  \item According to this model, at what rate (in billions of persons per year) was the population growing in 1990?

  \item Using Euler's method, estimate the world population in the years 1980, 2000 and 2040.  Use successive approximations until you have one decimal place of accuracy (in billions).  What step size did you have to use to obtain this accuracy?

  \item Use an Euler approximation with a step size of 0.1 to estimate the world population in the year 2230.  What happens if you repeat your calculation with a step size of 0.01?  [Comment:  Something strange is going on here.  We will look again at this model in the next section.]


\end{enumerate}
\end{enumerate}
\subsubsection{Two-species models}
\label{subsubsec:two-species-models}
...
\begin{enumerate}
\item {\bf The May Model}.\label{May model} This model has been
...
[[NOTE:While a term like ``hectorabbits'' is deliberately whimsical,
...
\item The fox population is governed by the logistic equation, and the
 ...
 \end{itemize}
  ...
  \begin{enumerate}
  \item Explain why $D(x) =\dfrac{c\,x}{x+d}$ ($d$ some constant) might
...
reflected by making $d$ smaller or larger?
  ...
  \item Explain how the following system of equations incorporates May's assumptions.
\begin{align*}
...
the prey to be hares and the predators to be lynx.\index{lynx}
  ...
  \begin{enumerate}
  \item Explain why the following system of equations incorporates the assumptions of the basic model.  (The parameters $a$, $b$, $c$,  and  $d$ are all positive.)
\begin{align*}
 H' & =  a\,H  -  b\,HL  \\
 L' & =  c\,HL - d\,L
\end{align*}
...
by the mathematical ecologist and demographer Alfred James Lotka\index{Lotka, A.} a few years earlier.  Though simplistic, they form one of the principal starting points in ecological modeling.)
  ...
  \item Explain why $a$ and $b$ have the units hares per month per hare and hares per month per hare-lynx, respectively.  What are the units of $c$ and $d$?  Explain why.
...
\begin{align*}
 ...
 \end{align*}
  ...
  \item Suppose that you start with 2000 hares and 10
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/hare1.pdf}
\begin{center}
...
You should get graphs like those above. Notice that the hare and lynx populations rise and fall in a fashion similar to the rabbits and foxes, but here they oscillate---returning {\em periodically\/} to their original values.
  ...
  \item What happens if you keep the same initial hare population of
2000, but use different initial lynx populations?  Try $L(0) = 20$ and
$L(0) = 50$.  (In each case, use a step size of .1 month.)


  \item Start with 2000 hares and 10 lynx.  From part (c), you know the
...
lynx populations have the same periods?
  ...
  \item Plot the hare populations over time when you start with 2000 hares and, successively, 10, 20, and 50 lynx.  Is the hare population periodic in each case?  What is the period?  Does it vary with the size of the initial lynx population?


\end{enumerate}
\end{enumerate}
\subsubsection{Fermentation}\index{fermentation}
\label{subsubsec:fermentation}
...
\begin{enumerate}
\item In our example, the temperature of the room is
...
where $k$ is some positive constant.
  ...
  \begin{enumerate}
  \item Why is there a minus sign in the equation?
...
\]
  ...
  \item \label{ex:coffee1} Use Euler's method to determine the temperature
...
stabilize to the second decimal place.
  ...
  \item \label{ex:coffee2} How long does it take for the temperature $Q$
to drop to \mbox{$30^{\circ}$C}?  Use a DO-WHILE loop\index{loop!DO-WHILE} to construct a table of successive approximations that
stabilize to the second decimal place.


\end{enumerate}
\item On a hot day, a cold drink warms up at a rate approximately
...
where $k_1$ and $k_2$ are positive constants.
  ...
  \begin{enumerate}
  \item Explain the signs that appear in these differential equations.

  \item Suppose  $k_1 = .3$ and $k_2 = .1$.  If $Q(0) = \mbox{$90^{\circ}$C}$  and  $A(0) = \mbox{$20^{\circ}$C}$, when will the temperature of the coffee be \mbox{$40^{\circ}$C}?  What is the temperature of the air at this time?  Your answers should be accurate to one decimal place.

  \item What does the temperature of the coffee become eventually?  How long does it take to reach that temperature?


\end{enumerate}
\end{enumerate}
\subsubsection{$S$-$I$-$R$ revisited}
\label{subsubsec:s-i-r-revisited}
...
\begin{enumerate}
\item How many susceptibles are left after 40 days?  When is the
...
\item What happens as the epidemic ``runs its course''?  That is, as
more and more time goes by, what happens to the numbers of infecteds
and susceptibles?


\item One of the principal uses of a mathematical model is to get a
...
\item Draw the graphs of $I(t)$ for the same initial conditions as in
...
\label{subsubsec:constructing-models}
\index{model}
...
\begin{enumerate}
\item We deposit a fixed sum of money in a bank, and we'd like to know
how much will be there in ten years.


\item We know the diameter of the mold spot growing on a cheese
sandwich is 1/4 inch, and we'd like to know when its diameter will be
one inch.


\item We know the fecal bacterial and coliform concentrations in a
...
\item We know what the temperature and rainfall is today, and we'd like
to know what both will be one week from today.


\item We know what the winning lottery number was yesterday, and we'd
like to know what the winning number will be the day after tomorrow.


\item We know where the earth, sun, and moon are in relation to each
...
\label{subsec:differential-equations-are-equations}

Until now, we have viewed a system of differential equations
[[NOTE:Differential equations give instructions for Euler's method]]
...
[[NOTE:Equations have {\em solutions}]]
...
[[NOTE:Substitute $y = \sqrt{t}$  into the differential equation]]
equation:
	\[
...
Since $2t$ and $1/2t^2$ are different functions, the two sides are unequal and the equation is therefore false.  
	...
	[[NOTE:A {\em solution\/} makes  the equation true]]
...
the two sides are equal---no matter what value $C$ happens to have.  
	[[NOTE:A differential equation can have infinitely many solutions]]
...
[[NOTE:An initial value problem has  only one solution]]
...
\subsubsection{Checking solutions versus finding solutions.}
Notice that we
...
[[NOTE:There are special methods to solve particular equations, but other methods work generally]]
...
provide solutions.
[[NOTE:Most initial value problem have one,  and only one, solution.  Euler's method  will find it.]]
...
% --- converted boxed semantic block to observation ---
\begin{observation}
...
\label{subsec:world-population-growth}

\subsubsection{Two models}
\label{subsubsec:two-models}
...
years, with $t=0$ in 1990.  In this notation, [[NOTE:Models for world population growth]] the two models are
	\[
\begin{array}{rll}
...
\label{subsubsec:a-formula-for-the-supergrowth-model}
\index{formula}
...
\]
First of all, 
	[[NOTE:Checking the  initial condition]]
...
\]
	...
	To check that it 
	[[NOTE:Checking the differential equation]]
...
[[NOTE:Notice two things about this result.  First, when we work with
...
\label{subsubsec:behavior-of-the-supergrowth-solution}
...
Consider what happens, though, as $t$ approaches this special value 241.6\ldots .  The 
	[[NOTE:$Q$ ``blows up'' as $t \to 241.6\ldots$]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/pic204a.pdf}


The range of values of $Q$ for $0 \leq t \leq 240$ is so immense that these graphs are useless.  In a case like this, it is helpful to rescale the vertical axis so that the space between one power of 10 and the next is the same.  In other words, instead of seeing 1, 2, 3, \ldots , we see $10^1$, $10^2$, $10^3$, \ldots .  This is called a {\bf logarithmic} scale.\index{logarithmic scale}  Here's what happens to the graphs if we put a logarithmic scale on the vertical axis: 



\mbox{}

\noindent \includegraphics[width=\linewidth]{../ps/calc1/pic204b.pdf}
...
The second graph makes it clearer that the Euler approximations do indeed approach the graph of the function given by our formula, but they approach more and more slowly, the closer $t$ approaches 241.6\ldots .

[[NOTE:Graphs are made with logarithmic scales particularly when the
  ...
  when both axes are logarithmic, the result is called a {\em
	...
	Since $Q(t)$ becomes infinite when $t = 241.6\ldots$ , we must conclude that the solution to the original initial value problem 
	[[NOTE:The initial value problem has a solution only for $t < 241.6\ldots$]]
is meaningful only for $t < 241.6\ldots$ .  Of course, the formula for $Q$ works quite well when $t > 241.6\ldots$ .  It just has no meaning as the size of a population.  For instance, when $t = 260$ we get
	\[
...
In other words, the function determined by the initial value problem is defined only on intervals around $t=0$ that do not contain $t = 241.6\ldots$~.
	...
	\[
Q'(t) = .015 \left( \frac{1}{1/\sqrt[5]{5}-.003t}
		\right)^6
\]
...
Since $Q'$ has the same denominator as $Q$, 
	[[NOTE:$Q'$ blows up as $t \to 241.6\ldots$]]
it becomes infinite the same way $Q$ does: $Q'(t) \to \infty$ as $t \to 241.6\ldots$ .  Because Euler's method uses the microscope equation $\Delta Q \approx Q' \cdot \Delta t$ to predict the next value of $Q$, we can now understand why the estimates of $Q(240)$ were so slow to stabilize: as $Q' \to \infty$, $\Delta Q \to \infty$, too.


\subsubsection{A formula for constant per capita growth}
\label{subsubsec:a-formula-for-constant-per-capita-growth}

	The constant per capita growth model for the world population that we are considering is 
	\[
\dfrac{dP}{dt} = .02\, P \qquad P(0) = 5.
\]
This differential equation has a very simple form; if $P(t)$ is a solution, then the derivative of $P$ is just a multiple of $P$.  We have already seen in chapter~3 that {\em exponential\/} functions\index{function!exponential} behave this way (exercises 5--7 in section~3).  For example, if $P(t) = 2^t$, then
	\[
\dfrac{dP}{dt} = .69 \cdot 2^t = .69 \, P.
\]
Of course, the multiplier that appears here is .69, not .02, so $P(t) = 2^t$ is not a solution to our problem.

	However, the multiplier that appears 
	[[NOTE:Exponential functions satisfy $dy/dt = ky$]]
when we differentiate an exponential function changes when we change the base.  That is, if $P(t) = b^t$, then $P'(t) = k_b \cdot b^t$, where $k_b$ depends on $b$.  Here is a sample of values of $k_b$ for different bases $b$:
...
In other words, {\em $k_b$ is the slope of the graph of $P(t) = b^t$ at the origin}.
	...
	[[NOTE:The correct exponential function has slope .02 at the origin]]
whose graph has slope .02 at the origin.  This is a problem that we can solve with a computer microscope.  Pick a value of $b$ and graph $b^t$.  Zoom in on the graph at the origin and measure the slope.  If the slope is more than .02, choose a smaller value for $b$; if the slope is less than .02, choose a larger value for $b$.  Repeat this process, narrowing down the possibilities for $b$ until the slope is as close to .02 as you wish.  Eventually, we get 
	\[
P(t) = (1.0202)^t.
\]
You should check that $P'(0) = .02000\ldots$ ; see the exercises.  

	Thus $P(t) = (1.0202)^t$ solves the differential equation $P' = .02\,P$.  However, it does not satisfy the initial condition, because
	\[
P(0) = (1.0202)^0 = 1 \neq 5.
\]
This is easy to fix; $P(t) = 5 \cdot (1.0202)^t$ satisfies both conditions.  More generally, $P(t) = C \,(1.0202)^t$ satisfies the initial condition $P(0) = C$ as well as the differential equation $P'(t) = .02 \,P(t)$.  To check the initial condition, we compute
	\[
...
So we have verified that the solution to our problem is 
	[[NOTE:The formula   for $P$]]
	\[
P(t) = 5 \, (1.0202)^t.
\]

	Because exponential functions are involved, constant per capita growth is commonly called {\bf exponential growth}.\index{growth!exponential}   In the figure below we compare exponential growth $P(t)$ to ``supergrowth'' $Q(t)$.  The two graphs agree quite well when $t < 50$.  Notice that population is plotted on a logarithmic scale (a {\em semi--log\/} plot).  This makes the graph of $P$ a straight line!  



\mbox{}

\noindent \includegraphics[width=\linewidth]{../ps/calc1/pic207.pdf}
...
\label{subsec:differential-equations-involving-parameters}

\label{DEs w/ params-begin}

The $S$-$I$-$R$ model contained two parameters---the transmission and
recovery coefficients $a$ and $b$.\index{parameter} When we used
...
[[NOTE:How do parameters affect solutions?]]
That made it more difficult to deal with our questions about the
effects of {\em changing\/} the parameters.  As a result, we took
other approaches to explore those questions.  For example, we used
algebra to see that there was a threshold for the spread of the
disease: if there were fewer than $b/a$ people in the susceptible
population, the infection would fade away.
...
Here is the formula for a function that solves this 
	[[NOTE:The supergrowth solution with parameters]]
...
Since both sides equal $k\,u^{-6}$, they equal each other, proving that $Q(t)$ is a solution to the differential equation.
	...
	Next, we ask when the population becomes infinite.  Exactly as before, this will happen when the denominator of the formula for $Q(t)$ becomes 
	[[NOTE:The time to infinity]]
zero:
	\[
...
[[NOTE:Uncertainty in  the size of $\tau$]]
...
chapter~3.4.  For example, suppose we are uncertain of our value of
the growth rate $k$; there may be an error of size $\Delta k$.  How
uncertain does that make us about the calculated value of $\tau$?
Likewise, if the current world population $A$ is known only with an
error of $\Delta A$, how uncertain does {\em that\/} make $\tau$?
Also, how are the {\em relative\/} errors related?  Let's do this
analysis, assuming that $k = .015$ and $A = 5$.

	Our tool is the error propagation equation---which is 
	[[NOTE:How an error in $k$ propagates]]
...
Thus $\Delta \tau \approx -16106 \cdot \Delta k$.  For example, if the uncertainty in the value of $k = .015$ is $\Delta k = \pm .001$, then the uncertainty in $\tau$ is about $\mp 16$ years.
	...
	[[NOTE:How an error in $A$ propagates]]
we first write
	\[
...
This says that it takes a 5\% error in $A$ 
	[[NOTE:How sensitive $\tau$ is to errors in $k$ and $A$]]
...
\label{supergrowth errors-end}
	...
	[[NOTE:Special methods give formulas; general methods are numerical]]
...
\item {\bf Powers of $y$}.
  ...
  \begin{enumerate}
  \item $y'=y^2$, $y(0)=5$: \quad $y(t)= 1/(\frac{1}{5} - t)$


  \item $y'=y^3$, $y(0)=5$:   \quad $y(t) = 1/\sqrt{\frac{1}{25} - 2t}$


  \item $y'=y^4$, $y(0)=5$:  \quad $y(t) =  1/\sqrt[3]{\frac{1}{125} - 3t}$


  \item Write a general formula for the solution of the initial value problem $y'=y^n$, $y(0)=5$, for any integer $n > 1$.

  \item Write a general formula for the solution of the initial value problem $y'=y^n$, $y(0)=C$, for any integer $n > 1$ and any constant $C\geq 0$.
...
\item {\bf Powers of $t$}.
  ...
  \begin{enumerate}
  \item $y'=t^2$, $y(0)=5$:  \quad $y(t) = \frac{1}{3}t^3 + 5$


  \item $y'=t^3$, $y(0)=5$:  \quad $y(t) = \frac{1}{4}t^4 + 5$


  \item $y'=t^4$, $y(0)=5$:  \quad $y(t) = \frac{1}{5}t^5 + 5$


  \item Write a general formula for the solution of the initial value problem $y'=t^n$, $y(0)=5$ for any integer $n > 1$.

  \item Write a general formula for the solution of the initial value problem $y'=t^n$, $y(0)=C$ for any integer $n > 1$ and any constant $C$.



\end{enumerate}
\item {\bf Sines and cosines}.

  \begin{enumerate}
  \item $x'=-y$, $y'=x$, $x(0)=1$, $y(0)=0$: \quad $x(t)=\cos t$, $y(t) = \sin t$


  \item $x'=-y$, $y'=x$, $x(0)=0$, $y(0)=1$: \quad $x(t) = \cos(t+\pi/2)$, $y(t) = \sin(t+\pi/2)$
...
\item {\bf Exponential functions}.
  ...
  \begin{enumerate}
  \item $y'=2.3 \, y$, $y(0)=5$: \quad $y(t)=5 \cdot 10^t$


  \item $y'=2.3 \, y$, $y(0)=C$: \quad $y(t)=C \cdot 10^t$


  \item $y'=-2.3 \, y$, $y(0)=5$: \quad $y(t)=5 \cdot 10^{-t}$


  \item $y'=4.6 \,ty$, $y(0)=5$: \quad $y(t)= 5 \cdot 10^{t^2}$
...
\item {\bf Initial Conditions}.
  ...
  \begin{enumerate}
  \item Choose $C$ so that $y(t) = \sqrt{t+C}$ is a solution to the initial value problem
	\[
y'=\frac{1}{2y} \quad y(3)=17.
\]


  \item Choose $C$ so that $  y(t) = -1/(t+C)$  is a solution to the initial value problem
	\[
y'=y^2 \quad y(0)=-5.
\]
...
\label{subsubsec:world-population-growth-with-parameters}
...
\begin{enumerate}
\item Using a graphing utility or a calculator, show that the derivative of $P(t)=(1.0202)^t$ at the origin is approximately .02: $P'(0) \approx .02$.  Since quick convergence is desirable, use
	\[
...
\]
  ...
  \begin{enumerate}
  \item By using more decimal places to get higher precision, show that $P(t) = (1.0202013)^t$ satisfies $P'(0) = .02$ even more exactly.


\end{enumerate}
\item \label{ex:base 2} Show that the function $y = 2^{t/.69}$ satisfies the differential equation $dy/dt = y$.  Use the chain rule: $y = 2^u$, $u = t/.69$. (Recall that $k_2=.69\ldots\,.$)


  \begin{enumerate}
  \item Show that the function $y = 2^{kt/.69}$ satisfies the differential equation $dy/dt = k\,y$.

  \item Show that the function $P(t) = A \cdot 2^{kt/.69}$ is a solution to the initial value problem
	\[
...
\end{enumerate}
\item Show that the function $y = 10^{t/2.3}$ satisfies the differential equation $dy/dt = y$.  Use the chain rule: $y = 10^u$, $u = t/2.3$. (Recall that $k_{10}=2.3\ldots\,.$)


  \begin{enumerate}
  \item Show that the function $y = 10^{kt/2.3}$ satisfies the differential equation $dy/dt = k\,y$.

  \item Show that the function $P(t) = A \cdot 10^{kt/2.3}$ is a solution to the initial value problem
	\[
\dfrac{dP}{dt} = k\,P \qquad P(0) = A.
\]
...
\end{enumerate}
\item The formula $P(t) = 5\cdot 2^{k\,t/.69}$ describes how an initial population of 5 billion will grow at a constant per capita rate of $k$ persons per year per person.  Use this formula to determine how many years $t$ it will take for the population to double, to 10 billion persons.

  \begin{enumerate}
  \item Suppose the initial population is $A$ billion, instead of 5 billion.  What is the doubling time then?

  \item Suppose the initial population is 5 billion, and the per capita growth rate is .02, but that value is certain only with an error of $\Delta k$.  How much uncertainty is there in the doubling time that you found in part (a)?
...
\begin{enumerate}
\item Show that the function $y = 2^{-.1\,t/.69}$ is a solution to the
...
whose temperature is 20$^{\circ}$C.
  ...
  \begin{enumerate}
  \item Use the formula in part (a) to find the temperature of the coffee
after 20 minutes.  Compare this result with the value you found in
exercise~12~(b), page~\pageref{ex:coffee1}.

  \item Use the formula in part (a) to determine how many minutes it
...
\end{enumerate}
\item A cold drink is initially at $ Q = 36^{\circ}$F when the air
...
the temperature after $t$ minutes.
  ...
  \begin{enumerate}
  \item Use the formula to find the temperature of the drink after 5
...
\end{enumerate}
\item Find a formula for a function that solves the initial value problem
	\[
...
\label{subsubsec:a-leaking-tank}
...
\begin{enumerate}
\item Let $V(t)$ be the volume of water in the tank $t$ hours from now.  Explain why the leakage rate can be written as the differential equation
	\[
V'(t) = -k\sqrt{V(t)},
\]
for some positive constant $k$.  (The issue to deal with is this: why is it permissible to use the square root of the {\em volume\/} here, when the rate is known to depend on the square root of the {\em height\/}?)
  ...
  \begin{enumerate}
  \item Determine the value of $k$.  [Answer: $k \approx .3364$; you need to explain {\em why\/} this is the value.]


\end{enumerate}
\item How much water leaks out of the tank in 12 hours; in 24 hours?  Use Euler's method, and compute successive approximations until your results stabilize.

  \begin{enumerate}
  \item How many hours does it take for the tank to empty?


\end{enumerate}
\item Use the differentiation rules to show that any function of the form
	\[
V(t) = \left\{ 
		\begin{array}{ll}
		\dfrac {{k^2}}{4} (C - t)^2 & 
...
satisfies the differential equation.
  ...
  \begin{enumerate}
  \item For the situation we are considering, what is the value of $C$?  According to this solution, how long does it take for the tank to empty?  Compare this result with your answer using Euler's method.

  \item Sketch the graph of $V(t)$ for $0 \leq t \leq 2C$, taking particular care to display the value $V(0)$ in terms of $k$ and $C$.
\label{leaking tank-end}

\end{enumerate}
\end{enumerate}
\subsubsection{Motion}
\label{subsubsec:motion}
\index{motion}

	Newton created the calculus to study the motion of the planets.  He said that all motion obeys certain basic laws.  One law says that the velocity of an object changes only if a force acts on the body.  Furthermore, the {\em rate\/} at which the velocity changes is proportional to the force.  By knowing the forces that act on a body we can construct---and then solve---a differential equation for the velocity.  
...
acting.  We call the force that pulls objects to the earth {\bf
  gravity}.\index{gravity} Near the earth, the strength of gravity is
...
\begin{enumerate}
\item Verify that $v(t) = -g\,t + v_0$\label{falling body0} is a
solution to the differential equation $dv/dt = -g$ with initial
velocity $v_0$.


\item Since $dx/dt = v$, and since $v(t) = -g\,t + v_0$, the {\em position\/} $x$ of the body satisfies the differential equation
	\[
...
\item Suppose the initial position of the body is $x_0$, so that the position $x$ is a solution to the initial value problem
	\[
...
\item Suppose a body is held motionless 200 meters above the ground, and then released.  What values do $x_0$ and $v_0$ have?  What is the formula for the motion of this body as it falls to the ground?

  \begin{enumerate}
  \item How far has the body fallen in 1 second?  In 2 seconds?

  \item How long does it take for the body to reach the ground?
...
\end{enumerate}
\item \label{ex:falling body1}  Show that
	\[
...
\item \label{ex:falling body2} Show that the position $x(t)$ of a body
...
\]
  ...
  \begin{enumerate}
  \item \label{ex:falling body3} Suppose the coefficient of air resistance
...
was no air resistance.
  ...
  \item How long does it take for the body to reach the ground?  (Use a computer graphing package to get this answer.)  Compare this value with the one you obtained assuming these was no air resistance.  How much does air resistance add to the time?



\end{enumerate}
\item According to the equation $dv/dt = -g - b\,v$, there is a velocity $v_T$ at which the force of air resistance exactly balances the force of gravity, and the velocity doesn't change.  What is $v_T$, expressed as a function of $g$ and $b$.  Note: $v_T$ is called the {\bf terminal velocity}\index{terminal velocity} of the body.  Once the body reaches its terminal velocity, it continues to fall at that velocity.

  \begin{enumerate}
  \item What is the terminal velocity of the body in the previous exercise?
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/spring9.pdf}
...
is a solution to the initial value problem.
  ...
  \begin{enumerate}
  \item What range of values does $x$ take on; that is, how far does the
weight move from its rest position?


\end{enumerate}
\item Use a graphing utility to compare the graphs of
...
the motion $x(t)=a \cos(ct)$ for a fixed value of $a$.
  ...
  \begin{enumerate}
  \item How long does it take the weight to complete one cycle (from $x = a$ back to $x = a$) when $c = 1$?  The motion of the weight is said to be {\bf periodic}, and the time it takes to complete one cycle is called its {\bf period}.\index{period!of an oscillation}


  \item What is the period of the motion when $c = 2$?  When $c = 3$?
Does the period depend on the initial position $a$?

  \item Write a formula that expresses the period of the motion in terms
of the parameters $a$ and $c$.



\end{enumerate}
\item The parameter $c$ depends on two things: the mass $m$ of the
...
Write a formula that expresses the period of the motion of the weight in terms of $m$ and $k$.
  ...
  \begin{enumerate}
  \item Suppose you double the weight on the spring.  Does that {\em
  increase\/} or {\em decrease\/} the period of the motion?  Does your
answer agree with your intuitions?

  \item Suppose you put the first weight on a second spring that is twice
as stiff as the first (i.e., double the value of $k$).  Does that {\em
  increase\/} or {\em decrease\/} the period of the motion?  Does your
answer agree with your intuitions?

  \item When you calculate the period of the motion using your formula
...
\label{sec:the-exponential-function}

\subsection{The Equation $y'= ky$}
\label{subsec:the-equation-y-ky}

	As we have seen, initial value problems {\em define\/} functions---as their solutions.  They therefore provide us with a vast, if somewhat bewildering, array of new functions.  Fortunately, a few differential equations---in fact, the very simplest---arise over and over again in an astonishing variety of contexts.  The functions they define are among the most important in mathematics.

	One of the simplest differential equations is 
	[[NOTE:A simple and natural model of growth and decay]]
$dy/dt =ky$, where $k$ is a constant.  It is also one of the most useful.  We used it in chapter 1 to model the populations of Poland and Afghanistan, as well as bacterial growth and radioactive decay.  In this chapter, it was our initial model of a rabbit population and one of our models of the world population.  Later, we will use it to describe how money accrues interest in a bank and how radiation penetrates solid objects.

In this section we will look at the solutions to differential equations of this form from two different vantage points.  On the one hand, we already have named functions which solve such equations---the exponential functions.  On the other hand, the fact that Euler's method produces the same functions will allow us to prove properties of such functions and  to compute their values effectively.

	In chapter 3 we established that the solutions 
	[[NOTE:Exponential solutions---variable base~\ldots]]
to $dy/dt = ky$ are exponential functions.\index{function!exponential}  Specifically, for each base $b$, the exponential function $y = b^t$ was a solution to $dy/dt = k_b \cdot b^t = k_b \cdot y$, where $k_b$ was the slope of the graph of $y = b^t$ at the origin.  In this approach, if the constant $k$ changes, we must change the base $b$ so that $k_b = k$.


Exercise~7 in the previous section (page~\pageref{ex:base 2}) [[NOTE:\ldots
  and   fixed base]] opened up a new possibility: for the {\em
  fixed\/} base 2, the function
	\[
y = 2^{kt/.6931\ldots}
\]
...
\]
	...
	[[NOTE:Advantages of a fixed base]]
...
\begin{array}{ccc}
	...
	\end{array}
\]
Notice that the growth constant $k$ gets ``swallowed up'' in the exponent of the solution when $k$ has a specific numerical value.  The number that appears in the exponent is $k$ divided by $k_2 = .6931\ldots$ (when the base is 2) and by $k_{10} = 2.3025\ldots$ (when the base is 10).

	The most vivid solution to $dy/dt = ky$ would use the base $b$ for which $k_b = 1$ {\em exactly}.  
	[[NOTE:The base $e$]]
There is such a base, and it is always denoted $e$\index{$e$}.  (We will determine the value of $e$ in a moment.)  Since $k_e = 1$, $k$ would stand out in the exponent:
	\[
\begin{array}{cc}
...
[[NOTE:The use of the symbol $e$ to denote the base dates back to a
  paper that Euler wrote at age 21, entitled {\em
  ...
  ubiquitous numbers in mathematics.]]

	By design, $y = e^t$ is a solution to the differential equation $dy/dt = y$.  In particular, the slope of the graph of $y = e^t$ at the origin is exactly~1.  As we have just seen, the function $y = e^{kt}$ is a solution to the differential equations $dy/dt = ky$ whose growth constant is $k$.  
	[[NOTE:The general initial value problem for exponential functions]]
Finally: 
\begin{center}
...
\mbox{}\quad $y = C \cdot e^{kt}$ is the solution to the initial value problem
	...
	$\displaystyle \dfrac{dy}{dt} = ky \qquad y(0) = C.$
}}}
\end{picture}
...
\label{subsec:the-number-e}

	The number $e$ is determined by the property that $k_e = 1$.  
[[NOTE:Finding $e$ with a computer microscope]]
...
[[NOTE:Under successive magnifications: $e = 2.71828\ldots$]]
...
[[NOTE:Finding $e$ by  Euler's method]]
...
\label{subsec:differential-equations-define-functions}
...
[[NOTE:Euler's method uses only arithmetic]]
...
[[NOTE:Defining a function is not the same as being able to evaluate it]]
...
Returning to exponents, think what would be involved in evaluating $2^{\sqrt{3}}$ using the pre-calculus concept of exponents.  We might first get a series of rational approximations to $\sqrt{3}=1.73205081\ldots$: $17/10=1.7$, $173/100=1.73$, $433/250=1.732$, and so on.  We would then calculate 
...
We will assume {\em nothing\/} about the function $y = e^t$.  Instead,
we begin simply with the observation that each initial value problem
defines a function---its solution.  Therefore, the specific problem
	\[
y' = y \qquad y(0) = 1
\]
...
The proof uses the Existence and Uniqueness Principle for differential equations we articulated  	[[NOTE:The proof of Theorem~1]]
...
If we show this, it will then follow that $P(t)$ and $Q(t)$ must be the same function.

Since
	\[
\begin{array}{l}
	P(0) = \exp(r + 0) = \exp(r) \\
	Q(0) = \exp(r) \cdot \exp(0) = \exp(r) \cdot 1 = 
		\exp(r),
	\end{array}
\]
...
\]
so $Q(t)$ is a solution.

To differentiate $P(t)$ we construct a chain:
	\[
...
\]
so $P(t)$ is also a solution.

Therefore $P(t)$ and $Q(t)$ must be the same function.  It follows then that $P(t)=Q(t)$ for all values of $t$, in particular for $t=s$.  But this means that
	\[
...
which is exactly the statement of Theorem 1, and so completes the proof.
	...
	Now that we have established $\exp(x) = e^x$, we will call $\exp(x)$ {\bf the exponential function} and we will use the forms $e^x$ and $\exp(x)$ interchangeably.  The following theorem summarizes several more properties of the exponential function.\index{function!exponential}
...
{\bf Theorem 3}.  {\em For any real numbers\/ $r$ and\/ $s$}, 
	...
	To make the statements in this theorem seem more natural, you should stop and translate them from $\exp(x)$ to $e^x$.  Proofs will be covered in the exercises.  


\subsection{Exponential Growth}
\label{subsec:exponential-growth}
\index{growth!exponential}

	The function $\exp(x) = e^x$, like polynomials and the sine and cosine functions, is defined for all real numbers.  Nevertheless, it behaves in a way that is quite different from any of those functions.  

	One difference occurs when $x$ is large, either positive or negative.  The sine function and the cosine function stay bounded between $+1$ and $-1$ over their entire domain.  By contrast, every polynomial ``blows up'' as $x \to \pm \infty$.  In this regard, the exponential function is a hybrid.  As $x \to -\infty$, $\exp(x) \to 0$.  As $x \to + \infty$, however, $\exp(x) \to +\infty$.  



	Let's look more closely at what happens 
	[[NOTE:How fast do $x^n$ and $e^x$ become infinite?]]
to power functions $x^n$ and the exponential function $e^x$ as $x \to \infty$.  Both kinds of functions ``blow up'' but they do so at quite different rates, as we shall see.  Before we compare power and exponential functions directly, let's compare one power of $x$ with another---say $x^2$ with $x^5$.  As $x \to \infty$, both $x^2$ and $x^5$ get very large.  However, $x^2$ is only a small fraction of the size of $x^5$, and that fraction gets smaller, the larger $x$ is.  The following table  demonstrates this.  Even though $x^2$ grows enormous, we interpret the fact that $x^2/x^5 \to 0$ to mean that $x^2$ {\em grows more slowly than\/} $x^5$. 
...
[[NOTE:$x^p$ grows more slowly than $x^q$ if $p < q$]]
...
than any lower power of $x$.  The table above compares $x^{50}$ to
...
\]
	...
	So $x^{50}$ grows more slowly than $e^x$, and so does any lower power of $x$.  Perhaps a higher power of $x$ would do better.  
	[[NOTE:$e^x$ grows more rapidly than any power of $x$]]
It does, but ultimately the ratio $x^p/e^x \to 0$, no matter how large the power $p$ is.  We don't yet have all the tools needed to prove this, but we will after we introduce the logarithm function in the next section.  

	The speed of exponential growth has had an impact in computer science.  In many cases, the number of operations needed to calculate a particular quantity is a power of the number of digits of precision required in the answer.  Sometimes, though, the number of operations is an {\em exponential\/} function of the number of digits.  When that happens, the number of operations can quickly exceed the capacity of the computer.  In this way, some problems that can be solved by an algorithm that is straightforward in theoretical terms are intractable in practical terms.  
...
\label{subsubsec:the-exponential-functions-bt}
\index{function!exponential}



\begin{enumerate}
\item Use a graphing utility or a calculator to approximate the slopes of the following functions at the origin and show:


  \begin{enumerate}
  \item If  $f(t) = (2.71)^t$, then $f'(0)<1$.

  \item If  $g(t) = (2.72)^t$, then $g'(0)>1$.

  \item Use parts (a) and (b) to explain why $2.71 < e < 2.72$.



\end{enumerate}
\item In the same way find the value of the parameter $k_b$ for the bases $b =$ .5, .75, and .9 accurate to 3 decimal places.

  \begin{enumerate}
  \item What is the shape of the graph of $y=b^t$ when $0 < b < 1$?  What does that imply about the {\em sign\/} of $k_b$ for $0 < b < 1$?  Explain your reasoning.


\end{enumerate}
\end{enumerate}
\subsubsection{Differentiating exponential functions}
\label{subsubsec:differentiating-exponential-functions}
...
\item Differentiate the following functions.
  ...
  \begin{enumerate}
  \item $7e^{\displaystyle 3x}$


  \item $Ce^{\displaystyle kx}$, where $C$ and $k$ are constants.

  \item $1.5e^{\displaystyle t}$


  \item $1.5e^{\displaystyle 2t}$


  \item $2e^{\displaystyle 3x} - 3e^{\displaystyle 2x}$


  \item $e^{\displaystyle \cos t}$



\end{enumerate}
\item Find partial derivatives of the following functions.

  \begin{enumerate}
  \item $e^{\displaystyle xy}$


  \item $3x^{\displaystyle 2}e^{\displaystyle 2y}$


  \item $e^{\displaystyle u} \, \sin v$
...
\label{subsubsec:powers-of-e}



\begin{enumerate}
\item Simplify the following and rewrite as powers of $e$.  For each, explain your work, citing any theorems you use.
...
\end{enumerate}
\item Use the second property in Theorem 3  to explain why
	\[
\lim_{t \rightarrow - \infty} \exp(t) = 0.
\]


\item This purpose of this exercise is to prove the fourth property listed in Theorem~3: $\exp(rs) = (\exp(s))^r$, for all real numbers $r$ and $s$.  The idea of the proof is the same as for Theorem~1: show that two different-looking functions solve the same initial value problem, thus demonstrating that the functions must be the same.  The initial value problem is
	\[
y' = ry \qquad y(0) = 1.
\]
  ...
  \begin{enumerate}
  \item Show that $P(t) = \exp(rt)$ solves the initial value problem.  (You need to use the chain rule.)


  \item Show that $Q(t) = (\exp(t))^r$ solves the initial value problem.  (Here use the chain $Q = u^r$, where $u = \exp(t)$.  There is a bit of algebra involved.)


  \item From parts (a) and (b), and the fact that an initial value problem has a unique solution, it follows that $P(t) = Q(t)$, for every $t$.  Explain how this establishes the result.


\end{enumerate}
\end{enumerate}
\subsubsection{Solving $y'= ky$ using $e^t$}
\label{subsubsec:solving-y-ky-using-et}



\begin{enumerate}
\item {\bf Poland and Afghanistan}.\index{Poland}\index{Afghanistan}  Refer to problem 25 in chapter~1.2.

  \begin{enumerate}
  \item Write out the initial value problems that summarize the information about the populations $P$ and $A$ given in parts (a) and (b) of problem 21.

  \item Write formulas for the solutions $P$ and $A$ of these initial value problems.

  \item Use your formulas in part (b) (and a calculator) to find the population of each country in the year 2005.  What were the populations in 1965?



\end{enumerate}
\item {\bf Bacterial growth}.\index{growth!bacterial} Refer to problem
26 in chapter~1.2.

  \begin{enumerate}
  \item Assuming that we begin with the colony of bacteria weighing 32 grams, write out the initial value problem that summarizes the information about the weight $P$ of the colony.

  \item Write a formula for the solution $P$ of this initial value problem.

  \item How much does the colony weigh after 30 minutes?  after 2 hours?



\end{enumerate}
\item {\bf Radioactivity}.\index{radioactivity}  Refer to problem~27 in chapter~1.2.

  \begin{enumerate}
  \item Assuming that when we begin the sample of radium weighs 1~gram,  write out the initial value problem that summarizes the information about the weight $R$ of the sample.

  \item How much did the sample weigh 20 years ago?  How much will it weigh 200 years hence?



\end{enumerate}
\item {\bf Intensity of radiation}.\index{radiation} As gamma rays
...
large; they are used as radiation shields.
  ...
  \begin{enumerate}
  \item Write down a differential equation which models the intensity of
gamma rays $I(x)$ as a function of distance $x$.

  \item Some materials, such as lead, are better shields than others,
such as air.  How would this difference be expressed in your
differential equation?

  \item Assume the unshielded intensity of the gamma rays is $I_0$.
...
\end{enumerate}
\item In this problem you will find a solution for the initial value problem $y'=ky$ and $y(t_0)=C$.  (Notice that this isn't the original initial value problem, because $t_0$ was 0 originally.)


  \begin{enumerate}
  \item Explain why you may assume $y=Ae^{kt}$ for some constant $A$.

  \item Find $A$ in terms of $k, C$ and $t_0$.


\end{enumerate}
\end{enumerate}
\subsubsection{Solving other differential equations}
\label{subsubsec:solving-other-differential-equations}



\begin{enumerate}
\item {\bf Newton's law of cooling}.\index{Newton's law of cooling}  Verify that
	\[
Q(t)=70e^{-.1\,t} + 20
\]
...
\end{enumerate}
\item In {\em An Essay on the Principle of Population}, written in
...
suppressed by unnatural means).
  ...
  \begin{enumerate}
  \item Write differential equations for the size $P$ of a human population and the size $F$ of the food supply that reflect Malthus' assumptions about growth rates.

  \item Keep track of the population in millions, and measure the food supply in millions of units, where one unit of food feeds one person for one year.  Malthus' data suggested to him that the food supply in Great Britain was growing at about .28 million units per year and the per capita growth rate of the population was 2.8\% per year.  Let $t=0$ be the year 1798, when  Malthus estimated the population of the British Isles was  $P=$ 7 million people.  He assumed his countrymen were on average adequately nourished, so he estimated that the food supply was $F=$ 7 million units of food.  Using these values, write formulas for the solutions $P=P(t)$ and $F=F(t)$ of the differential equations in (a).

  \item Use the formulas in (b) to calculate the amount of food and the population at 25 year intervals for 100 years.  Use these values to help you sketch graphs of $P=P(t)$ and $F=F(t)$ on the same axes.

  \item The per capita food supply in any year equals the ratio $F(t)/P(t)$.  What happens to this ratio as $t$ grows larger and larger?  (Use your graphs in (c) to assist your explanation.)  Do your results support Malthus's prediction?  Explain.



\end{enumerate}
\item {\bf Falling bodies}.\index{falling object} Using the base $e$
...
that the modified expression is still a solution.
  ...
  \begin{enumerate}
  \item If an object that falls against air resistance is $x(t)$ meters
...
page~\pageref{ex:falling body2}.)
  ...
  \item Suppose the coefficient of air resistance is .2 per second.  If a
...
\label{subsubsec:interest-rates}
...
\begin{enumerate}
\item Many credit cards charge interest at an annual rate of 18\%.  If
this rate were compounded monthly what would the effective annual rate
be?


\item In fact many credit cards compound interest daily.  What is the
effective rate of interest for 18\% interest compounded daily?  Assume
that there are 365 days in a year.


\item The assumption that a year has 365 days is, in fact, {\em not\/}
...
\item In fact, once they've obtained the daily rate as 1/360-th of the
...
\item Consider the following advertisement.
\vskip .125in
\centerline{\bf Civic Bank and Trust}
\begin{itemize}
\item Annual rate of interest 6\%.
...
\item There are two advertisements in the newspaper for savings
...
\label{sec:the-logarithm-function}
...
[[NOTE:The doubling time  of a population]]
...
\]
	...
	We can't proceed because one side is expressed in exponential form while the other isn't.  One remedy is to express 2 in exponential form.  In fact, $2 = e^{.693147}$, as you should verify with a calculator.  Then 
	\[
e^{.003\,t} = 2 = e^{.693147}
	 \qquad \mbox{implies} \qquad .003\,t = .693147,
\]
so $t = .693147/.003 = 231.049$.  Thus it will take about 231 years for the population to double.



	To determine the doubling time of the population we had to know the number $b$ for which
	\[
\exp(b) = e^{\displaystyle b} = 2.
\]

This is an aspect 
[[NOTE:Solving an  exponential equation]]
of a very general question: given a positive number $a$, find a number
$b$ for which
	\[
e^{\displaystyle b} = a.
\]
A glance at the graph of the exponential function below shows that, by working backwards from any point $a>0$ on the vertical axis, we can indeed find a unique point $b$ on the horizontal axis which gives us $\exp(b) = a$.

\noindent \includegraphics[width=\linewidth]{../ps/calc1/log1.pdf}
...
[[NOTE:The natural logarithm function]]
...
If we set $y = \ln(x)$, then $x = e^{\displaystyle y}$ and we can restate the last equation as a pair of companion equations:
	\[
e^{\ln(x)} = x \quad \mbox{and} \quad
		\ln(e^{y}) = y.
\]
The first equations says 
[[NOTE:The logarithm and exponential functions are inverses]]
...
\label{subsec:properties-of-the-logarithm-function}
...
\label{subsec:the-derivative-of-the-logarithm-function}
\index{derivative}
...
\end{center}
The picture below 
[[NOTE:Reflection across  the $45^{\circ}$~line]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/log2.pdf}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/log3.pdf}

Finally, since the two graphs are reflections of one another,
\index{microscope}
[[NOTE:Microscopic views at mirror image points]]
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/log4.pdf}

It follows that the slopes of the two lines 
[[NOTE:The slopes  are reciprocals]]
...
Note that one interpretation of this theorem is that the function $\ln(x)$ is the solution to a certain initial value problem, namely 
	\[
\dfrac{dy}{dx} = \dfrac{1}{x} \qquad y(1) = 0.
\]
As was the case with the exponential function, we can now apply Euler's method to this differential equation as an effective way to compute values of $\ln(x)$.  Applications of this idea can be found in the exercises.
...
[[NOTE:Comparing rates  of growth]]
...
The expression $p/x - 1$ is positive when $x$ is 
	[[NOTE:$y = \ln{Q}$ decreases when $x > p$ \ldots]]
less than $p$ and negative when $x$ is greater than $p$.   For $x$ near $p$, the graph of $y$ must therefore look like this:
\begin{center}
...
\end{center}
	...
	[[NOTE:\ldots but $y$ may still  ``level off'']]
...
\]
In other words, 
	[[NOTE:$y$ lies below a line that slopes down 
to~$-\infty$]]
...
\label{subsec:the-exponential-functions-bdisplaystyle-x}
\index{function!exponential}

	We have come to adopt the exponential function $\exp(x) = e^x$ as the natural one for calculus, 
	[[NOTE:One exponential function is enough]]
...
Two functions $f$ and $g$ are {\bf inverses} if
\begin{align*}
	f(g(a)) &=  a  \\
	\mbox{and} \ g(f(b)) &=  b
\end{align*}
...
\subsubsection{[[EX:1]].}
Suppose $f(x)=\exp(x)$ and $g(x)=\ln(x)$.  Then the
equations
\begin{align*}
...
\subsubsection{[[EX:2]].}
Suppose $f(x)= x^2$ and $g(x) = \sqrt{x}$.
	[[NOTE:$x^2$ is invertible  on $x \geq 0$]]
...
\subsubsection{[[EX:3]].}
Suppose $f(x) = \sin(x)$ and $g(x)=\arcsin(x)$.  
	[[NOTE:$\sin{x}$ is invertible  on
...
\noindent Exactly as before, we have that the point $(a, b)$ and the point $(b, a)$ are reflections of each other about the line $y=x$.  
	[[NOTE:The graphs of inverse functions are mirror images \ldots]]
...
the rate of change of $f(x)$ at $x = a$, where $a = g(b)$.  
	[[NOTE:\ldots and their derivatives are reciprocals]]
...
\begin{enumerate}
\item Determine the numerical value of each of the following.
...
\item In the text we noted that the function $\ln{x}$ is the solution to the initial-value problem
	\[
\dfrac{dy}{dx} = \dfrac{1}{x} \qquad y(1) = 0,
\]
so that we can use Euler's method to compute values for $\ln{x}$.  Use this method to evaluate $\ln{2}$ to 3 decimal places.  What value of $\Delta x$ gives the desired accuracy?
  ...
  \begin{enumerate}
  \item If you now wanted to calculate $\ln{6}$ to 3 decimals, can you think of a better way to do it than simply starting at $x=1$ and running Euler's method out to $x=6$?  Remember the basic properties of logarithms, and figure out a way to use the results of part (a).

  \item Suppose you had figured out that $\ln{2}=0.693147\ldots$\,.  How would you use Euler's method to calculate $\ln{1300}$ quickly?  You might find the fact that $2^{10}=1024$ helpful.


\end{enumerate}
\item The rate of growth of the population of a particular country is proportional to
 the
...
\item Find the derivatives of the following functions.
  ...
  \item $ \ln(e^{\displaystyle w})$


  \item $\ln(2^{\displaystyle t})$


  \item $\pi\ln(3e^{\displaystyle 4s})$


\end{enumerate}
\item Suppose a bacterial population\index{growth!bacterial} grows so that its mass is
	\[
...
\item Suppose a beam of X-rays whose intensity is $A$ rads (the ``rad'' is a unit of radiation)\index{radiation} falls perpendicularly on a heavy concrete wall.  After the rays have penetrated $s$ feet of the wall, the radiation intensity has fallen to
	\[
...
\item Virtually all living things take up carbon as they grow.  This
...
\item Atmospheric pressure is a function of altitude.  Assume that at
...
\item An important concept in many economic analyses is the idea of
      ...
      annual interest rate.  What is it?
  ...
  \item At what interest rate do \$5,000 now and
      \$8,000 in ten years have the same present value?


\end{enumerate}
\item Use properties of exp to prove the following properties of the logarithm.
  ...
  \item $\ln(m/n) = \ln(m) - \ln(n)$.

  \item $\ln(m^n) = n \ln(m)$.


\end{enumerate}
\item Use a graphing program to find a good numerical approximation to
...
(e^x)/\Delta x$.
  ...
  \item What is the relationship between the values you got in parts (a)
and (b)?


\end{enumerate}
\item Find a solution (using $\ln{x}$) to the differential equation
	\[
...
\item Find a formula using the natural logarithm function giving the
solution of $y'= a/x$ with $y(1)=b$.

  \begin{enumerate}
  \item Solve $P' = 2/t$ with $P(1)=5$.



\end{enumerate}
\item Find the domain and range of each of the following pairs of
inverse functions.

  \begin{enumerate}
  \item $f(x) = x^2$ (restricted to $x \geq 0$) and $g(x)=\sqrt{x}$.

  \item $f(x) = \sin(x)$ (restricted to $-\pi/2 \leq x \leq \pi/2\/$) and
$g(x) = \arcsin(x)$.


\end{enumerate}
\item Show that $f(x) = 1/x$ equals its own inverse.  What are the
domain and range of $f$?


\item Let $n$ be a positive integer. and let $f(x)=x^n$.  What is an
inverse of $f$?  How do we need to restrict the domain of $f$ for it
to have an inverse?  Caution: the answer depends on $n$.


\item What is the inverse $g$ of the function $f(x)=1-3x$?
...
\end{enumerate}
\item What is an inverse of $f(x) = x^2-4$?


\item Use the relationship between the derivatives of a function and
its inverse to find the indicated derivatives.

  \begin{enumerate}
  \item $g'(100)$ for $g(x)=\sqrt{x}$.

  \item $g'(\sqrt{2}/2)$ for $g(x)=\arcsin(x)$.

  \item $g'(1/2)$ for $g(x)=1/x$.


\end{enumerate}
\item Use Theorem 2 and the fact that $(x^2)'=2x$ to derive the
formula for the derivative of $\sqrt{x}$.

  \begin{enumerate}
  \item Use Theorem 2 and the fact that $(x^n)'=nx^{n-1}$ to derive the
formula for the derivative of $\sqrt[n]{x}$



\end{enumerate}
\item Compare the rates of growth of $e^x$ and $b^x$ for both $e < b$
and $1 < b < e$.


\end{enumerate}
\section{The Equation $y' = f(t)$}
\label{sec:the-equation-y-ft}
...
[[NOTE:The motion of  a falling body \ldots]] will express the rate of
...
We can call upon our knowledge of the rules of differentiation to find $x$.  Consider $-gt$ first.  
        [[NOTE:$-gt$ is the derivative  of $-gt^2/2$]]
...
[[NOTE:$v_0$ is the derivative  of $v_0t$ and of $v_0t + b$]] 
...
[[NOTE:The antiderivative  of a function]]
...
Note that a function 
        [[NOTE:All the functions $F(x) +C$ are antiderivatives of $F'(x)$]]
...
chapter 3 (see [[REF:page146]]) can be ``turned around'' to become a list of
functions and their antiderivatives.  Note that the antiderivative
column should really be labelled ``an antiderivative'' since we could
add a constant to any of the listed functions and still have an
antiderivative for the function in the first column.
...
[[NOTE:Every power of $x$ has an antiderivative]]
...
\label{subsec:eulers-method-revisited}
...
is $y = 4t^3 - \cos{t} + C$.  In such a case we have a shortcut to solving the differential equation without needing to use Euler's method.  Often, though, there is no formula for an antiderivative of $f(t)$---even when $f(t)$ itself has a simple formula.  There is no formula for the antiderivative of $\cos(t^2)$, or $\sin{t}/t$, or $\sqrt{1 + t^3}$, for instance.  In other cases, $f(t)$ may not even be given by a formula.  It may be a data function, given as a graph made by a pen tracing on a moving sheet of graph paper.
        ...
        [[NOTE:The volume  of a reservoir  varies over time]]
...
$V$ over these 14 days.

The following table tells us two things: first, how much $V$ changes
over each two-day period; second,
[[NOTE:The accumulated change in $V$]]\index{change!accumulated}
the {\em total\/} change in $V$ that has accumulated by the end of
each period. Since $\Delta t = 2$ days, we calculate $\Delta V$ by
       \[
...
[[NOTE:Calculating just  the accumulated  change in~$y$]]
...
[[NOTE:The accumulated change in $y$]]
...
\begin{verbatim}
  ...
  NEXT k
\end{verbatim}
TABLE is a modification of the program SIRVALUE ([[REF:page65]]).  To
emphasize the fact that it is the accumulated change that matters
rather than the actual value of $y$, we have modified the program
accordingly.  Note that {\tt accumulation} always starts at 0, no
matter what the initial value of $y$ is.  The first line of the
program takes advantage of a capacity most programming languages have
to define functions which can then be referred to elsewhere in the
program.
...
([[REF:page69]]), we can change the program TABLE into one that will {\em
  plot\/} the values of $y$.  In the following program all those
changes are made, and one more besides: we have increased the number
of steps to 400 to get a closer approximation to the true values of
$y$.\index{program!PLOT} The output of PLOT is shown immediately
below.

\noindent \includegraphics[width=\linewidth]{../ps/calc1/pic260.pdf}
...
\end{verbatim}
        ...
        Let's compare our reservoir model with population growth.  The rate at which a population grows depends, in an obvious way, on the size of the population.  By contrast, the rate at which the reservoir fills does {\em not\/} depend on how much water there is in the reservoir.  
        [[NOTE:Exogenous and endogenous factors]]
It depends on factors {\em outside\/} the reservoir: rainfall and consumption.  These factors are said to be {\bf exogenous} (from the Greek {\em exo-}, ``outside'' and {\em -gen\/}, ``produced,'' or ``born'').\index{exogenous factor}  The opposite is called an {\bf endogenous} factor (from the Greek {\em endo-}, ``within'').\index{endogenous factor}  Evaporation is an endogenous factor for the reservoir model; population size is certainly an endogenous factor for a population model.  
...
\dfrac{dy}{dt} = f(t)
\]
Thus, we can view this section as dealing with models that involve exogenous factors.  



[[NOTE:The differential equation of motion for a falling body, $dx/dt
...
\begin{enumerate}
\item Find a formula $y = F(t)$ for a solution to the differential equation $dy/dt = f(t)$ when $f(t)$ is
...
\end{enumerate}
\item Find $G(5)$ if $y = G(x)$ is the solution to the initial value problem
        \[
\dfrac{dy}{dx} = \dfrac{1}{x^2} \qquad y(2) = 3.
\]


\item Find $F(2)$ if $y = F(x)$ is the solution to the initial value problem
        \[
\dfrac{dy}{dx} = \dfrac{1}{x} \qquad y(1) = 5.
\]


\item Find $H(3)$ if $y = H(x)$ is the solution to the initial value problem
        \[
\dfrac{dy}{dx} = x^3-7x^2+19 \qquad y(-1) = 5.
\]


\item Find $L(-2)$ if $y = L(x)$ is the solution to the initial value problem
        \[
\dfrac{dy}{dx} = e^{3x} \qquad y(1) = 6.
\]


\item Sketch the graph of the solution to the initial value problem
        \[
...
over the interval $0 \leq x \leq 4\pi$.
  ...
  \begin{enumerate}
  \item By finding a suitable antiderivative, evaluate $y(2)$.
...
finding a formula for the antiderivative.)
  ...
  \begin{enumerate}
  \item What is the slope of the solution graph at $x = 0$?  Does your graph show this?

  \item How many peaks (local maxima) does the solution have on the interval $0 \leq x \leq 5$?

  \item What is the maximum value that the solution achieves on the interval $0 \leq x \leq 5$?  For which value of $x$ does this happen?
...
\end{enumerate}
\item What is the accumulated change in $y$ if $dy/dt = 3t^2 - 2t$ and $t$ increases from 0 to 1?  What if $t$ increases from 1 to 2?  What if $t$ increases from 0 to 2?

  \begin{enumerate}
  \item Sketch the graph of the accumulated change in $y$ as a function of $t$.  Let $0 \leq t \leq 2$.



\end{enumerate}
\item Here's another problem for which there is no formula for an antiderivative. Sketch the graph of the solution to the initial value problem
        \[
...
on the interval $0 \leq x \leq 40$.  [Note: $\sin{x}/x$ is not defined when $x = 0$, so take the initial value of $x$ to be .00001.  That is, use $y(.00001) = 0$.]
  ...
  \begin{enumerate}
  \item How many peaks (local maxima) does the solution have on the interval $0 \leq x \leq 40$?

  \item What is the maximum value of the solution on the interval $0 \leq x \leq 40$?  For which $x$ is this maximum achieved?
