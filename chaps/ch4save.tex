...
\footnote{Differential equations and initial value problems}
...
Euler's method.
\footnote{If we use Leibniz's notation for derivatives, a differential
...
\footnote{Models can  provide successive approximations  to reality}\index{successive approximation}
...
\footnote{Constant  per capita growth}
...
\footnote{Use Euler's method  to find $R(t)$}\index{Euler's method}
...
\footnote{The graph of $R$  curves up}
...
\includegraphics{../ps/calc1/pic178.pdf}
...
\footnote{It is a good idea to think qualitatively about the functions determined by a differential equation and make some rough estimates before doing extensive calculations.  Your sketches may help you see ways in which the model doesn't correspond to reality.  Or, you may be able to catch errors in your computations if they differ noticeably from what your estimates led you to expect.}
...
{\bf The second model}.  One way out of the problem of unlimited growth is to modify equation (1) to take into account the fact that any given ecological system 
	\footnote{The carrying capacity of the environment}
...
Check to see that this equation really does have the behavior claimed for it---namely, that a population of 25,000 rabbits neither grows or declines.  Notice also that $R'$ is positive as long as $R$ is less than 25000, so the population increases.  However, as $R$ approaches 25000, 
	\footnote{The graph of $R(t)$ levels off near $R = 25000$}
...
\includegraphics{../ps/calc1/pic180.pdf}
...
\index{population model!two-species}
	...
	\footnote{Introduce predators}\index{predator}
...
\end{align*}
These are 
\footnote{Lotka--Volterra equations with bounded growth} 
...
\includegraphics{../ps/calc1/rf1.pdf}
	...
	\footnote{How rabbits respond to changes in the initial fox population}
...
\includegraphics{../ps/calc1/rf2.pdf}
\begin{center}
...
\footnote{While a term like ``hectorabbits'' is deliberately whimsical,
...
\includegraphics{../ps/calc1/hare1.pdf}
\begin{center}
...
\footnote{Differential equations give instructions for Euler's method}
...
\footnote{Equations have {\em solutions}}
...
\footnote{Substitute $y = \sqrt{t}$\\ into the differential equation}
equation:
	$$
...
Since $2t$ and $1/2t^2$ are different functions, the two sides are unequal and the equation is therefore false.  
	...
	\footnote{A {\em solution\/} makes\\ the equation true}
...
the two sides are equal---no matter what value $C$ happens to have.  
	\footnote{A differential equation can have infinitely many solutions}
...
\footnote{An initial value problem has  only one solution}
...
\begin{center}
% TODO: picture environment (manual conversion needed)
...
\footnote{There are special methods to solve particular equations, but other methods work generally}
...
provide solutions.
\footnote{Most initial value problem have one,  and only one, solution.  Euler's method  will find it.}
...
years, with $t=0$ in 1990.  In this notation, \footnote{Models for world population growth} the two models are
	$$
	\begin{array}{rll}
...
\index{formula}
	...
	$$
First of all, 
	\footnote{Checking the  initial condition}
the formula satisfies the initial condition $Q(0)=5$:\index{initial condition}
	...
	$$
	To check that it 
	\footnote{Checking the differential equation}
...
Therefore, the right-hand side is equal to $.015\,u^{-6}$.  But so is the left-hand side, so $Q(t)$ is indeed a solution to the differential equation
	...
	\dfrac{dQ}{dt} = .015\,Q^{1.2}.
	$$
\footnote{Notice two things about this result.  First, when we work with
...
Consider what happens, though, as $t$ approaches this special value 241.6\ldots .  The 
	\footnote{$Q$ ``blows up'' as $t \to 241.6\ldots$}
...
\includegraphics{../ps/calc1/pic204a.pdf}
...
\mbox{}
\includegraphics{../ps/calc1/pic204b.pdf}
\begin{center}
...
\footnote{Graphs are made with logarithmic scales particularly when the
  ...
  when both axes are logarithmic, the result is called a {\em
	...
	\footnote{The initial value problem has a solution only for $t < 241.6\ldots$}
...
Of course, the multiplier that appears here is .69, not .02, so $P(t) = 2^t$ is not a solution to our problem.
	...
	\footnote{Exponential functions satisfy $dy/dt = ky$}
...
In other words, {\em $k_b$ is the slope of the graph of $P(t) = b^t$ at the origin}.
	...
	\footnote{The correct exponential function has slope .02 at the origin}
...
\mbox{}
\includegraphics{../ps/calc1/pic207.pdf}
\begin{center}
...
\footnote{How do parameters affect solutions?}
...
\footnote{Supergrowth parameters}
...
Here is the formula for a function that solves this 
	\footnote{The supergrowth solution with parameters}
...
\footnote{Uncertainty in  the size of $\tau$}
...
analysis, assuming that $k = .015$ and $A = 5$.
	...
	\footnote{How an error in $k$ propagates}
...
Thus $\Delta \tau \approx -16106 \cdot \Delta k$.  For example, if the uncertainty in the value of $k = .015$ is $\Delta k = \pm .001$, then the uncertainty in $\tau$ is about $\mp 16$ years.
	...
	\footnote{How an error in $A$ propagates}
we first write
	$$
...
This says that it takes a 5\% error in $A$ 
	\footnote{How sensitive $\tau$ is to errors in $k$ and $A$}
...
\label{supergrowth errors-end}
	...
	\footnote{Special methods give formulas; general methods are numerical}
...
\end{minipage}
\includegraphics{../ps/calc1/spring9.pdf}
...
\label{subsec:the-equation-y-ky}
	...
	\footnote{A simple and natural model of growth and decay}
...
In this section we will look at the solutions to differential equations of this form from two different vantage points.  On the one hand, we already have named functions which solve such equations---the exponential functions.  On the other hand, the fact that Euler's method produces the same functions will allow us to prove properties of such functions and  to compute their values effectively.
	...
	\footnote{Exponential solutions---variable base~\ldots}
...
Exercise~7 in the previous section (page~\pageref{ex:base 2}) \footnote{\ldots
...
Then $dy/du = k_b\cdot b^u = k_b\cdot y$, while $du/dt = k/k_b$.  Thus, by the chain rule we have
	...
	\footnote{Advantages of a fixed base}
...
\footnote{The use of the symbol $e$ to denote the base dates back to a
  ...
  ubiquitous numbers in mathematics.}
	...
	\footnote{The general initial value problem for exponential functions}
Finally: 
\begin{center}
% TODO: picture environment (manual conversion needed)
...
\footnote{Finding $e$ with a computer microscope}
...
\footnote{Under successive magnifications: $e = 2.71828\ldots$}
...
\footnote{Finding $e$ by  Euler's method}
...
\end{array}
	...
	\footnote{Expressing $e$ as a limit}
...
\footnote{Euler's method uses only arithmetic}
...
\footnote{Defining a function is not the same as being able to evaluate it}
...
The proof uses the Existence and Uniqueness Principle for differential equations we articulated  	\footnote{The proof of Theorem~1}
...
\index{growth!exponential}
	...
	\footnote{How fast do $x^n$ and $e^x$ become infinite?}
...
\footnote{$x^p$ grows more slowly than $x^q$ if $p < q$}
...
\end{array}
	...
	\footnote{$e^x$ grows more rapidly than any power of $x$}
...
\footnote{The doubling time  of a population}
...
so $t = .693147/.003 = 231.049$.  Thus it will take about 231 years for the population to double.
	...
	$$
This is an aspect 
\footnote{Solving an  exponential equation}
...
\includegraphics{../ps/calc1/log1.pdf}
...
\footnote{The natural logarithm function}
...
\footnote{The logarithm and exponential functions are inverses}
...
\end{center}
The picture below 
\footnote{Reflection across  the $45^{\circ}$~line}
...
$y=x$.
\includegraphics{../ps/calc1/log2.pdf}
...
\includegraphics{../ps/calc1/log3.pdf}
Finally, since the two graphs are reflections of one another,
\index{microscope}
\footnote{Microscopic views at mirror image points}
...
\includegraphics{../ps/calc1/log4.pdf}
It follows that the slopes of the two lines 
\footnote{The slopes  are reciprocals}
...
\footnote{Comparing rates  of growth}
...
The expression $p/x - 1$ is positive when $x$ is 
	\footnote{$y = \ln{Q}$ decreases when $x > p$ \ldots}
...
\end{center}
	...
	\footnote{\ldots but $y$ may still  ``level off''}
...
\end{center}
	...
	$$
In other words, 
	\footnote{$y$ lies below a line that slopes down 
to~$-\infty$}
...
\index{function!exponential}
	...
	\footnote{One exponential function is enough}
...
{\bf Example 2}.  Suppose $f(x)= x^2$ and $g(x) = \sqrt{x}$.
	\footnote{$x^2$ is invertible  on $x \geq 0$}
...
{\bf Example 3.}  Suppose $f(x) = \sin(x)$ and $g(x)=\arcsin(x)$.  
	\footnote{$\sin{x}$ is invertible\\ on
...
\noindent Exactly as before, we have that the point $(a, b)$ and the point $(b, a)$ are reflections of each other about the line $y=x$.  
	\footnote{The graphs of inverse functions are mirror images \ldots}
...
the rate of change of $f(x)$ at $x = a$, where $a = g(b)$.  
	\footnote{\ldots and their derivatives are reciprocals}
...
\footnote{The motion of  a falling body \ldots} will express the rate of
...
\label{subsec:antiderivatives}
        ...
        \footnote{\ldots and its solution}
...
We can call upon our knowledge of the rules of differentiation to find $x$.  Consider $-gt$ first.  
        \footnote{$-gt$ is the derivative  of $-gt^2/2$}
...
\footnote{$v_0$ is the derivative  of $v_0t$ and of $v_0t + b$} 
...
\footnote{The antiderivative  of a function}
...
Note that a function 
        \footnote{All the functions $F(x) +C$ are antiderivatives of $F'(x)$}
...
chapter 3 (see page 146) can be ``turned around'' to become a list of
...
\footnote{Every power of $x$ has an antiderivative}
...
is $y = 4t^3 - \cos{t} + C$.  In such a case we have a shortcut to solving the differential equation without needing to use Euler's method.  Often, though, there is no formula for an antiderivative of $f(t)$---even when $f(t)$ itself has a simple formula.  There is no formula for the antiderivative of $\cos(t^2)$, or $\sin{t}/t$, or $\sqrt{1 + t^3}$, for instance.  In other cases, $f(t)$ may not even be given by a formula.  It may be a data function, given as a graph made by a pen tracing on a moving sheet of graph paper.
        ...
        \footnote{The volume  of a reservoir  varies over time}
...
\footnote{The accumulated change in $V$}\index{change!accumulated}
...
\footnote{Calculating just  the accumulated  change in~$y$}
...
\footnote{The accumulated change in $y$}
...
\begin{verbatim}
  ...
  NEXT k
\end{verbatim}
TABLE is a modification of the program SIRVALUE (page 65).  To
...
(page 69), we can change the program TABLE into one that will {\em
...
below.
\includegraphics{../ps/calc1/pic260.pdf}
\begin{center}
...
\end{verbatim}
        ...
        \footnote{Exogenous and endogenous factors}
...
\footnote{The differential equation of motion for a falling body, $dx/dt
