...
\footnote{The standard way  to graph solutions}
...
\footnote{A new way  to graph solutions:  as trajectories  in state space} 
...
first 250 months:
\mbox{}
\includegraphics{../ps/calc2/pred1.pdf}
Let's see how the same model looks  
\footnote{Two ways of representing the same solution graphically}
...
\label{`trajectory'}
\includegraphics{../ps/calc2/pred2.pdf}
...
\end{minipage}
\includegraphics{../ps/calc2/predtime.pdf}
...
\footnote{The differential equations indicate how the state changes}
...
30$, with $\Delta t =1$.
\label{`vecfield'}
\includegraphics{../ps/calc2/vecf1.pdf}
Several things are immediately clear from this picture:  
\footnote{Arrows indicate the change in state\ldots}
...
\footnote{\ldots and trajectories are tangent to the arrows}
...
following:
\includegraphics{../ps/calc2/vecf2.pdf}
\enlargethispage*{30pt}
The net effect of this construction is thus 
\footnote{How to construct a geometrical visualization of a dynamical system}
...
\end{itemize}
\includegraphics{../ps/calc2/phasepor.pdf}
There is almost too much detail 
\footnote{Simplifying the picture}   
...
\footnote{Divide the state space into regions}
...
\label{`equilibrium'}
\includegraphics{../ps/calc2/pred3.pdf}
...
\footnote{There are simple trajectories: three are just points\ldots}
...
three other 
\footnote{\ldots and three are straight line segments} 
...
\footnote{Different kinds of  equilibrium points}
...
following pictures:
\includegraphics{../ps/calc2/eqpoints.pdf}
...
\footnote{Saddle point equilibrium}
...
\footnote{Attractors and repellors}  
...
\includegraphics{../ps/calc2/pend1a.pdf}
...
\includegraphics{../ps/calc2/pendvf.pdf}
We have included in this diagram the lines where $v'=0$ (the vertical lines at every multiple of $\pi$) and the line where $x'=0$ (the horizontal line at $v=0$).  Note that the velocity vectors are horizontal on the lines corresponding to $v'=0$ and are vertical on the line corresponding to $x'=0$.
        \footnote{The equilibrium points}
...
\includegraphics{../ps/calc2/pendpha.pdf}
\enlargethispage*{30pt}
...
\item There are the wavy trajectories moving from left to right across
  ...
  value of $v$ is \footnote{Trajectories from left to right in the state
...
\item There are the closed loops.  Here $x$ oscillates back and forth
  \footnote{Oscillations of the pendulum correspond to closed loops in the
...
\footnote{A center: a neutral equilibrium}
...
\item There are the trajectories connecting the saddle points.  These
  ...
  direction.  In fact, these \footnote{A connected curve in the phase
...
\footnote{First integrals  can give equations  of trajectories}
...
$E$:
\includegraphics{../ps/calc2/pendpha2.pdf}
...
\footnote{A mathematical model can be used to think about the feasibility of a proposed explanation}
...
\footnote{A capsule summary of the immune response: Vertebrates have a
...
\label{`amprob'}
\includegraphics{../ps/calc2/andmay1.pdf}
...
\item Since the velocity vectors differ so much in their size,
  we \footnote{Knowing the direction of the trajectories is often
...
\item Since the range of values we want to represent is so
  ...
  space.
\footnote{Expressing graphical information over several different orders of magnitude}
...
\includegraphics{../ps/calc2/andmay3.pdf}
...
\footnote{Phase portraits under the microscope}
...
\includegraphics{../ps/calc2/locbeha1.pdf}
...
\includegraphics{../ps/calc2/locbeha2.pdf}
...
\footnote{Near an ordinary point, a vector field is  almost constant}
...
\footnote{Near an ordinary point all trajectories look  the same}
...
Near an 
\footnote{Equilibrium points  are different}
...
\includegraphics{../ps/calc2/locbeha3.pdf}
...
\footnote{Linear approximation of the vector field}
...
and $f_2$ are just
\footnote{To linearize a vector field, linearize the functions that determine it}
...
\footnote{General form for the local linearization at  an equilibrium point\ldots}
...
For completeness, 
\footnote{\ldots and at  a generic point}
...
\footnote{An intuitive classification of equilibrium points} 
...
\footnote{The existence or not of straight line trajectories and how to
...
\includegraphics{../ps/calc2/classeq3.pdf}
...
\includegraphics{../ps/calc2/classeq4.pdf}
...
equilibrium in closed loops.  \index{neutral point}\index{equilibrium
  point!neutral}
\includegraphics{../ps/calc2/classeq5.pdf}
...
\footnote{The condition for a fixed-line trajectory}
...
\includegraphics{../ps/calc2/limcyc.pdf}
...
\footnote{Limit cycles give models for cyclic behavior} 
...
\includegraphics{../ps/calc2/ctr.pdf}
...
\includegraphics{../ps/calc2/cycle.pdf}
\enlargethispage*{20pt}
...
\includegraphics{../ps/calc2/torus.pdf}
...
system above.
\includegraphics{../ps/calc2/lor.pdf}
