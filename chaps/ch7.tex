...
\footnote{Many patterns  are periodic}	
...
happens in nature?
\footnote{Predator and  prey populations  fluctuate periodically}
...
\includegraphics{../ps/calc2/lynx1.pdf}
{\bf Example 2: The earth's orbit}.  The earth orbits the sun, 
\footnote{The position and the shape of the earth's orbit both fluctuate
  periodically}
...
\footnote{Fluctuations in the climate appear in the geological record}
...
\includegraphics{../ps/calc2/sunspot.pdf}
...
\footnote{Data can have  both periodic and  random influences}
...
\footnote{Defining a  periodic function}	
...
\includegraphics{../ps/calc2/period1.pdf}
...
\footnote{A periodic function  has many periods\ldots}	
...
\footnote{\ldots but we call the smallest positive one  {\em the\/} period}
...
\footnote{The frequency of  a cycle is the  reciprocal of its period}	
...
\includegraphics{../ps/calc2/period2.pdf}
...
second.
	...
	\footnote{The units for measuring frequency over time}	
...
\footnote{Functions can be periodic over other units as well}
...
\footnote{Why $\cos{t}$ and $\sin{t}$\\ are periodic}	
...
\includegraphics{../ps/calc2/sincos.pdf}
\includegraphics{../ps/calc2/circle2.pdf}
The circular functions are constructed 
\footnote{Radian measure}\index{radian}\index{radian measure}\index{angle}	
...
\footnote{Changing the frequency}
with that of $y = \sin(4t)$, below.  
\includegraphics{../ps/calc2/sin4t.pdf}
...
\footnote{Constructing a  circular function with  a given frequency}	
...
\leq t \leq 1$.
\includegraphics{../ps/calc2/cos10pit.pdf}
We will denote the frequency by $\omega$,
	\footnote{Frequency $\omega$}
...
\footnote{Amplitude}	\index{amplitude}
...
function.
\includegraphics{../ps/calc2/ampli.pdf}
...
\includegraphics{../ps/calc2/phase1.pdf}
...
\includegraphics{../ps/calc2/phase2.pdf}
...
\linebreak
\end{minipage}
\includegraphics{../ps/calc2/spring8.pdf}
...
\footnote{Newton's laws of motion}	
...
\footnote{The linear oscillator}	
...
\footnote{The solution with  fixed parameters}	
...
b &= 5 \ \mbox{per sec}, &\qquad  b &= 9 \ \mbox{per sec}.
	\end{alignat*}
\includegraphics{../ps/calc2/spring1.pdf}
...
\footnote{The solution  for arbitrary  parameter values}	
...
is satisfied.
	...
	\footnote{The formula {\em proves\/} the motion is periodic}	
...
We can see the relation between the motion and the parameters in the graph 
	\footnote{Graph of the general linear oscillator}
below (in which we take $a > 0$).
\includegraphics{../ps/calc2/spring2.pdf}
...
spring is linear.
\linebreak
\includegraphics{../ps/calc2/spring3.pdf}	 
...
\footnote{A non-linear oscillator}\index{oscillator!non-linear}
problem:
\begin{alignat*}{2}
x' &= v,                  &\qquad  x(0) &= a,   \\
...
\footnote{Comparing a hard spring to a linear spring}
...
frequency.
\includegraphics{../ps/calc2/spring4.pdf}
The non-linear spring behaves 
\footnote{The effect of amplitude on acceleration}	
...
\includegraphics{../ps/calc2/spring5.pdf}
...
\footnote{Large-amplitude oscillations}	
...
\includegraphics{../ps/calc2/spring6.pdf}
As a consequence, 
\footnote{The frequency of a non-linear spring depends on  its amplitude}
...
.95$ Hz.
	...
	\footnote{A mathematical comment}	
...
\includegraphics{../ps/calc2/spring7.pdf}
There are other ways we might have modified 
	\footnote{Other non-linear springs}	
...
last three pages.
	...
	\footnote{The harmonic oscillator}	
...
\footnote{A computable definition of the  sine and cosine}	
...
\includegraphics{../ps/calc2/pend1.pdf}
...
\includegraphics{../ps/calc2/pend2.pdf}
...
\footnote{Newton's laws  produce a model  of the pendulum}	
...
\footnote{The pendulum is  a soft spring}	
...
\includegraphics{../ps/calc2/pend3.pdf}
...
\includegraphics{../ps/calc2/pend4.pdf}	
...
\includegraphics{../ps/calc2/pend5.pdf}
\includegraphics{../ps/calc2/pend6.pdf}
...
\footnote{The pendulum balanced on end}	
...
{\bf equilibrium}
\footnote{Stable and unstable equilibrium solutions}
...
\footnote{Why do populations fluctuate?}	
...
\footnote{A predator-free equilibrium \ldots}	
...
\footnote{\ldots upset by predators}	
...
period:
\includegraphics{../ps/calc2/may1.pdf}
In the other models of periodic behavior we have studied, 
\footnote{Solutions with various initial conditions \ldots}	
...
\includegraphics{../ps/calc2/may2.pdf}
...
cases.
\footnote{\ldots all have the  same amplitude  and frequency}	
...
\footnote{Can we {\em prove\/} that systems have periodic oscillations?}
...
\footnote{The virtue   of a formula}	
...
\footnote{It may seem that what we should do is develop more methods for
...
\footnote{Are we back  where we started?}	
...
\footnote{Changes in speed \ldots modified}
...
\footnote{Showing $E$ is a constant}	
...
In particular, we have thus shown that $B_2$ (the position of the pendulum after it's gone over and back) $= a$ (the position of the pendulum at the beginning).  Thus the value for $x$ and the value for $v$ are the same in stage 5 and in stage 1---the two stages are mathematically indistinguishable.
	\footnote{\ldots and that the oscillations are periodic}
