...
\footnote{Some properties of contagious diseases} 
...
\footnote{The idea of a mathematical model}
...
\footnote{The quantities that our model analyzes}
...
\footnote{Some quantitative questions that the graphs raise}
...
\footnote{Knowing rates, we can predict future values}  
...
\footnote{The equation $S' = -470$   is a model}
...
\footnote{Predictions depend on the initial value, too}
...
\footnote{The first piece of the $S$-$I$-$R$ model}
\[
...
\footnote{Are you uneasy about our claim that {1/14-th} of the infected
...
\footnote{Contacts are proportional to both $S$ and $I$}
...
\footnote{Here is the second piece of the $S$-$I$-$R$ model}
\[
...
\setlength{\unitlength}{1pt}
\end{minipage}
\footnote{We haven't talked about the units in which to measure $a$ and
...
\mbox{}
\footnote{Here is the complete $S$-$I$-$R$ model}
\begin{alignat*}{2}
		S' &=  -a S I,           &    &     \\
...
\footnote{You should not draw the conclusion that the only use of rate
...
\footnote{The model is part of mathematics; it only approximates reality}
...
\footnote{Rates are continually changing; this affects the calculations}
...
third.
\footnote{Stop and do  the calculations}
...
\footnote{Go forward a day  and then back again}
...
\footnote{The differences measure how rough an estimate is}
...
\footnote{$\Delta$ stands for a change, a difference, or a step}
...
together.  The diagram below, called a {\bf flow chart},\index{flow
  chart} 
\footnote{The flow chart  forms a loop} 
...
million, though).
\footnote{A computer program  will carry out  the three steps}
...
\footnote{Rate equations\index{rate equation} have always been at the
...
\footnote{The threshold determines whether there will be an epidemic}
...
other contexts.
\footnote{We often draw out of a few particular experiences a lesson that
...
\footnote{Functions and  their notation}
...
In the function $S(t)$ the variable $t$ is called the {\bf
  input}\index{input}
\footnote{A function has input and output}
...
\footnote{Part of math is simply learning the language} important later in
...
\footnote{A graph is a function rule given visually}
...
\footnote{If $y$ depends on $x$, then $\Delta y$ depends on $\Delta x$}
...
\footnote{The defining property of a linear function}
...
that it 
\footnote{Stop and do  the translation}
says 
\[
...
as a {\bf multiplier}.
\footnote{It is easier to understand why the usual symbol for {\em
...
\footnote{If $x$ and $y$ have units, so does $m$}
...
\footnote{A function can have several input variables} 
...
\footnote{Some functions  depend on parameters}
...
\footnote{How the next three chapters are connected} 
...
\includegraphics{../ps/calc1/trig1.pdf}
...
\end{alignat*}
\includegraphics{../ps/calc1/trig2.pdf}
...
\footnote{Simplicity determines the choice of radians for measuring angles}
...
\footnote{How do we get  values for the  circular functions?}  
...
\footnote{Read a program line by line from the top} The code here is
...
be several letters long.  A few lines later $S'$ is typed out as {\tt
  Sprime}, for instance.  The instruction on the sixth line \footnote{Line
...
\footnote{How a program computes new values}
...
{\tt A} = current {\tt B}.''
\footnote{Notice that the instructions {\tt A = B} and {\tt B = A} mean
...
\footnote{Lines 7 and 19:  the loop}
...
continues down the program to line 19.  \footnote{How the program stops}
