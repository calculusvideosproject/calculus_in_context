...
\label{chap:a-context-for-calculus}
...
\label{sec:the-spread-of-disease}

\subsection{Making a Model}
\label{subsec:making-a-model}
...
[[NOTE:Some properties of contagious diseases]] 
...
[[NOTE:The idea of a mathematical model]]
...
[[NOTE:The quantities that our model analyzes]]
...
[[NOTE:Some quantitative questions that the graphs raise]]
...
\label{subsec:a-simple-model-for-predicting-change}
...
[[NOTE:Knowing rates, we can predict future values]]  
...
[[NOTE:The equation $S' = -470$   is a model]]
...
[[NOTE:Predictions depend on the initial value, too]]
...
\label{subsec:the-rate-of-recovery}
...
[[NOTE:The first piece of the $S$-$I$-$R$ model]]
\[
...
[[NOTE:Are you uneasy about our claim that {1/14-th} of the infected
...
{\bf Other diseases}.  What can we say about the recovery
...
\label{subsec:the-rate-of-transmission}
...
[[NOTE:Contacts are proportional to both $S$ and $I$]]
...
[[NOTE:Here is the second piece of the $S$-$I$-$R$ model]]
\[
...
[[NOTE:We haven't talked about the units in which to measure $a$ and
...
\label{subsec:completing-the-model}
...
\mbox{}
[[NOTE:Here is the complete $S$-$I$-$R$ model]]
\begin{alignat*}{2}
		S' &=  -a S I,           &    &     \\
...
[[NOTE:You should not draw the conclusion that the only use of rate
  equations is to model an epidemic.  Rate
  equations have a long history, and they have been put to many uses.
  Isaac Newton (1642--1727) introduced them to model
...
[[NOTE:The model is part of mathematics; it only approximates reality]]
...
\label{subsec:analyzing-the-model}
...
[[NOTE:Rates are continually changing; this affects the calculations]]
...
Consider a measles epidemic in a school population
...
Rate equations:
	...
	Initial values: when $t = 0$,
		...
		$\displaystyle S = 45400, \qquad I = 2100, \qquad R = 2500.$
}}}
\end{picture}
\end{center}
...
{\bf Tomorrow}.  From our earlier discussion, $R' = 2100/14
...
{\bf Several days hence}.  According to the model, we
...
third.
[[NOTE:Stop and do  the calculations]]
...
\subsubsection{Yesterday.}
We already pointed out, on
...
[[NOTE:Go forward a day  and then back again]]
...
[[NOTE:The differences measure how rough an estimate is]]
...
[[NOTE:$\Delta$ stands for a change, a difference, or a step]]
...
\subsubsection{Using the computer as a tool.}
...
together.  The diagram below, called a {\bf flow chart},\index{flow
  chart} 
[[NOTE:The flow chart  forms a loop]] 
...
million, though).
[[NOTE:A computer program  will carry out  the three steps]]
...
[[NOTE:Rate equations have always been at the
...
{\bf Analysis without a computer}.  A computer is a powerful
...
[[NOTE:The threshold determines whether there will be an epidemic]]
...
\label{subsubsec:reading-a-graph}
...
\begin{enumerate}
\item When does the infection hit its peak?\index{maximum} How many
people are infected at that time?


\item Initially, how many people are susceptible?  How many days does
it take for the susceptible population to be cut in half?


\item How many days does it take for the recovered population to reach
25,000?  How many people {\em eventually\/} recover?  Where did you
look to get this information?


\item On what day is the size of the infected population increasing
most rapidly?  When is it decreasing most rapidly?  How do you know?


\item How many people caught the illness at some time during the first
...
\item Copy the graph of $R$ as accurately as you can, and then
...
\label{subsubsec:a-simple-model}
...
\begin{enumerate}
\item Suppose the initial susceptible population was 20,000 on
Wednesday.  Use the model to answer the following questions.

  \begin{enumerate}
  \item How many susceptibles will be left ten days later?

  \item How many days will it take for the susceptible population to
vanish entirely?

  \item How many susceptibles were there on the previous Sunday?

  \item How many days before Wednesday were there 30,000 susceptibles?
...
\label{subsubsec:mark-twains-mississippi}
\index{Mississippi}
...
\begin{enumerate}
\item According to Twain's data, what is the exact {\bf
...
\item Twain wrote his book in 1884.  Suppose the Mississippi that Twain
wrote about had been 1100 miles long; how long would it have become in
1990?


\item Twain does not tell us how long the Lower Mississippi was in 1884
...
\item Suppose $t$ is the number of years since 1884.  Write a formula
...
\item From your answer to question 10, you know how long the river was
...
\item Use your formula to find what $L$ was a million years ago.  Does
your answer confirm Twain's assertion that the river was ``upwards of
1,300,000 miles long'' then?


\item Was the river ever 1,300,000 miles long; will it ever be
...
\label{subsubsec:the-measles-epidemic}
...
\begin{enumerate}
\item Calculate the current rates of change $S'$, $I'$, and $R'$ when
$t = 1$, and then use these values to determine $S$, $I$, and $R$ one
day later\label{`measles'}.


\item In the previous question you found $S$, $I$, and $R$ when $t =
...
\item {\bf Double the time step}.  Go back to the starting time $t = 0$
...
\item {\bf There and back again}.\index{there and back
...
and with a pair of time steps of $\Delta t = \pm 1$ .
  ...
  \begin{enumerate}
  \item ($\Delta t = \pm 2$).  Using the values of $S$, $I$, and $R$ at
...
2500?
  ...
  \item ($\Delta t = \pm 1$).  Now make a new start, using the values
\begin{alignat*}{3}
...
differ from the original values 45,400, 2100, 2500?
  ...
  \item Which process leads to a {\em smaller\/} set of differences: a
...
\end{enumerate}
\item {\bf Quarantine}.\index{quarantine} One of the ways to treat an
...
alters the {\em transmission coefficient}.
  ...
  \begin{enumerate}
  \item Suppose a quarantine is put into effect that cuts in half the
chance that a susceptible will fall ill.  What is the new transmission
coefficient?

  \item On page~\pageref{`threshold'} it was determined that whenever
...
for $S$.  What is the new threshold?
  ...
  \item Suppose we start with $S$ = 45,400.  Does quarantine eliminate
...
cases?
  ...
  \item Since the new transmission coefficient is not small enough to
...
with $S = 45400$.
  ...
  \item Suppose the initial susceptible population is 45,400.  What is
...
\label{subsubsec:other-diseases}




\begin{enumerate}
\item Suppose the spread of an illness similar to measles is modelled
...
do not apply here.
  ...
  \begin{enumerate}
  \item Roughly how long does someone who catches this illness remain
infected?  Explain your reasoning.

  \item How large does the susceptible population have to be in order for
the illness to take hold---that is, for the number of cases to
increase?  Explain your reasoning.

  \item Suppose 100 people in the population are currently ill.
According to the model, how many (of the 100 infected) will recover
during the next 24 hours?

  \item Suppose 30 {\em new\/} cases appear during the same 24 hours.
What does that tell us about $S'$?

  \item Using the information in parts (c) and (d), can you determine how
large the current susceptible population is?


\end{enumerate}
\item Construct the appropriate $S$-$I$-$R$ model for a measles-like
...
day, and the infection is transmitted in only one contact out of six.
  ...
  \begin{enumerate}
  \item How small does the susceptible population have to be for this
illness to fade away without becoming an epidemic?


\end{enumerate}
\item Consider the general $S$-$I$-$R$ model for a measles-like
illness:
\begin{alignat*}{2}
S' &=  - a S I,         &     &     \\
I' &= \hphantom{-}a S I & -\, & b I,  \\
...
\end{alignat*}
  ...
  \begin{enumerate}
  \item The threshold level for $S$---below which the number of infected
...
expression?\label{`setthresh'}
  ...
  \item Consider two illnesses with the same transmission coefficient
...
\label{subsubsec:what-goes-around-comes-around}
...
\begin{enumerate}
\item Draw a compartment diagram for such an illness.  Besides having
...
\item Suppose immunity is lost after about six weeks.  Show that you
can set $c = 1/42$ per day, and explain your reasoning carefully.  A
suggestion: adapt the discussion of recovery in the text.


\item Suppose this illness lasts 5 days and it has a transmission
...
\item We saw in the text that the model for an illness that confers
...
\item For a mild illness that confers permanent immunity, the size of
the recovered population can only grow.  This question explores what
happens when immunity is only temporary.

  \begin{enumerate}
  \item Will $R$ increase or decrease if
\[
S = 45400, \qquad I = 2100, \qquad R = 2500?
\]


  \item Suppose we shift 20000 susceptibles to the recovered population
(so that $S = 25400$ and $R = 22500$), leaving $I$ unchanged.  Now,
will $R$ increase or will it decrease?

  \item Using a total population of 50,000, give two other sets of values
for $S$, $I$, and $R$ that lead to a decreasing $R$.

  \item In fact, the relative sizes of $I$ and $R$ determine whether $R$
...
\end{enumerate}
\item {\bf The steady state}.  Any illness that confers only temporary
...
population is 50,000.
  ...
  \begin{enumerate}
  \item What must be true if the inflow and outflow to the $I$
compartment are to balance?

  \item What must be true if the inflow and outflow to the $R$
compartment are to balance?

  \item If neither $I$ nor $R$ is changing, then the model must be at the
steady state.  Why?

  \item What is the value of $S$ at the steady state?

  \item What is the value of $R$ at the steady state?  A suggestion: you
...
\label{sec:the-mathematical-ideas}
...
[[NOTE:We often draw out of a few particular experiences a lesson that
  ...
  means literally ``drawing from.'' Of course
...
[[NOTE:Functions and  their notation]]
...
In the function $S(t)$ the variable $t$ is called the {\bf
  input}\index{input}
[[NOTE:A function has input and output]]
...
\subsubsection{Some technical details.}
It is important not to confuse an
...
[[NOTE:Part of math is simply learning the language]] important later in
...
[[NOTE:A graph is a function rule given visually]]
...
[[NOTE:If $y$ depends on $x$, then $\Delta y$ depends on $\Delta x$]]
...
[[NOTE:The defining property of a linear function]]
...
% --- converted boxed semantic block to observation ---
\begin{observation}
\bf A linear function is one that 
			satisfies \boldmath 
...
that it 
[[NOTE:Stop and do  the translation]]
says 
\[
...
[[NOTE:It is easier to understand why the usual symbol for {\em
...
% --- converted boxed semantic block to observation ---
\begin{observation}
\bf For a linear function satisfying
		\boldmath $\Delta y = m\cdot\Delta x$, \\
\bf the coefficient \boldmath $m$ is \\
...
[[NOTE:If $x$ and $y$ have units, so does $m$]]
...
\noindent$\bullet$ \bf The initial-value form}.\index{initial-value form} Here
...
\noindent$\bullet$ \bf The interpolation form}.\index{interpolation form} This
...
\noindent$\bullet$ \bf The slope-intercept form}.\index{slope-intercept form}
...
\label{subsec:functions-of-several-variables}
\index{function!of several variables}

\subsubsection{Language and notation.}
...
[[NOTE:A function can have several input variables]] 
...
[[NOTE:Some functions  depend on parameters]]
...
\label{subsec:the-beginnings-of-calculus}
...
[[NOTE:How the next three chapters are connected]] 
...
\label{subsubsec:functions-and-graphs}




\begin{enumerate}
\item Sketch the graph of each of the following functions.  Label each
...
indicate
  ...
  \begin{enumerate}
  \item $y =  -\tfrac{1}{2} x + 3$ \hfill c) $5x + 3y = 12$
\hfill \mbox{}
...
\end{enumerate}
\item Graph the following functions.  Put labels and scales on the
axes.

  \begin{enumerate}
  \item $V = .3Z -1$; \hfill b)  $W = 600 - P^2$. \hfill \mbox{}


\end{enumerate}
\item Sketch the graph of each of the following functions.  Put labels
...
\item Determine the following values:
\begin{center}
...
\item True or false.  Give reasons for your answers: if you say true,
explain why; if you say false, give an example that shows why it is
false.

  \begin{enumerate}
  \item For every non-zero number $x$, $r(r(x)) = j(x)$.

  \item If $a > 1$, then $s(a) > 1$.

  \item If $a > b$, then $s(a) > s(b)$.

  \item For all real numbers $a$ and $b$, $s(a+b) = s(a) + s(b)$.

  \item For all real numbers $a$, $b$, and $c$, $D(D(a,b),c)) = D(a,D(b,c))$.


\end{enumerate}
\item Find all numbers $x$ for which $Q(x) = r(Q(x))$.
...
1/x$ is the set of all non-zero real numbers.
  ...
  \begin{enumerate}
  \item Find the natural domains of $Q$ and $H$.

  \item Find the natural domains of $P(z) = Q(r(z))$;
$R(v) = r(Q(v))$.

  \item What is the natural domain of the function $W(t) =
...
\label{subsubsec:computer-graphing}
...
\begin{enumerate}
\item Graph the function $f(x) = .6 x + 2$ on the interval $-4 \leq x \leq 4$.

  \begin{enumerate}
  \item What is the $y$-intercept of this graph?  What is the $x$-intercept?

  \item Read from the graph the value of $f(x)$ when $x = -1$ and when $x
...
\end{enumerate}
\item Graph the function $f(x) = 1 - 2 x^2$ on the interval $-1 \leq x
\leq 1$.\label{roots}


  \begin{enumerate}
  \item What is the $y$-intercept of this graph?  The graph has two
...
to one decimal place of accuracy.
  ...
  \item Regraph $f(x)$ on the interval $.70 \leq x \leq .71$ to get two
...
What is the $x$-intercept?
  ...
  \end{enumerate}
\end{enumerate}
\subsubsection{The circular functions}
Graphing packages ``know'' the familiar
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/trig1.pdf}
...
\noindent \includegraphics[width=\linewidth]{../ps/calc1/trig2.pdf}
...
[[NOTE:Simplicity determines the choice of radians for measuring angles]]
...
[[NOTE:How do we get  values for the  circular functions?]]  
...
\begin{enumerate}
\item Graph the function $f(x) = \sin(x)$ on the interval $-2 \leq x
\leq 10$.

  \begin{enumerate}
  \item What are the $x$-intercepts of $\sin(x)$ on the interval $-2 \leq
x \leq 10$?  Determine them to two decimal places accuracy.

  \item What is the largest value of $f(x)$ on the interval $-2 \leq x
\leq 10$?  Which value of $x$ makes $f(x)$ largest?  Determine $x$ to
two decimal places accuracy.

  \item Regraph $f(x)$ on the very small interval $-.001 \leq x \leq
.001$.  Describe what you see.  Can you determine the slope of this
graph?


\end{enumerate}
\item Graph the function $f(x) = \cos(x)$ on the interval $0 \leq x
\leq 14$.  On the same screen graph the {\em second\/} function $g(x)
= \cos(2x)$.

  \begin{enumerate}
  \item How far apart are the $x$-intercepts of $f(x)$?  How far apart
are the $x$-intercepts of $g(x)$?

  \item The graph of $g(x)$ has a pattern that repeats.  How wide is this
pattern?  The graph of $f(x)$ also has a repeating pattern; how wide
is {\em it\/}?

  \item Compare the graphs of $f(x)$ and $g(x)$ to one another.  In
...
vertical or the horizontal direction?
  ...
  \item Construct a {\em new\/} function $f(x)$ whose graph is the same
...
\end{enumerate}
\item The aim here is to find a solution to the equation $\sin x =
...
help us.)
  ...
  \begin{enumerate}
  \item Graph the two functions $f(x) = \sin(x)$ and $g(x) = \cos(3x)$ on
the interval $0 \leq x \leq 1$.

  \item Find a solution of the equation $\sin(x) = \cos(3x)$ that is
accurate to six decimal places.

  \item Find {\em another\/} solution of the equation $\sin(x) =
\cos(3x)$, accurate to four decimal places.  Explain how you found it.



\end{enumerate}
\item Use a graphing program to make a sketch of the graph of each of
...
\item Graph on the same screen the following three functions:
\[
...
Use the interval $-2 \leq x \leq 1.2$.
  ...
  \begin{enumerate}
  \item Which function has the largest value when $x = -2$?

  \item Which is climbing most rapidly when $x = 0$?

  \item Magnify the picture at $x = 0$ by resetting the size of the
...
\label{subsubsec:proportions-linear-functions-and-models}
...
\begin{enumerate}
\item Go back to the three functions given in problem 1.  For each
...
\item You should be able to answer all parts of this problem without
ever finding the equations of the functions involved.

  \begin{enumerate}
  \item Suppose $y=f(x)$ is a linear function with multiplier $m=3$.  If
$f(2)=-5$, what is $f(2.1)$? $f(2.0013)$?  $f(1.87)$? $f(922)$?

  \item Suppose $y=G(x)$ is a linear function with multiplier $m=-2$.  If
$G(-1)=6$, for what value of $x$ is $G(x)=8$? $G(x)=0$? $G(x)=5$?
$G(x)=491$?

  \item Suppose $y=h(x)$ is a linear function with $h(2)=7$ and $h(6)=9$.  What is $h(2.046)$? $h(2+a)$?


\end{enumerate}
\item In Massachusetts there is a sales tax of 5\%.\index{sales tax}
...
\item Suppose $W = 213 - 17\, Z$.  How does $W$ change when $Z$ changes
...
\item In the following table, $q$ is a linear function of $p$.  Fill
...
\end{center}
  ...
  \begin{enumerate}
  \item Find a formula to express $\Delta q$ as a function of $\Delta p$,
and another to express $q$ as a function of $p$.


\end{enumerate}
\item {\bf Thermometers}.\index{thermometer} There are two scales in
...
\end{center}
  ...
  \begin{enumerate}
  \item Which represents a larger change in temperature, a Celsius degree
or a Fahrenheit degree?

  \item How many Fahrenheit degrees does it take to make the temperature
go up one Celsius degree?  How many Celsius degrees does it take to
make it go up one Fahrenheit degree?

  \item What is the multiplier $m$ in the equation $\Delta F = m \cdot
...
What is the relation between $\mu$ and $m$?
  ...
  \item Express $F$ as a linear function of $C$.  Graph this function.
Put scales and labels on the axes.  Indicate clearly the slope of the
graph and its vertical intercept.

  \item Express $C$ as a linear function of $F$ and graph this function.
How are the graphs in parts (d) and (e) related?  Give a clear and
detailed explanation.

  \item Is there any temperature that has the same reading on the two
...
\end{enumerate}
\item {\bf The Greenhouse Effect}.\index{greenhouse effect} The
...
PPM per year during the 1960s.
  ...
  \begin{enumerate}
  \item At that rate, how many years does it take for the concentration
to increase by 5 PPM; by 15 PPM?

  \item At the beginning of 1960 the concentration was about 316 PPM.
What would it be at the beginning of 1970; at the beginning of 1980?

  \item Draw a graph that shows CO$_2$ concentration as a function of the
time since 1960.  Put scales on the axes and label everything clearly.

  \item The {\em actual\/} CO$_2$ concentration at the South Pole was 324
...
values.
  ...
  \item Using the actual concentrations in 1970 and 1980, calculate a new
...
the CO$_2$ concentration at the beginning of 1990.
  ...
  \item Using the rate of .8 PPM per year that held during the 1960s,
...
\end{enumerate}
\item {\bf Thermal Expansion}.\index{thermal expansion} Measurements
...
when the temperature increases to 80$^\circ$F.
  ...
  \begin{enumerate}
  \item How long is the bar when the temperature is 60$^\circ$F?
100$^\circ$F?

  \item What is the multiplier that connects an increase in length
$\Delta L$ to an increase in temperature $\Delta T$?

  \item Express $\Delta L$ as a linear function of $\Delta T$.

  \item How long will the bar be when $T = 0^\circ$F?

  \item Express $L$ as a linear function of $T$.

  \item What temperature change would make $L$ = 100.01 inches?

  \item For a {\em steel\/} bar that is also 100 inches long when the
...
when the temperature is increased; aluminum or steel?
  ...
  \item How long will this steel bar be when $T = 80^\circ$F?


\end{enumerate}
\item {\bf Falling Bodies}\label{`falling'}.  In the simplest model of
...
of proportionality is approximately~32.
  ...
  \begin{enumerate}
  \item A ball is falling at a velocity of 40 feet/sec after 1 second.
How fast is it falling after 3 seconds?

  \item Express the change in the ball's velocity $\Delta v$ as a linear
function of the change in time $\Delta t$.

  \item Express $v$ as a linear function of $t$.
...
\end{align*}
  ...
  \item At what rate is the distance increasing after 1 second? After 2
seconds?  After 3 seconds?

  \item Is $d$ a linear function of $t$?  Explain your answer.
...
\end{enumerate}
\item In the equation $P' = k\, P$, above, explain why the units for
$k$ are
\[
...
\item {\bf Poland and Afghanistan}.\index{Poland}\index{Afghanistan}\label{Poland} In
...
was 21.6 persons per year per thousand.
  ...
  \begin{enumerate}
  \item Let $P$ denote the population of Poland and $A$ the population of
Afghanistan.  Write the equations that govern the growth rates of
these populations.

  \item In 1985 the population of Poland was estimated to be 37.5 million
...
the larger net growth rate.
  ...
  \item On the average, how long did it take the population to increase
...
\end{enumerate}
\item {\bf Bacterial Growth}.\index{growth!bacterial} A colony of
...
links the growth rate to the size of the population.
  ...
  \begin{enumerate}
  \item What is $\Delta P$ if $\Delta t$ = 1~minute?  Estimate how long
it would take to make $\Delta P$ = .5~grams.


\end{enumerate}
\item {\bf Radioactivity}.\index{radioactivity} In radioactive decay,
...
\item {\bf Cooling}.\index{cooling}\label{`cooling'} Suppose a cup of
...
element here is that $C'$ is proportional, not to $C$, but to the {\em
  ...
  \begin{enumerate}
  \item Write an equation that relates $C'$ and $C$.  It will contain a
proportionality constant $k$.  How did you indicate that the coffee is
{\em cooling\/} and not {\em heating up\/}?

  \item When the coffee is at 180$^{\circ}$F it is cooling at the rate of
9$^{\circ}$F per minute.  What is $k$\/?

  \item At what rate is the coffee cooling when its temperature is
120$^{\circ}$F?

  \item Estimate how long it takes the temperature to fall from
...
\label{sec:using-a-program}
\index{program}

\subsection{Computers}
\label{subsec:computers}
\index{computer}
...
\label{subsubsec:the-program-sir}
...
[[NOTE:Read a program line by line from the top]] The code here is
...
be several letters long.  A few lines later $S'$ is typed out as {\tt
  Sprime}, for instance.  The instruction on the sixth line [[NOTE:Line
...
[[NOTE:How a program computes new values]]
...
[[NOTE:Notice that the instructions {\tt A = B} and {\tt B = A} mean
...
continues down the program to line 19.  [[NOTE:How the program stops]]
...
\begin{enumerate}
\item Run the program to confirm that it reproduces what you have
already calculated by hand (table, page~\pageref{`threeday'}).



\item On a copy of the program, mark the instructions that carry out
the following tasks:


  \begin{enumerate}
  \item give the input values of $S$, $I$, and $R$;


  \item say that the calculations take us 1 day into the future;


  \item carry out step II (see page~\pageref{`flowchart'});


  \item carry out step III;


  \item give us the output values of $S$, $I$, and $R$;


  \item take us once around the whole loop;


  \item say how many times we go around the loop.



\end{enumerate}
\item Delete all the lines of the program from line 7 onward (or else
type in the first 6 lines).  Will this program run?  What will it do?
Run it and report what you see.  Is this what you expected?



\item Starting with the original SIR program on page~\pageref{prog},
...
\item Using the 17-line program you constructed in the previous
...
\label{subsubsec:programs-to-practice-on}
...
\begin{enumerate}
\item When Program 1 runs it will print the values of $A$ and $B$ that
are current when the program stops.  What values will it print?  Type
in this program and run it to verify your answers.


\item What will Program 2 do when it runs? Type in this program and run
it to verify your answers.


\item After each line in Program 3 write the values that $A$ and $B$
...
\item What output does Program 4 produce?  Type in the code and run the
program to confirm your answer.


\item What is the difference between the code in Program 5 and the code
in Program 4?  What is the output of Program 5?  Does it differ from
the output of Program 4?  If so, why?


\item What output does Program 6 produce?  Type in the code and run the
program to confirm your answer.


\begin{center}
\begin{description}
\item[\textbf{Program 7}]
...
\item Program 7 prints five lines of output.  What are they?  Type in
the program and run it to confirm your answers.


\item What is the output of Program 8?  Type in the program and run it
to confirm your answers.


\item Describe exactly how the {\em codes\/} for Programs 8 and 9
differ.  How do the {\em outputs\/} differ?


\end{enumerate}
\subsubsection{Analyzing the measles epidemic}
\label{subsubsec:analyzing-the-measles-epidemic}




\begin{enumerate}
\item Alter the program SIR to have it calculate estimates for $S$,
$I$, and $R$ over the first {\em six\/} days.  Construct a table that
shows those values.\index{estimate}



\item Alter the program to have it estimate the values of $S$, $I$, and
$R$ for the first {\em thirty\/} days.

  \begin{enumerate}
  \item What are the values of $S$, $I$, and $R$ when $t = 30$?

  \item According to these figures, on what day does the infection peak?
What values do you get for $S$, $I$, and $R$?

  \item How can you reconcile the value you just got for $S$ with the
value we obtained algebraically on page~\pageref{`threshold'}?



\end{enumerate}
\item By adding an appropriate PRINT statement after line 10 you can
...
\item According to these estimates, on what day do the largest number
new infections occur?  How many are there?  Explain where you got your
information.


\item On what day do you estimate that the largest number of recoveries
occurs?  Do you see a connection between this question and 17~(b)?


\item On what day do you estimate the infected population grows most
rapidly?  Declines most rapidly?  What value does $I'$ have on those
days?


\item Alter the original SIR program so that it will go {\em
...
with those in the text (page~\pageref{`threeday'}).
  ...
  \begin{enumerate}
  \item Estimate the values of $S$, $I$, and $R$ {\em three\/} days
before today.



\end{enumerate}
\item According to the $S$-$I$-$R$ model, when did the infection begin?
That is, how many days before today was the estimated value of $I$
approximately 1?



\item {\bf There and back again}.\index{there and back again} Use the
...
\item In exercises 22--26 at the end of section~2
...
\label{subsubsec:a-model-of-an-orchard}
...
\begin{enumerate}
\item Find a formula for the straight segment of the new graph of
$Y(N)$ on the interval $40 \leq N \leq 180$.  What is the formula for
$T(N)$ on the same interval?


\item What are the formulas for $T(N)$ when $0 \leq N \leq 40$ and when
$180 \leq N$?  Graph $T$ as a function of $N$.  Describe the graph in
words.


\item What is the maximum possible total yield $T$?  For which $N$ is
this maximum attained?


\item Suppose the endpoints of the sloping segment were $P$ and $Q$,
...
\label{subsubsec:rate-equations}
\index{rate equation}
...
\begin{enumerate}
\item {\bf Radioactivity}.\index{radioactivity} From exercise~25 of
...
\item {\bf Poland and Afghanistan}.\index{Poland}\index{Afghanistan} If
...
\item {\bf Falling bodies.}  If $d$ and $v$ denote the distance fallen
